...
| How far a CHARACTER has got through a guide | `QuestLedgerStore.Guides` (manual, per character) + `Core/GuideProgressRouting.cs`. An objective with a `RewardKey` is a Sky turn-in: the ledger REFUSES that tick and the read routes to `SkyQuestCompleted` — one fact, one store. Progress and authoring are different axes; neither is readable off the other. Nothing renders it yet (P1b) |
