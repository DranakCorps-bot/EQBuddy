...
| Turning a guide into the rows every surface draws | `UI.Shared/GuideChecklistProjection.cs` — `Apply` for Sky (matched on the REWARD KEY) and `ApplyEpic` for Epic (matched on the CLASS — an epic class has one quest, so its section groups collapse into one). A guided group's rows are REPLACED by its objectives; an unguided one comes back the same object (lock 5). `UI.Shared/GuidePresentation.cs` owns every word, and **each of the six is drawn in exactly one place**: WHAT is the row title, `who · where` the row's line, when+why+how the hover (the phone has no hover, so they ride the row — trap 35). An unanswered question draws nothing |
| Where a guide step's tick lives | `UI.Shared/GuideProgressRouter.cs` — FOUR homes, one writer each: `SkyTurnIn` (a `RewardKey`), **`SkyItem`** (an acquire-shaped step naming exactly one of ITS reward's checklist rows → that row's own box, so the loot auto-tick lights it), **`EpicItem`** (an objective whose ID names an `EpicQuestChecklistItem` → that row's box; `EpicCompleteToggle` stays the per-class done store), else the guide ledger. Group-scoped: one class's Wind Rune — or epic row — is never another's. `Drawn()` is the one producer of "which objectives this tab shows" (the Epic classic-era lens) |
