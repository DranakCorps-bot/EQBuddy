...
| Turning a guide into the rows every surface draws | `UI.Shared/GuideChecklistProjection.cs` — `Apply` for Sky (matched on the REWARD KEY), `ApplyEpic` for Epic (matched on the CLASS — an epic class has one quest, so its section groups collapse into one), and **`ForQuest` for a NORMAL quest** (matched on `Guide.QuestName`, one group or null — the General tab draws one quest at a time). A guided group's rows are REPLACED by its objectives; an unguided one comes back the same object (lock 5). **`WalkthroughRows` is the one producer of "which rows the guide BODY draws"** — a `LedgerItemName` row is the turn-in item row the surface already draws, and drawing it twice is trap 4 on the first screen a player opens (DRA-46). `UI.Shared/GuidePresentation.cs` owns every word, and **each of the six is drawn in exactly one place**: WHAT is the row title, `who · where` the row's line, when+why+how the hover (the phone has no hover, so they ride the row — trap 35). An unanswered question draws nothing |
