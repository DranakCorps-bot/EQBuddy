## 2026-09-12 ~2:45 AM CT — Fable: DRA-66 CHARACTER SETUP — rename the shell's Home room + show the class EQBuddy believes, with a correction — the plan (Founder smoke). Executor kicks only after Helm SIGN.

To: Helm

**Seat:** `fable-dra66-charsetup`, plan only — no product code was written in this seat. Founder ask (Paperclip DRA-66): *rename shell "Home" to Character Setup (or equivalent clear label), and show what class EQBuddy thinks the character is with a way for the user to update/correct it.* Helm's ~2:05 AM CT scope honored throughout: KEEP the Character identity block, the DRA-63 readiness catch-up paste (Bags/Achievements/Factions/Spellbook), and Recent session; Soft LEAVE dropping the catch-up block, reinventing the Go-to rail, Pages / Play Console / tag / `release.ps1` / signing / Evolved restore / Founder mail. Executor is one Claude/Opus seat, kicked after Helm SIGNs this plan.

### 0. What exists today (evidence, verified in-tree this session)

- **The room's name is one function and its readers already follow it.** `ShellPages.Label(Home)` = "Home" (`ShellPages.cs:217`); `RailLabel` falls through to `Label` for every page but Quests. Readers: the rail row + tooltip (`RailRow.cs:41,51`), the window title (`ShellWindow.xaml.cs:360`, `"EQBuddy — {Label}"`), the Ctrl+K palette (`ShellWindow.xaml.cs:542`), and the E2E close/minimize calls (`ShellHostTests.cs:1423,1461` pass `ShellPages.Label(ShellPage.Home)` — they follow a rename with zero edits). **The Guide precedent is the law here** (ShellPages' own comment): the LABEL moved on 2026-09-08 and the enum member + wire `Key` "home" did NOT, because `page:room` is persisted, arrives in `EQBUDDY_SHELL`, and lives in every E2E dump.
- **The class EQBuddy believes already has one producer.** `MainWindow.ClassSourceFor` (`MainWindow.xaml.cs:1477`) → `CharacterClasses.Resolve(unlocked, inferred, picks)`: the achievements dump leads (game's statement), log inference fills in, Quest Tracker picks are LAST and only WIDEN (#104's lens rule, Bevel-locked). `CharacterClasses.SourceLabel` is the ONE table of source sentences (Bevel/Helm-signed 2026-08-23: the phone must not compose a second verb around it). Readers today: buff sets, the unlock filter, Gear Locker, the quest window's note (`QuestsView.xaml.cs:846`), and the phone (`classSourceLabel` rides the wire as a finished string — an old page draws a new source with no page edit, trap 32 untouched).
- **Home's Identity block deliberately omits class** — `HomeReadout.IdentityDetail`'s own comment: *"Zone rather than level or class."* That was a HomeReadout decision, not a Bevel lock (the signed Home pre-design rules on empty states and block count, not on the detail line's fields), so amending it is this plan's to propose, with a Bevel notify.
- **There is no way to CORRECT the guess.** Picks may widen identity, never narrow it; nothing lets a player whose log reads as Warrior · Cleric say "I am a Warrior." The store that would carry it exists: `QuestLedgerStore._byCharacter` already holds `Classes` (picks) and `UnlockedClasses` (dump) per character key (`QuestLedgerStore.cs:499,506`).
- **"Setup" is already a different thing in this window.** The first-run Setup LAYER (`SetupView`/`SetupReadout`, OE-6) draws OVER this room; its headline is "Set EQBuddy up", its re-open door is Settings → Behavior → "Setup…", and `SetupReadout.ReopenNote` says "Home lists whatever is still missing" (pinned by `SetupReadoutTests.cs:126`).
- **The rename's blast radius outside `src/`:** `scripts/shoot.ps1` hard-codes Title `'EQBuddy — Home'` in the `shell-home`, `shell-home-narrow`, `shell-home-ready`, and `setup-screen` recipes (:790–:964) plus the widget/shell disambiguation comment (:3571) — trap 53's exact shape; `site/index.html:325` names the rail rows in prose. The phone offers no "Home" screen (`CompanionSurfaces.PageFor` uses `ShellPage.Home` only as a fallthrough), so no phone label moves.
- **Content locks that stand:** the owner's 2026-09-06 "Home stays guidance hub (not metrics)" lock is about CONTENT and is untouched by a rename; DRA-63's three blocks (Identity · Readiness · Recent session) are Helm's KEEP list verbatim.

### 1. Decisions (defaults chosen; David vetoes from DECISIONS.md, Helm signs here)

- **D1 — The label becomes "Character"; the key, the enum member, and the shot filenames do not move.** Rail row, tooltip, palette and window title all read "Character" via one edit to `ShellPages.Label` (RailLabel keeps falling through — no Quests-style split). Why not the Founder's literal "Character Setup": (a) the rail's own rule is short nouns — a two-word row forces the collapsed rail early; (b) "Setup" already names the first-run layer that draws OVER this room and its Settings → Behavior door, so a rail row called "Character Setup" gives one window two different things named Setup. "Character" is the equivalent clear label the ask allows: the room IS the character — who they are, what they play, what EQBuddy still needs, where they left off. **If Helm or the Founder prefers the literal two-word label, that is a one-line swap in the same place and the rest of this plan is unchanged — say so in the SIGN.** Wire key `home`, `ShellPage.Home`, `EQBUDDY_SHELL=home`, and the `shell-home*` shot filenames all stay (Guide precedent; trap 21 — renamed shot files orphan every doc that cites them). `Describe(Home)` stays — "Who you are playing…" is truer, not less true.
- **D2 — The Identity block gains a class line, read from the one resolution that already exists.** `HomeRoom` becomes another caller of `MainWindow.ClassSourceFor` and draws `HomeReadout.ClassLine(classes, source)`: the classes joined " · " with `CharacterClasses.SourceLabel` in the parenthetical idiom `QuestsView.xaml.cs:846` already uses (no second verb composed — the table's own sentence, verbatim). `ClassSource.Unknown` draws a sentence, never a blank (silent no-ops are broken): class not known yet, EQBuddy reads it from your log as you play, and the Achievements catch-up one block down answers it at once — pointing at the ⧉ row that already ships the command, so no new command literal and no `GameCommandsTests` row.
- **D3 — Correction is a per-character STATEMENT that replaces the guess and never the game's fact.** New `ClassSource.Stated` + a `stated` parameter in `CharacterClasses.Resolve`, per-character `StatedClassesFor`/`SetStatedClasses` beside picks and dump classes in `QuestLedgerStore` (same store, same key, same persistence — no new file, trap 65 already answered). Precedence: dump entries first, then stated; **a non-empty statement suppresses the `inferred` and `picks` contributions to identity** — the statement exists precisely because the guess is wrong, and union can widen but never un-guess. **It never suppresses the dump**: the dump is the game's own writing, and a player who disagrees with their dump has a stale dump — the Readiness block one block down is already that repair (the same fact/proxy line as trap 4 and DRA-46's refused manual tick). Cost, named: a stated-only player who later unlocks a second class won't see the log widen their identity until they restate or dump — acceptable for an explicit override with a visible clear path. `SourceLabel` gains one row: `Stated => "set by you"` — the phone gets it wire-free. Quest Tracker picks and their filter are untouched (the lens keeps its own job).
- **D4 — The editor is an inline chip strip on the Identity block, and the words live in `HomeReadout`.** A "Set class…" affordance under the class line expands an `EqChip` strip in a `WrapPanel` (trap 25; never hand-build a pill) offering the same class list the Quest Tracker's picker offers (one producer — Executor wires the existing list, no new enumeration), multi-select capped at `CharacterClasses.Max` (3), plus a clear action worded as "Let EQBuddy work it out" that empties the statement and returns to the guess. A caption states what stating does. When the source is Achievements, the editor collapses to a sentence — your achievements dump answers this; run it again if it is out of date — rather than disabled chips (trap 17: disabled without a visual is invisible; a sentence says why). All sentences and the show-editor/collapse state rule live in `HomeReadout` (the WPF layer has no tests); the room keeps the wiring.
- **D5 — The rename carries its whole blast radius in one PR.** `SetupReadout.ReopenNote` reworded ("The Character room lists whatever is still missing…") + its pinning test; `scripts/shoot.ps1` Title strings in the four recipes and the :3571 comment move to `'EQBuddy — Character'` in the same change (trap 53), shot KEYS unchanged; `site/index.html:325` prose updated (repo edit only — Pages publish is Soft LEAVE); `WhatsNew.json` speaks the X-is-now-Y promise by name: **"Home is now Character"** (the 1.99.6 public promise applies to a rename exactly as to a move), plus the class line + correction as their own entry, Founder ask credited (DRA-66). `docs/TestPlan.md` updated.
- **D6 — Repaint and dump facts, or the feature invisibly doesn't exist.** `HomeRoom`'s paint fingerprint folds the resolved classes + source + stated list (trap 72 is the exact bug this invites — the room would keep drawing the moment before the correction). `DebugFacts` gains `shellHomeClass=<joined>` and `shellHomeClassSource=<source>` so E2E asserts the arithmetic, never the screen.

### 2. Delivery — one Executor seat (`dra66-charsetup`), after SIGN

1. `Core/CharacterClasses.cs` — `ClassSource.Stated`, `Resolve(unlocked, stated, inferred, picks)` with the D3 suppression, `SourceLabel` row. `Core/QuestLedgerStore.cs` — `StatedClasses` per character + accessors, persisted with the entry.
2. `MainWindow.ClassSourceFor` passes the stated list; every existing reader answers the corrected class with no further edits.
3. `UI.Shared/HomeReadout.cs` — class line, unknown-state sentence, editor captions + state rule. `UI.Shared/ShellPages.cs` — the label. `UI.Shared/SetupReadout.cs` — ReopenNote.
4. `EQBuddy/HomeRoom.cs` — class line + chip editor wiring, fingerprint, `DebugFacts` keys.
5. Tests: `CharacterClassesTests` (stated narrows a wrong guess — prove-fail; stated never removes a dump class; clear restores the guess; picks stop feeding identity only while stated is non-empty), `SetupReadoutTests`, `ShellTerminologyTests` walk (fall-through round-trip already pins Label==RailLabel for Home), HomeReadout sentence tests. E2E (`ShellHostTests`): the class facts against a staged log that infers a class, and the existing achievements-dump staging asserts the line moves to "from your achievements" when the dump lands (positive event, trap 62). `dotnet build EQBuddy.slnx -c Release` before E2E (trap 64).
6. Shots: `shell-home*` + `setup-screen` batch re-cut with PREDICTED content (trap 23) — this fold absorbs DRA-63's named stale-shot debt; screen mutex honored, named again as debt if the Founder's live session holds the screen. Solarized once.
7. `WhatsNew.json`, `docs/TestPlan.md`, `DECISIONS.md` (label default + precedence default), and a `BEVEL-FEEDBACK.md` note: the rename and the Identity block's new class line amend Bevel's signed Home pre-design surface — notify, not a gate (its locks on empty states and blocks are untouched).

### 3. Acceptance

- **A1** Rail row, tooltip, palette entry and title bar read "Character"; `EQBUDDY_SHELL=home` and every stored `home` address still land; E2E close/minimize pass untouched.
- **A2** Identity block shows the class line in all four source states; Unknown draws the sentence, never blank; the parenthetical uses `SourceLabel` verbatim.
- **A3** Stating narrows a wrong two-class guess to the player's one (prove-failed); a dump class can never be removed by a statement; clearing returns to the guess; the phone's quest header shows "set by you" with no page edit.
- **A4** `shellHomeClass`/`shellHomeClassSource` in the EXPAND dump and asserted from E2E; the room repaints on a statement (fingerprint folds it).
- **A5** Blast radius closed in one PR: SetupReadout note, shoot.ps1 titles, site prose, WhatsNew X-is-now-Y ("Home is now Character"), TestPlan.
- **A6** KEEP list intact: catch-up ⧉ rows, `shellHomeCopyCmd == 4`, Recent session, and no Go-to block returns.

### 4. Verification class and traps

V1–V2: unit + targeted E2E + `scripts/check.ps1`; `build-and-test` + `e2e-windows` stay the merge bar. Read before executing: traps 4, 16, 17, 21, 23, 25, 29, 53, 56, 62, 64, 64b, 72.

### 5. needs-david: none

The Founder asked for the rename and the correction by name; "Character" vs "Character Setup" is inside "or equivalent clear label" and is logged in `DECISIONS.md` for veto (Helm may flip it in the SIGN at one-line cost). The precedence default (statement replaces guess, never the dump) is logged the same way. Nothing here touches the values line, another player, a release, or anything off the machine.
