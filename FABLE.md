...
## 2026-09-11 — PLAN (ready): DRA-61 — landing tray BUILD-loop GIF (DRA-48 follow-up)
To: Claude

**The ask (Founder, DRA-48 follow-up):** landing looks amazing; add a GIF of *building*
the minimized bar, full loop: (1) maximize/expand, (2) select DPS · Progress · Healing ·
Motes · Money · Pet DPS, (3) minimize to the tray with those chips, (4) reorder to
DPS · Pet DPS · Healing. `record-tray-gifs.ps1` covers hover/click/drag/park only — the
claim that it already covers this loop is wrong and stays LEFT.

**The mapping (the plan's real content — the six names onto shipped mechanics):**

- **DPS, Progress (XP%/hr), Healing (HPS)** are the always-on glance trio (SA-1,
  `HudGlance`). They are never *added*; the third slot swaps XP↔HPS by activity. The GIF
  must not fake a toggle for them.
- **Pet DPS, Motes** are ★ stars ON the expanded widget (`StarPet` on Combat's header,
  `StarMotes` on the Motes card). These are the two honest on-camera "adds".
- **Money (Coin)** — its ★ lives in the Progress *window* (the fold's one writer). A
  detour there would double the GIF's length for one tick, so Coin is SEEDED already-on
  and the copy says the trio-plus-coin was the starting loadout. Judgment call, logged
  in DECISIONS.md.
- **"Reorder to DPS · Pet DPS · Healing"** is the SIGNED #422 pet insert: carry the pet
  chip left past the DPS cell's right edge, drop into the insertion gap — the row reads
  DPS · pet · third-slot. The **Healing** ending is the third slot's real swap: append
  live `You healed …` lines to the staged log during the closing beat (the fixture
  script's own "append lines while the app runs" affordance); with no damage in the
  resume window `HudGlance.NextThird` flips XP→HPS on screen. Shipped behavior, staged
  honestly — nothing invented.

**Storyboard — ONE continuous take, `tray-build-loop` (~18 s, 12 fps):**

1. Open on the pristine minimized tray: dot · name · DPS · XP · coin chip · ⤢ ✕
   (seed `MiniStats=['money']`, motes card visible via `MotesCardRestored/Offered`).
2. Click ⤢ (UIA name "⤢") — the widget expands in place, top-left anchored.
3. Star Pet (AutomationId `StarPet`), then Motes (`StarMotes`) — two real clicks.
4. Click – (minimize): the tray returns wearing pet · motes · money beside the trio.
5. Press the pet chip, carry it left past DPS, drop — it joins the always-on row.
6. Append heal lines; the third slot swaps to HPS: **DPS · Pet DPS · Healing**. Rest.

**Recipe changes (`scripts/record-tray-gifs.ps1`):** new `tray-build-loop` entry; a
taller region (the expanded widget grows DOWN from the bar's top-left — H≈790, W from
the bar + slack); per-GIF settings override so the build GIF's seed differs from the
four shipped clips (theirs unchanged, choreography untouched); helpers:
find-by-AutomationId, an all-matches chip lookup (two "N dps" cells exist after the
insert — pick by position; `Require-Chip`'s first-match answer is not enough), a
live-append helper writing parser-true heal lines with now-stamps.

**Landing (`site/index.html`):** new `hudstep` "02 · Build it" between Glance and
Hover-to-peek (renumber the later `stage` labels 02→03, 03→04, 04→05); honest alt text
naming every beat; copy sells "star what you watch; the trio is always on".

**Verification (trap 23 — predict, then look):** extract frames at fixed timestamps
with ffmpeg; assert the predicted states: frame 1's tray has NO pet/motes cell; the
post-minimize frame has pet+motes+money cells; the final frame's row leads
DPS · pet-dps · hps. Illustration lock satisfied: the GIF ships in the same change as
its recipe; `-Gif tray-build-loop` re-records it alone.

**Out of scope (Soft LEAVE, per the card):** Pages enable, Play Console, any gear rec;
no re-record of the four shipped clips.

**needs-david:** none. LIVE ASK Helm when BUILT (the landing is public surface — Helm
signs before it goes live).
