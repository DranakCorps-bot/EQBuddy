## 2026-09-17 ~6:40 AM CT — Fable: DRA-164 PLANE OF SKY ISLAND VIEW — the plan from the Founder's ask (KEEP class view; ADD a Class/Island toggle). Executor kicks only after Helm SIGN.

To: Helm

**Seat:** `fable-dra164-sky-island` (Paperclip DRA-164, claimed 10:55:15Z). Plan only: no
product code from this seat and no Executor kick from it. Paperclip DRA-164 moves to
`in_review` with this PR. **This is not a DRA-149 gate and touches none of that sequence.**
Nothing in any slice touches Pages / Play / tag / signing. **No slice fetches anything and
no curated file is written**: `GuideCatalog.json` and `SkyQuestDefaults.cs` are read, never
edited — the one data gap found (the Efreeti stage, below) is closed parser-side over prose
the catalog already ships. Target: Founder Desktop smoke ~2:00 PM CT today, which is why
this plan is three slices over machinery that already exists rather than a new surface.

The Founder's ask, kept verbatim in shape: KEEP current class-sorted Sky view. ADD toggle:
Class view (existing) vs Island view. Island view: multi-select which classes I am, then
...
- **The multi-select the ask names EXISTS.** The Sky tab's class picker is `EqMultiPicker`
  (its first caller), per character through `QuestLedgerStore.SetClasses`/`ClassesFor`,
  empty pick = the character's own classes off the dump or log. The warrior/monk/druid
  example is: tick three classes in the picker that is already there, flip the new toggle.
  No new picker, no second class store (trap 4).
- **The island fact EXISTS, with two producers already on screen.** Classic rows:
  `SkyIslands.Parse` over each step's `Source` prose → `QuestChecklistLayout.Sky` orders
  rows by island WITHIN a reward and stamps `QuestChecklistRow.IslandHeading`
  (`SkyIslandGroupingTests`; island 1.5 is real, sorting is numeric, multi-island steps
  honor `AppSettings.SkyStepsUnderEveryIsland`). Guided rows: `GuideChecklistProjection.Apply`
  stamps `IslandHeading` = the guide STAGE NAME verbatim — and since Delivery 1 **all 95
  PoS rewards are guided**, so on a tab with a character the stage names are what draws.
- **Survey of the shipped `GuideCatalog.json` — 95 PoS guides, 317 objectives** (script
  parse agreeing with the stage-name distinct-count, per the two-parses standard):
  - **103** under numbered isle stages (`Isle 3: Gorgalosk` 16, `Isle 4: Keeper of Souls`
    13, `Isle 5: The Spiroc Lord` 17+1 (`Isle five - The Spiroc Lord`), `Isle 6: Bazzt
    Zzzt` 17+1, `Isle 7: Sister of the Spire` 17+3, `Isle 8: Eye of Veeshan` 16, `Isle 2`
    2) — every one parses under the SHIPPED `SkyIslands.Parse`, spelled-out "five"
    included.
  - **22** under `The Efreeti drop` — the stage name parses to NOTHING; the fact is in
    the objective's Where, one identical string on all 22: *"Plane of Sky - Isles 1.5, 4
    and 8 respectively."* **The shipped `Parse` reads that as `[1.5]`** — the plural-list
    shape captures only the number touching the word — which would file a three-island
    drop under Island 1.5 alone, a wrong island printed with full confidence. This is the
    one parser change the plan makes (D1, scope-locked).
  - **95** under `The wind rune` — Where says *"any isle"*; parses empty, which is the
    TRUE answer (trash drops anywhere).
  - **95** turn-in objectives (`Turn in to <NPC>`) — hand-ins, not collection. 48 of
    their Where strings mention Isle 1 (the Efreeti Chamber directions), which is exactly
    what a naive Where-prose fallback would misfile as Island-1 gathering work.
  - **2** under `Not placed` (stubs).
- **A finding, not a gate:** with every PoS reward guided, the
  `SkyStepsUnderEveryIsland` checkbox currently changes nothing a player with a character
  sees — the projection replaces the classic rows it repeats. The island view makes that
  setting live again (P4); no separate fix is scheduled.
- **Where the regroup slots in without a second producer:** `RenderChecklist` builds
  `QuestChecklistLayout.Sky` → `GuideChecklistProjection.Apply` → class picker + class
  lens + state lens → render; `CompanionProjection.BuildSky` makes the same two calls in
  the same order. The `SearchByItem` precedent already rearranges those finished groups
  cross-class with its own records and re-wires the SAME tick setters by row id. The
  island view is that shape a second time.

### 1. Decisions (defaults chosen; David vetoes from DECISIONS.md; Helm signs here)

- **P1 — The island view is a REARRANGEMENT, never a second producer.** It takes the
  already-projected, already-scoped groups and regroups their rows; same row ids, same
  tick setters (guided rows keep routing through `GuideProgressRouter`, `LedgerItemName`
  rows keep refusing ticks and drawing the bag count). No engine, no store, no new
  producer of "what is on this checklist".
- **P2 — The island fact rides the ROW.** `QuestChecklistRow` gains `string IslandKey`
  (additive, default `""`; a string so the record stays value-equal) — the parsed island
  SET in one sortable encoding owned by `SkyIslands` (the existing `SetKey` convention
  moves there and gains its inverse, round-trip tested). It is stamped by the SAME code
  that stamps the heading, in both producers: `IslandPlacements` from the islands it
  already holds; the guide projection from `Parse(stage.Name)`, falling back to
  `Parse(objective.Where)` ONLY when the stage name parses empty AND the objective is not
  a turn-in. The fallback is what places the 22 Efreeti rows on {1.5, 4, 8}; the turn-in
  exclusion is what keeps the 48 "Isle 1" hand-in directions out of Island 1's work list.
  Heading strings are never parsed back.
- **P3 — `SkyIslands.Parse` learns ONE new shape and no second:** a number list
  immediately following the isle word — *"Isles 1.5, 4 and 8"* → `[1.5, 4, 8]`. Committed
  positive: the catalog's verbatim Efreeti string, prove-failed against the shipped parser
  (which must be shown returning `[1.5]` before the fix). Committed negatives: *"any
  isle"*, *"one of the early isles"*, "2 spawns" stay empty; every existing
  `SkyIslandsTests` row stays green untouched.
- **P4 — Grouping and ordering land in `QuestChecklistLayout`, nowhere else** (#184):
  `SkyByIsland(groups, repeatMultiIsland)` → new `SkyIslandGroup`/`SkyIslandRow` records
  beside `ChecklistItemMatch`. Numbered islands ascend (1.5 before 2); multi-island SETS
  honor the existing `SkyStepsUnderEveryIsland` — on: the row appears under every island
  it names, off: one group per set with `SeveralHeading` naming them; rows with no island
...
- **P5 — What the island view EXCLUDES, it counts out loud** (trap 50). Turn-in rows are
  excluded — the Ready band above the lens already owns "what can I hand in right now"
  cross-class and stays on screen in both modes; one owner per question. Rows of
  turned-in rewards are excluded — a turned-in reward's rows are all force-acquired by
  `MarkRewardTurnedIn`, so they would render every island as mostly-done noise — with an
  already-worded line from the layout: how many rewards are hidden and that the Class
  view keeps them.
- **P6 — The toggle is `AppSettings.SkyGroupByIsland`, default FALSE.** KEEP is the ask's
  first word: class view stays the default and nothing about it changes. Profile-level,
  beside its sibling `SkyStepsUnderEveryIsland`; drawn as a two-chip segmented control
  (`EqSegmentedStrip` — never hand-built, ChipStyle's rule) on the Sky tab only. Epic /
  General / Unlocks untouched (`EpicRowsHaveNoIslandHeading` is already the committed
  negative). The phone follows the PC's choice the same way it already follows
  `SkyStepsUnderEveryIsland` — no protocol change, no per-surface toggle.
- **P7 — The island view draws rows, not guide chrome.** No per-reward guide cards, no
...
- **D1 — Core: the fact and the regroup** (V1). P2's `IslandKey` in both producers; P3's
  parse extension; P4's `SkyByIsland` + records; P5's exclusions and their sentences.
  Guards: the parse round-trip; the prove-failed Efreeti positive; **a sweep over the
  SHIPPED GuideCatalog pinning the placement outcome — 103 single-isle / 22 on {1.5, 4, 8}
  / 97 no-island (95 wind rune + 2 Not placed) / 95 turn-ins excluded** — so a future
  catalog edit that moves one row is a red diff, never a silent regroup; and the
  distinct-count score test in island shape.
- **D2 — Desktop: toggle, render, smoke pack** (V2 — `check.ps1` + affected suites +
  targeted E2E). The `EqSegmentedStrip` toggle writing `SkyGroupByIsland`; island-mode
  render in `RenderChecklist` downstream of picker/lens/state (search still returns early
  and crosses everything, unchanged; Ready band, leftover bands and class counts stay
  above the lens in BOTH modes); **trap 72 by name: `SkyGroupByIsland` joins the refresh
  signature in the same commit its reader lands.** E2E: the `EQBUDDY_EXPAND` dump gains
  the island-mode facts (mode, island group count, first heading, and a tick round-trip
  on an island row that routes through the guide router), asserted from `EQBuddy.E2E` —
  inputs dumped from one moment, screen never asserted (trap 56). A staged Sky
  island-mode shot in `shoot.ps1` in the same change (trap 22; name checked against
  `docs/screenshots/` first, trap 21). `WhatsNew.json` entry (player-noticeable; nothing
  moves, so no "X is now Y" row is owed) + `docs/TestPlan.md` row.
- **D3 — Phone parity** (V1 + mobile-harness screenshot). `CompanionProjection.BuildSky`
  reads the same setting and emits the same `SkyByIsland` groups through the existing
  `CompanionChecklistGroup` shape (heading + rows + tickable ids; envelope and
  `CurrentProtocol` untouched). Every new sentence rides the wire, and the page-side
  must-list gains its row for any wording the page must DRAW (the DRA-84 D5 lesson: a
  sentence the page is sent but never draws passes every projection test).
  `SurfaceParityTests` extends: both surfaces' island groups are the same projection of
  the same layout call.

### 3. Sequence, smoke, and the seams

**D1 → D2 → D3 on green gates; one signature authorizes the sequence** (execution-flow).
D1+D2 are the 2:00 PM Desktop smoke surface; after D2 merges green the Executor
republishes the Desktop and pings Helm with 2.0.0+sha, per the card's own route. D3 lands
inside the same signed sequence before any release tag — mobile and desktop are both
first-class, and "the phone follows within the sequence" is what keeps it from quietly
falling behind. Executor per card: Opus, kicked by the Founder after SIGN.

Stop-and-escalate seams (HOLD, not a stalled slice): the P3 extension failing to stay
scope-locked (more prose shapes surfacing than the one named); the row-field addition
disturbing any wire or equality reader; the phone needing a protocol change after all.

**needs-david: NONE.** Both consequence-list tests fail — the ask itself set the
direction, the grouping is presentation over data we already ship, no privacy / release /
...
2. ~~**OE-2 — Open EQBuddy / shell recover** (`must-fix`) — parallel lane or next.~~
   **TAKEN 2026-09-06 and built** (`claude/oe2-open-eqbuddy-20260906`, its own worktree, no
   ...
   widget's `Open EQBuddy…` context-menu row through `ShellHost.OpenDoor`, with the shell
