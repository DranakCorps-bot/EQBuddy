...
## 2026-09-14 ~7:45 PM CT — Fable: DRA-84 FARM GEAR — the plan from the Founder's FAIL of the Farm Gear intents (Desktop 2.0.0+`3174aa5f`). Executor kicks only after Helm SIGN.

To: Helm

**Seat:** `fable-dra84-farmgear`, plan only — no product code in this seat, and no Executor kicked from it. Paperclip DRA-84. The Founder FAILED Helper Farm Gear on Desktop 2.0.0+`3174aa5f` with four acceptance items; every one is addressed below by number. Prior KEEPs honored: never-BiS three refusals, single-select intent strip, include-quests toggle, `DropMobs`-as-annotation (trap 4), HOME-006 vocabulary, curated-catalogs-never-auto-written. Nothing in any slice touches Pages / Play Console / tag / `release.ps1` / signing (acceptance 4); the ONE slice that touches the parked harvest is named for its own AUTHORIZE (§3 D3) rather than ridden on the sequence SIGN.

### 0. What exists (evidence, verified in-tree this session)

- **The sweep already runs the full catalog.** `GearUpgrades.Sweep` compares worn anchors against the ~11k-record `ItemCatalog` slot index, so acceptance 1's failure is not the candidate list — it is the WHO half: **the shipped `DropMobs` is EMPTY** (D6 decision (b): the items cache is gitignored, rebuilding means fetching ~11k pages, "the creatures arrive with the next weekly refresh") — and that refresh has never run; Helm's 2026-09-14 ~7:15 PM CT tip still reads "Harvest PARKED STAND." So on the Founder's build the only creature any row could name came from his own session loot (`Recommendations.SeenDrop`), and `HelperPresentation`'s WHO clause says so in its own comment: *"silent where the wiki named nobody, which is every row until the weekly refresh."* Session loot as the sole WHO source is exactly what the Founder failed.
- **Even where the catalog knows several creatures, a row names at most one** — `GearRow` draws `MobsIn(zone).FirstOrDefault()`.
- **There is no viability gate.** Zone rows are bucketed from `DropZones` and ranked by upgrade COUNT; `LevelUseFor(FarmGear)` is `Exempt`, resting on D6's survey (0 of 11,146 item records carry a level requirement) — logged then as *"the default most worth a veto."* **The FOUNDER FAIL is that veto arriving.** A low-band zone full of records that arithmetically dominate a weak worn slot ranks — Crushbone as pluses.
- **The new datum this plan rests on:** eqlwiki zone pages carry a `Level of Monsters` infobox band, and **all 118 zone wikitexts are COMMITTED** in `scripts/harvests/eqlwiki/cache/` (`zones-harvest.py` already reads them for adjacency; verified this session: Crushbone `5-20`, Rathe Mountains `13-45`). A band promoter fetches NOTHING.
- **Rathe Mountains' band is 13–45** — it spans most characters, so a level gate alone can NOT refuse the Rathe row the Founder called out. What was wrong with that row is that it named no who and no path. Two named exhibits, two distinct mechanisms: the band gate answers the Crushbone class of failure; the who-completeness rule answers the Rathe class.
- `ItemCatalogDropMobsTests` (containment), `HelperMustListTests` (per-goal level proof at two levels), `HelperSurfaceParityTests` (phone words), the trap-74 decompressed-contents gate and the trap-50 withheld idiom all exist and are the guards the slices below extend.

### 1. Decisions (defaults chosen; David vetoes from DECISIONS.md; Helm signs here)

- **P1 — Zone level bands become shipped data, from the cache we already have** *(acceptance 3)*. A transform in the guides-transform idiom — `zonelevels-transform.py`: committed cache in, `src/EQBuddy.Core/Data/ZoneLevelBands.json` out, fetches NOTHING, byte-reproducible, `--check` wired into `check.ps1` AND CI (unlike the items gate, this cache is committed, so the gate can run everywhere; plain JSON, no gzip container, so trap 74 cannot arise). Parsing is strict: `N-M` or a single `N` from the `Level of Monsters` row; prose, absent rows and anything else are ABSENT, never guessed, with the unparsed verbatims listed in the report and the distinct-count telltale over the parsed values (trap 73). The report also measures the JOIN: how many distinct `DropZones` spellings in the shipped catalog match a band key — item pages and zone pages do not have to agree on spelling; a miss is ABSENT, and the number is printed before anyone trusts the gate. Reader: `Core/ZoneLevels.cs`, `BandFor(zone)` → nullable band, eqlwiki-attributed.
- **P2 — The Founder's veto lands: `LevelUseFor(FarmGear)` flips `Exempt` → `Consumes`, and the gate REFUSES rather than demotes** *(acceptance 3)*. In `Recommendations.FarmGear` — UpgradeWorn / ReplaceSlot only: FarmToSell ranks the player's own loot and stays level-Exempt by D8's coin reasoning, and quest rows are not camps and are not band-gated. With a resolved level and a band for the zone, a zone row is REFUSED when the band's TOP is `OutgrownBy` (= 10, reused, not re-derived) or more under the level, or the band's BOTTOM is `GearBandReachAbove` (NEW named judgement, **5**) or more over it. Both numbers are judgements someone can veto, not measurements; no XP curve is invented. The refusal is REPORTED in the trap-50 idiom — the block says how many zones it held back and against which rule, with the band and the level in the sentence ("eqlwiki lists its creatures at 5–20 — you are 29"): two numbers and a source, never an adjective (HOME-006 sweep extended over the new words, prove-failed). **Refusal diverges from D3's demote-only `OutgrownWeight` precedent deliberately**: that rule protects the player's own measured evidence from judgement; these are Catalog rows, and their presence is what the Founder failed. It applies even where the player has personal seen-drops in the refused zone — Crushbone stays refused for a 29 who once farmed it at 12; the DECISIONS tip flags this as the default most worth a veto. Unknown resolved level → the gate stands down entirely, ranking on personal evidence unchanged plus the existing Character-door line (D3 idiom KEEP). No band → no refusal (an unanswered question gates nothing, trap 73). `HelperMustListTests`' FarmGear row flips to `Consumes` and is proven by running the engine at two levels, prove-failed first.
- **P3 — Every drop row answers WHO, or is withheld and says so** *(acceptance 2, and the Rathe exhibit)*. `GearRow` stops truncating: up to `GearMobsPerItem` (**3**, a cap that says so — trap 50) creatures per item line; the personal `SeenDrop` fact still WINS where it exists and the catalog clause stays empty by construction there — one producer per fact, trap 4 KEEP. Where unseen, the catalog names its creatures plural, Catalog-labeled as today. **A drop-sourced item that can name NO creature from either source and rides no quest row is WITHHELD from the zone rows**, counted in the existing withheld sentence — after the refresh, an empty `DropMobs` entry means the wiki page named nobody, and a rec that answers only "somewhere in this zone" is the row the Founder called Rathe. Ordering consequence: this slice lands AFTER the refreshed catalog is on main, and it OPENS with a coverage survey (what fraction of `DropZones`-bearing records carry ≥1 creature). If coverage comes back under half, the withhold default is wrong, and the slice STOPS AND ESCALATES to Helm with the number instead of shipping a hollowed room — that is the declared-boundary seam, named now. Phone: the same words ride the wire (trap 32); `HelperSurfaceParityTests` rows updated in-slice.
- **P4 — What the refresh fixes with NO new code, named so nobody re-implements it.** `MerchantCopper`/`MerchantCondition` land from the same rebuild (FarmToSell's Catalog labels gain data — readers shipped in DRA-71 D7), `ItemCatalogDropMobsTests`' containment starts guarding real data, and the Categories survey re-takes itself (`itemcatalog-build` already does). No FarmToSell engine change is in this plan.
- **P5 — The refresh run itself is ONE slice, and it is the only one that touches the parked harvest** *(acceptance 1)*. Run the standing weekly refresh exactly as designed — the committed polite client (~1 req/s, resume-safe, cached), ~11k item pages, no rate or policy change, so consequence-list item 7 is untouched. Rebuild `ItemCatalog.json.gz` through `itemcatalog-build`; the gate compares DECOMPRESSED contents (trap 74); the refresh PR carries the surveys (DropMobs coverage per P3; MerchantCopper parse rate with distinct-count; the usual reports). Curated files stay untouched (flag-only rule KEEP). **Helm: this slice does not ride the sequence SIGN — AUTHORIZE it by name or withhold it by name** (§3 D3); the sequence is cut so everything before it lands without it and P3's slice waits on it.
- **P6 — Re-smoke pack closes the loop** *(all four acceptance items)*. Staged shots with predicted numbers (trap 23): the gate visibly refusing at a staged level, a row naming catalog creatures, the phone helper. E2E facts dump the gate's INPUTS (band, level, refused count) and assert the relationship — nothing asserts the screen. `WhatsNew.json` entries for the player-noticeable changes (the release that ships them is LATER and is not this plan's to go — acceptance 4). A Founder re-smoke checklist mapping acceptance 1–3 to what to look at, filed through the channel.

### 2. Founder acceptance → answer map

| # | Founder's item | Answer | Slice |
|---|---|---|---|
| 1 | Full item/zone catalog, not session loot as sole source | P5 refresh fills `DropMobs`/`MerchantCopper`; P3 draws the catalog WHO | D3, D4 |
| 2 | Each rec: item + source mob(s) and/or quest + zone | P3 plural mobs + who-or-withheld | D4 |
| 3 | No zones with no viable upgrade path for intent/level (Rathe / Crushbone) | P1 bands + P2 gate (the Crushbone class); P3 who rule (the Rathe class — its band 13–45 spans mid levels, a level gate cannot refuse it) | D1, D2, D4 |
| 4 | No Pages/Play/tag/signing from this plan | §4 boundary — nothing in any slice touches them | — |

### 3. Delivery slices (one Executor seat each; each waits for the prior on Soft `main` except as stated)

- **D1 (`dra84-d1`)** — P1: `zonelevels-transform.py` + `ZoneLevelBands.json` + `Core/ZoneLevels.cs` + surveys + `--check` in `check.ps1`/CI. No engine change, no fetch.
- **D2 (`dra84-d2`)** — P2: the gate + the `Consumes` flip + the refused-count sentence + the HOME-006 sweep extension + shots/E2E. Depends on D1.
- **D3 (`dra84-d3`)** — P5: the weekly refresh run + the rebuilt catalog + the surveys. **Kicks only on its own named Helm AUTHORIZE** (harvest un-PARK). Code-independent of D1/D2 — may run in parallel with them once authorized.
- **D4 (`dra84-d4`)** — P3 (and P4's verification): plural WHO + who-or-withheld + the coverage survey (stop-and-escalate under half) + parity rows. Depends on D2 AND D3 on main.
- **D5 (`dra84-d5`)** — P6: re-smoke pack + WhatsNew + Founder checklist. Depends on D4.

### 4. Boundaries (what this plan does NOT authorize)

Pages, Play Console, tag, `release.ps1`, signing: untouched, no slice names them (acceptance 4); no release rides this plan — the re-smoke is a local Release build. The never-BiS three refusals, the single-select intent strip, the include-quests toggle, curated-catalogs-never-auto-written, `DropMobs`-as-annotation and the count-based zone ranking all KEEP (materiality of a gain is taste, not arithmetic — if junk-by-smallness survives P2+P3, that is a re-smoke finding, not a silent weight to invent). No Executor kicks from this seat; `dra84-d1` kicks only after this plan's SIGN.

### 5. needs-david: none

Both tests fail on every open call. Direction was given by the Founder in the FAIL itself (the `Exempt`→`Consumes` flip is his veto landing, not a new direction); the refresh is the standing approved cadence with no rate or policy change (consequence item 7 untouched); nothing touches privacy, the values line, a release go, or anything public. The veto surface is `DECISIONS.md`, tip written this session.
...
**Seat:** `fable-dra71-helper-d2`, plan only — no product code in this seat. Paperclip DRA-71 (child of DRA-70). The Founder smoked D1 (Desktop 2.0.0 + `6634a4ec`; plan #580, D1 #582, both on Soft `main`) and returned seven verbatim items; every one is addressed below by number. Prior KEEPs honored: **own Helper room** (not a Character block), **label "Helper"**, recipes `/outputfile recipes` ask ROUTINE at its slice, the **GuideAttachment flip stays its own later SIGN** (`NoShippedGuideCarriesAnAttachmentYet` stands — note the "D5" that named it in #580 is NOT a slice number in this table). Soft LEAVEs honored: no D2+ code before this SIGN, no Desktop republish, no Pages / Play Console from these seats. This plan **supersedes #580's D2–D4 rows** (D1 landed; nothing from the old D2+ was drained); the old D2-before-D3 KEEP survives in spirit — gear/motes/money still come before materials.

### 0. What exists (evidence, verified in-tree this session)

- **D1 shipped 4 of 9 engines** (`Recommendations.ShapeFor` — Answered: LevelUp, WorkOnFaction, UnlockClasses, UnlockRaces; the other five are Deferred chips with doors, pinned by `HelperMustListTests.FourGoalsAreAnsweredInThisDelivery` — the row each new slice edits). `HelperInputs` is the one parameter object (built so the phone later calls the same `Rank`); words live in `HelperPresentation` with the HOME-006 sweep and a prove-fail; every cap reports what it withheld; `ZoneHistory.MinHours` = 15 min.
- **No multi-select dropdown primitive exists.** `EqSegmentedStrip` is single-select by contract; today's multi-select is raw `EqChip` in a `WrapPanel` (the Helper's goal and faction chips — the "checkbox soup" the Founder is naming). The one dropdown multi-select in the app is the Quests **class picker** — a hand-built WPF `Popup` of CheckBoxes (`QuestsView.xaml:177-185`) behind a face capped by `ClassFilterLabel` (`MaxNamed=3`, the #184 lesson). No DesignSystem/DesignTokens popup support at all.
- **Level is ding-line only.** `LogParser.LevelRx` → `QuestLedgerStore.Level` (one int per character; `MainWindow.TrackedLevel`). No `/who` parsing; **no dump carries level** (inventory/faction/achievements verified); **no per-class levels anywhere**. The Character room deliberately shows zone-not-level today (`HomeReadout.cs:95` — a decision row, not a Helm lock). The override templates are `ClassSource.Stated` (+ `SourceLabel` one-table rule) and `MezOverrides` ("your number outranks anything EQBuddy inferred, for as long as you leave it there").
- **DPS/HPS are measured, SELF-only** (`SessionDps`/`Hps` over combat seconds; per-fight `EncounterInfo`; per-mob `AvgFightSeconds` in `MobHistory.Pool`; mob level bounds from cons). `_damageByAttacker`/`_healsByHealer` are who-hit/healed-YOU only — nothing measures another player and nothing here changes that. **There is no con-color or mob-HP model**; the only difficulty scale in the game's data is the **instance tier D0–D4** (`InstanceTier`, decoded from the zone-enter line).
- **Gear:** worn-vs-bag is known (`GearLocker.LocationRank`); `Dominates`/`CanClaimUpgrade` compare **within your bags only** (the "never BiS" scope lock); `ItemCatalog` (~11k, embedded) carries stats + `DropZones` + `Quests` + `Recipes` — but the promoter **discards the per-zone MOB names** (`DropsFrom` → zones only) and **discards `MerchantValue`/`SoldBy`**, both present in the cached harvest layer. `ZoneRoll.CopperPerHour` is already written and documented "read by the Make Money engine, which is a later slice."
- **Motes:** `Motes` is a pure per-session derivation (ladder wiki-verbatim). No zone→tier map, no mote ledger — but "which mob in which zone gave me which mote, at what rate" is derivable TODAY from `MobHistory.Pool` loot × `Motes.IsMote/PotencyOf`; nobody joins them yet (the #580 D7 fold, still unbuilt).
- **Tradeskills are the weakest goal, verified:** skill-ups parse but **do not persist** (session-scoped `_skills`); `Fashioned` names the output only; `Recipes` is names-only; **no profession list, no item→profession mapping** — the wiki's per-item `Categories` exist at harvest layer and are dropped at promotion. The eight Mastery AAs in `AaCatalog.json` are the one implicit profession list ("Jewel Craft Mastery" ≠ "Jewelcrafting" — normalization needed).
- **Unlocks: no player selection exists on any surface.** The tab lists every race and class; the Helper auto-picks top-N. `UnlockLayout.Groups` guarantees row↔criterion positional pairing, and `UnlockGuidance.Faction` is already public-by-name (DRA-70 widen KEEP) — filtering by a picked subject set is a filter over `unlocks` before `Groups(...)`, no guidance-layer change.

### 1. Decisions (defaults chosen; David vetoes from DECISIONS.md at delivery, Helm signs here)

- **P1 — ONE picker primitive, and then no hand-built popup ever again** *(smoke 1)*. `DesignSystem.EqMultiPicker` (a face button + themed `Popup` of check rows, `StaysOpen=False`) with its words and face-capping in UI.Shared: `PickerFace` generalizes `ClassFilterLabel`'s 0→"Any X" / >3→"N X" rule to any noun, unit-tested where the phone can later share it. Popup chrome from existing theme brushes + `DesignTokens` (no ad-hoc chrome). The ChipStyle rule gains a sibling: **the multi-select dropdown is `EqMultiPicker` — never hand-build another one.** The Quests class picker (`ClassPopup`) migrates onto it in the same slice — it proves the primitive on the exact precedent the Founder referenced, and retires the only hand-built popup (grep `scripts/` for staging that opens it — trap 53's neighbour).
- **P2 — Goals move INTO the dropdown; the chip soup leaves** *(smoke 1)*. One face ("Goals: Level Up · Farm Gear +2", `PickerFace`-capped), the nine rows with their existing tips inside, empty-means-all sentence KEEP. Sub-pickers (faction, unlock races, unlock classes, worn items, profession) are secondary `EqMultiPicker`s that appear **only when their goal is picked** — one face per decision, popup on demand, is the answer to "flat checkbox soup." Persistence unchanged in shape (`HelperGoals`/`HelperFactions` + new keys, same store idiom; writer and reader land in the same slice — trap 20). Polish bar for the slice: every pick in the room goes through the primitive; typography/spacing via `DesignTokens` roles only; a **`BEVEL.md` stub filed at land** asking for critique of the shots (non-blocking; follow-ups ride later slices).
- **P3 — Level is one resolved fact with two writers ordered by TIME** *(smoke 2)*. `Core/CharacterLevel.Resolve(observed(level, at), stated(level, at))` — **the fresher wins**. DRA-66's "a statement never beats a dump" carries over as "a statement never beats FRESHER game truth": a ding after your statement wins; your statement after the ding wins — which is exactly the class-swap case the Founder named (the log's level belongs to whatever classes were equipped when it printed). Store: `QuestLedgerStore.StatedLevel/StatedLevelAt` beside `Level` (which gains an observed-at stamp). Source labels through one table (`from your log's ding lines` / `set by you` — never a second phone copy). Clear affordance = the `ClearStated` idiom ("Let EQBuddy work it out"). **Per-class levels are PARKED** — no log line and no dump carries them (§4).
- **P4 — The Character room shows the level line and owns the editor** *(smoke 2)*. Identity block gains "Level 30 — set by you" + a numeric editor; the zone detail line stays (level is an ADDED identity row, not a replacement — the `HomeReadout` zone-not-level decision covered the line under the name, and the Founder's ask supersedes silence, not zone). The HOME/LIVE boundary lock is untouched: a level is identity, not a combat number. The Helper names the level it used and, when unknown, draws one line + a Character door instead of guessing.
- **P5 — Every zone-producing engine consumes the resolved level, and a must-list says so** *(smoke 2, the Founder's MUST)*. A test enumerates the engines; each either consumes level or is enumerated exempt with a reason (trap 34's pairing), proven to fail first. Unknown level: personal-evidence ranking still runs; level-gated catalog candidates draw nothing (trap 73).
- **P6 — Outgrown evidence is discounted, and the discount is a sentence** *(smoke 2×3)*. Evidence carries the level band it was earned at (cons → `MobSummary.LevelMin/Max`; session dings for the sessions' own level context). A zone whose mobs conned far below your resolved level is down-weighted and says why ("mobs you fought here conned L8–12 — you are L30"). No invented XP curve; the mechanism is a weight and a why-line.
- **P7 — Throughput vs difficulty is OUTCOME evidence first, level-delta second, and never an adjective** *(smoke 3)*. `ZoneRoll` gains observed per-zone `Dps`/`Hps` folded from the archived snapshots (same one-producer fold, same `MinHours` floor); ranking consumes fight length, deaths, downtime and DPS/HPS-vs-band as **weights with why-lines**, all `Personal`. There is no catalog difficulty data, so there is no catalog difficulty sentence. Where a session's zone-enter recorded an **instance tier**, that observation is usable evidence (see P10). The HOME-006 vocabulary sweep extends to every new sentence in the same slice, prove-failed. Nothing here measures another player — DPS and healing are the self-measured values the log already carries.
- **P8 — Farm Gear asks INTENT first; three intents, one single-select strip** *(smoke 4)*. (a) **Upgrade what I wear** → `EqMultiPicker` over your WORN rows; upgrades = catalog records that `Dominate` the picked item — the dominance sweep widens from bags to `ItemCatalog` stats with the same metric table, class-lock filter (`CharacterClasses`) and `CanClaimUpgrade` tier rule; candidates keyed by `DropZones` + `Quests` behind an **include-quests toggle**. (b) **Replace with better** → the same sweep anchored per worn slot rather than picked items; same toggle. (c) **Farm to sell** → P9. The GearLocker's "never BiS" lock is **amended knowingly**: the Locker still compares bags; the Helper may name catalog items as *farmable upgrades* — every such line `Catalog`-labeled (HOME-004), level-gated (P5), capped with the withheld count (trap 50). The data gap closes in the same slice: the promoter carries **`DropMobs`** (per-zone mob names it currently discards). Cached pages only — no new wiki fetches; the reproducibility gate compares DECOMPRESSED contents (trap 74).
- **P9 — Make Money / sell-gear un-PARKs the catalog copper value: the reopen condition is MET** *(smoke 4c)*. #580 PARKED it behind "Founder or a reporter asks for a sell-list"; smoke item 4c is that ask, from the Founder. The promoter carries a PARSED copper value from the cached `MerchantValue` (unparseable → absent, never guessed; a distinct-count survey over the parsed values runs before commit — trap 73's tell). The engine = your observed `CopperPerHour` + vendor-value-weighted drop evidence from your own kills and the catalog, `Catalog` lines labeled.
- **P10 — Motes rank places you have MEASURED; potency/hour is a new one-producer fold** *(smoke 5)*. `Motes.IsMote/PotencyOf` over `MobHistory.Pool` loot (the #580 D7 fold, built now): observed potency/hour per zone. Rank = closeness of the zone's conned band to your level (P5) × kill cadence × potency/hour, with a **preference for instance tiers 2–4 where a tier was observed** on your own sessions. **Assumption logged for veto: the Founder's "difficulty 2–4" is the game's instance-tier scale D0–D4** — the only 1–5 difficulty datum the game's data carries. No catalog mote→zone map is invented; the Executor may CHECK whether mote items carry `DropZones` in the shipped catalog — if yes, those are labeled Catalog fallback rows (match-the-wiki); if no, silence and an honest empty state.
- **P11 — Unlock picks are ONE store read by BOTH surfaces** *(smoke 7)*. New `AppSettings.UnlockPicks` (per character, subject names in the dump's own spelling — the `HelperFactions` idiom; absent = all, filter semantics). Helper: picking Unlock Classes/Races offers the picker over that section's subjects; the engine narrows to picks. Quests Unlocks tab: the SAME picker beside the section strip; the pick filters `unlocks` before `UnlockLayout.Groups` (guidance layer untouched; row↔criterion pairing preserved). One producer of the pick; both surfaces fold its content into their repaint signatures (trap 72).
- **P12 — Unlock guided detail keeps fact grammar, drawn in the six-question SHAPE where the fact answers it** *(smoke 7)*. A mover line already carries WHO (the mob) and WHERE (the zone); rows adopt the Guide idiom for layout (`who · where` on the row line, longer prose on hover — and on the phone, later, riding the row: trap 35). No kill-X/loot-Y checklist grammar is forced onto unlocks (Founder soft-leave KEEP, DRA-65); an unanswered question draws nothing (trap 73).
- **P13 — Resources are profession-first, evidence-gated, and PARKED honestly where the data is missing** *(smoke 6, the Founder's own "plan honestly on gaps")*. Curated `Core/Tradeskills.cs` — the eight professions, hand-written, wiki-matched (from the Mastery AA list; normalization noted), never auto-written. Picker → (i) your skill standing — skill values start PERSISTING per character on `SkillUpEvent` (today they die with the session; writer+reader same slice, trap 20), (ii) the Watch skill-up preset door, (iii) the profession's wiki door. **Item→profession arithmetic ships only if its evidence step passes:** the slice opens with a coverage survey of the cached wiki `Categories` (dropped at promotion today; distinct-count discipline) — good coverage → promoter carries `Categories` + a curated Category→Profession map (curated file, wiki-matched); poor coverage → picker + skill + doors ship and the arithmetic is PARKED (§4). The `/outputfile recipes` ROUTINE ask fires at this slice (Helm ruling STANDS); the recipe/ingredient model stays PARKED until that evidence exists.
- **P14 — Achievements stays Deferred; this smoke does not commission it.** The old #580 D3 Achievements engine is not among the Founder's seven. The chip keeps its door; the engine returns through the board when asked. Named here so the re-cut is a visible default Helm can veto (restore it as a slice), not a silent drop.
- **P15 — Phone parity stays by shared module, its own slice** (KEEP #580 D9). Same `Recommendations.Rank` through `HelperInputs`; projection + page + `SurfaceParityTests`; picker AFFORDANCES port as intent, not as hover (trap 35). No page work before that slice.

### 2. Founder smoke → answer map

| # | Founder's item | Answer | Slice |
|---|---|---|---|
| 1 | Multi-select dropdown, not checkbox soup; polish | P1 primitive + P2 room rebuild + Bevel stub | D2 |
| 2 | Show + override level; recs MUST factor it | P3 resolve + P4 Character line + P5 must-list + P6 discount | D3 |
| 3 | DPS/Healing vs mob difficulty | P7 outcome-first model | D4 |
| 4a/b | Upgrade existing / replace gear (± quests) | P8 intents + `DropMobs` promoter | D6 |
| 4c | Farm valuable gear to sell | P9 copper un-PARK + engine | D7 |
| 5 | Motes: highest-level zone, frequent kills, tier 2–4 | P10 fold + level join | D7 |
| 6 | Resources: profession-first; honest on gaps | P13 evidence-gated thin slice | D8 |
| 7 | Unlock races AND classes multi-select, Helper + Quests | P11 one store + P12 guided shape | D5 |
...
- **D3 (`dra71-d3`)** — **DRAINED 2026-09-13, seat `opus-dra71-d3`** (feedback in `FABLE-FEEDBACK.md`; ten defaults in `DECISIONS.md`). `CharacterLevel` + ledger stores (`LevelAt`/`StatedLevel`/`StatedLevelAt`) + Character room line/editor + `HelperInputs.Level` + the P5 must-list (`LevelUseFor`, proved by running each engine at two levels) + the P6 discount (`ZoneRoll.ConnedMin/Max/Kills` → `ZoneOutgrownFact`) + fixtures both ways at three layers (unit, real store, launched app). **NARROWED, in `DECISIONS.md` §6:** P6's evidence-band is built from `/consider` only — the "session dings for the sessions' own level context" half would be a `history.db` schema migration and is filed as its own slice rather than done quietly. **The default most worth a veto is §3:** only Level Up CONSUMES the level; faction and both unlock engines are exempt with named reasons.
- **D4 (`dra71-d4`)** — **DRAINED 2026-09-13, seat `opus-dra71-d4`** (feedback in `FABLE-FEEDBACK.md`; nine defaults in `DECISIONS.md`). `ZoneRoll.Dps/Hps/OutputPerSecond/DowntimeShare/DeathsPerHour/ObservedTier` + `ZoneHistory.Baseline` + `SessionRepository.ThroughputRows` (a snapshot probe — **no schema migration**, which D3 filed as its own slice) + four named discounts with a sentence each + `ZoneThroughputFact`/`ZoneDowntimeFact`/`ZoneTierFact` + the HOME-006 sweep widened over the throughput vocabulary with four new prove-fails + two staged shots. **Two things the plan did not foresee, both in `DECISIONS.md`:** `WhyCap` had to rise 4→6 (at four, a loaded row silently trimmed D3's own outgrown sentence), and the tier is REPORTED but weighs nothing — the P10 preference stays its own slice. **The comparison clause cannot be photographed** from the shared fixture: a session's dps is a session figure attributed whole to its primary zone, so two slices of one log always carry nearly the same output. Unit-tested at both ends and prove-failed; stated as a caveat rather than staged with invented damage.
- **D5 (`dra71-d5`)** — **DRAINED 2026-09-13, seat `opus-dra71-d5`** (feedback in `FABLE-FEEDBACK.md`; ten defaults in `DECISIONS.md`). `AppSettings.UnlockPicks` + `Core/UnlockPicks.cs` (`UnlockPickStore`) + `UI.Shared/UnlockPickReadout.cs` + the Helper sub-picker + the Quests Unlocks-tab picker beside the section strip + `HelperInputs.UnlockPicks` narrowed inside `Rank` + P12's `Who`/`RowDetail`/`RowLines`/`Hover` on `UnlockGuidanceRow` + three shots + tests/E2E + WhatsNew/DECISIONS/TestPlan/CLAUDE. **Three things the plan did not foresee, all in `DECISIONS.md`:** (a) one flat subject list needs a **per-section** narrowing rule or a race pick silently empties the Classes half — the rule the whole one-list design rests on, and the default most worth a veto; (b) P12's *"longer prose on hover"* taken literally puts the kills-to-go estimate and the piece count on a hover too, so the two QUANTITIES were kept on the row — a tab whose every sentence lived on a hover is a tab nobody can photograph (trap 22); (c) a six-mover hover is ~125 words against `SettingsProsePolicy.FitsOneHover`'s ~100, and it is knowingly NOT trimmed — that policy names Settings' instructional paragraphs as its scope and this is a scannable list of signed one-liners. **`MoverCap` is now load-bearing for a reading budget it was not chosen for**, which is worth a line in whatever slice next touches it.
- **D6 (`dra71-d6`)** — **DRAINED 2026-09-13, seat `opus-dra71-d6`** (feedback in `FABLE-FEEDBACK.md`; eleven defaults in `DECISIONS.md`). `Core/GearUpgrades.cs` (three `GearIntent`s + the sweep + `GearIntentStore` over three new settings keys) + `Core/ItemDominance.cs` (the metric table lifted out of `GearLocker`, which now delegates) + `Recommendations.FarmGear` with `GearUpgradeFact`/`GearDropSeenFact`/`RecommendationKind.Quest` + three new gap reasons + the Helper's intent strip, worn picker and include-quests pill + the `DropMobs` promoter with a decompressed-contents gate + three shots + tests/E2E + WhatsNew/DECISIONS/TestPlan/CLAUDE. **Two things the plan did not foresee, both in `DECISIONS.md`:** (a) **P8's "level-gated" could not be executed** — a survey of all 11,146 shipped item records found not one carries a level requirement, so Farm Gear is `Exempt` with that survey as its reason and `HelperMustListTests` proves the exemption behaviourally; this is the one place the delivery does not do what the signed plan says, and it is the default most worth a veto. (b) **`DropMobs` ships with NO DATA IN IT** — the item dump is gitignored and rebuilding it means fetching ~11k pages, which is the new fetch volume the plan forbids, so the promoter, the schema and both readers land and the creatures arrive with the next weekly refresh.
- **D7 (`dra71-d7`)** — mote potency/hour fold + engine; copper value promoter + Make Money/sell engine. (Splittable into two seats if the promoter work runs long; same order.)
- **D8 (`dra71-d8`)** — resources thin slice + coverage survey + recipes ROUTINE ask.
- **D9 (`dra71-d9`)** — **DRAINED 2026-09-14, seat `opus-dra71-d9`** (feedback in `FABLE-FEEDBACK.md`; ten defaults in `DECISIONS.md`). The `helper` wire surface + `CompanionProjection.Helper.cs` (which decides no word) + the page renderer + `HelperSurfaceParityTests` (29 rows, five prove-failed) + three `CommandPrompts` + `WhatsNew`/`DECISIONS`/`TestPlan`/`CLAUDE` + a staged `mobile-helper.png` with its numbers predicted. **Two things the plan did not foresee, both in `DECISIONS.md`:** (a) **P15's "same `Recommendations.Rank` through `HelperInputs`" needed the INPUT ASSEMBLY lifted too** — the room had seven reads, three folds and six store lookups inline, and calling the same `Rank` from a second hand-built bundle is #210 with an extra step; `UI.Shared/HelperSources.cs` is now the one producer and the room is one of its two callers, which is the largest un-planned change in the slice and the default most worth Helm's eye after the read-only one. (b) **The whole-room empty state turned out to be UNREACHABLE** — all nine goals answer, gap or defer — so the test asserts the unreachability instead of faking the branch; the first draft asserted the opposite and was wrong.

D4 and D5 are independent of each other; every other row depends on D3 (level) or an earlier promoter change. Helm may reorder those two freely.

### 4. PARKED (each with its reopen condition)

- **Per-class levels** — no log line, no dump carries them. Reopen: a game dump or log line that states level per class, or a reporter names a source.
- **Recipe/ingredient model** — STANDS from #580; reopen on `/outputfile recipes` evidence (ROUTINE ask at D8).
- **Item→profession arithmetic** — auto-PARKS if D8's coverage survey fails; reopen on better wiki category coverage or the recipes dump.
- **Generic camp/XP catalog** (HOME-004 fallback for empty-history players) — STANDS from #580, unchanged.

### 5. Acceptance

- **A1** Goals are one dropdown face; no flat chip strip remains; every sub-pick in the room goes through `EqMultiPicker`; the hand-built `ClassPopup` is gone (grep) and the Quests picker behaves identically (existing tests untouched).
- **A2** Character room shows the resolved level with its source label; override survives restart; ding-after-statement wins and statement-after-ding wins (both fixtures, prove-failed).
- **A3** The P5 must-list exists and was proven to fail on an engine that ignores level.
- **A4** Outgrown-zone fixture: a low-band history zone ranks below a current-band zone for a high-level character, with the P6 sentence drawn.
- **A5** Gear intent (a): a picked worn item yields zones/mobs/quests dropping dominating items; class-locked items excluded; quest rows appear only behind the toggle; every catalog line carries the estimate label (HOME-004).
- **A6** Money/sell: copper lines labeled; an unparseable `MerchantValue` is absent, never guessed; the distinct-count survey result is in the PR body.
- **A7** Motes: a zone with observed motes outranks one without; the tier-2–4 preference applies only where a tier was observed; no catalog mote sentence unless `DropZones` coverage was verified.
- **A8** Unlock picks: one store, both surfaces narrow from it, absent = all; the Quests filter runs before `Groups` and the row↔criterion pairing holds.
- **A9** HOME-006 sweep covers every new sentence (prove-failed); every cap reports what it withheld (trap 50); nothing measures another player.
- **A10** E2E asserts EXPAND-dump facts per slice (`helperLevel`, `helperIntent`, `helperUnlockPicks`, …), store-claim and screen-claim from one moment (trap 56).
- **A11** Shots staged with predicted numbers via the batch, Solarized once, illustration lock satisfied in the same change; the D2 Bevel stub is filed.
- **A12** `WhatsNew.json`: the pending 2.0.0 Helper entry grows per player-visible slice (these seats cut no release); `DeadSettingTests` clean for every new settings key; UI.Shared stays framework-free; one producer per sentence.

### 6. Verification class and traps

V2 per slice: affected suites + `scripts/check.ps1`; `build-and-test` + `e2e-windows` stay the merge bar; `dotnet build EQBuddy.slnx -c Release` before any E2E (trap 64). This plan PR itself is docs/channel-only (V0). Read before executing: traps 4, 8, 14, 16, 20–25, 30, 34, 35, 44, 50, 53, 56, 58, 61, 62, 64, 67, 72, 73, 74.

### 7. Open questions for Helm (rulings requested with the SIGN; none blocks the plan's shape)

1. **Slice order** — D4 (throughput) and D5 (unlock picks) are independent; my order weighs the Founder's item numbering; swap freely.
2. **The three promoter changes** (`DropMobs` P8, copper value P9, `Categories` P13) all read already-cached wiki pages — no new fetch volume, so my read is consequence 7 is untouched and each rides its slice under this SIGN. Rule if any should return separately.
3. **Achievements re-cut (P14)** — confirm it stays Deferred, or restore it as a slice.
4. **Bevel** — my default is build D2 then file the critique stub on its shots (non-blocking); rule if you want a Bevel faces pass BEFORE D2 implements instead.
5. **Class-picker migration in D2 scope** — my read: in (proves the primitive on the Founder's own precedent); rule it out if you want D2 minimal.

### 8. needs-david: none

Both tests fail: the direction is the Founder's own seven smoke items on his own DRA-70 line; every answer reads this player's own log, dumps, and the shipped wiki-matched catalogs (DPS/healing are SELF-measured — the values line is untouched, nothing measures another player); no release, no new surface beyond the room the PRD owns; the harvest-adjacent changes read cached pages only (no eqlwiki request policy change). Defaults — including the "difficulty 2–4 = instance tiers" reading — land in `DECISIONS.md` at delivery for veto.
...
### §1 and §2 — TAKEN AND BUILT 2026-09-11 (DRA-41)

`GuideAuthoring.Transcribed` and Delivery 3 (Epic 1.0 on the guided model) are on `main`:
the third authoring state with its refusals prove-failed both ways, the fourth progress home
`GuideProgressHome.EpicItem`, `GuideChecklistProjection.ApplyEpic` on both screens, and 14
`EpicQuest` guides carrying all 486 curated epic rows, generated byte-reproducibly by
`scripts/harvests/eqlwiki/epic-guides-build.py`. **The rules live in the code and
`docs/TestPlan.md` now** — read them there rather than here, because this inbox is not where
a shipped rule is kept true. Feedback, including two things §2 did not say and one hole the
...
**zero persisted geometry**, which is SA-2's deliberate design; free-drag reverses that on an
owner lock, so the persistence comes back by design and not by drift), `HudExpandWindow.cs` /
`HudExpandBar.cs` (the under-bar panel), `HudExpand.cs` (the UI.Shared state machine — grows a
placement field, not a new machine). Inherited from OE-1b, restated in the supersede block
below: grip anywhere on the panel box, drag-to-place primary / pop-out secondary, resize from
any edge or corner. New from the ~6:32 lock: chips (spawn, buffs-about-to-expire, the row
generally) park **anywhere**; from a parked placement the player picks expand direction
**up / down / left / right**; right-click hides a chip KIND.

Traps that bind, named before the diff: **1** (all px↔unit sums in `WidgetMetrics`, never
...
All four OE-1b locks survive, carried by the seats above and restated here verbatim as the
durable record (the branch was their only home):

1. **Grip LOCK** — drag starts anywhere on the expanded under-bar panel box. → OE-8.
2. Path priority — drag-to-place primary; pop-out secondary. → OE-8.
3. **Resize LOCK** — parked/dragged panel resizes from any corner or any edge. → OE-8.
4. **Toast LOCK** — after mini-bar pop-out ✕, remove the v1 Options re-enable message
   entirely; close → chip available; hover/click restores. → OE-7, broadened by the ~6:32
   lock 1 to every breakout/float ✕.

Why close rather than rebase: the tip is e2e-red (no waive), and the branch's diff against
main is **not additions-only** — it re-encodes ~85 existing lines of `BEVEL-FEEDBACK.md` into
...
- **Priority:** `ready` â€” **kick gated on Helm last-look of this plan** (per #359 SSC: "Do not
  kick Opus fix yetâ€¦ Phase 3 waits Fable plan + Helm last-look"). LIVE ASK is in
  `HELM-FEEDBACK.md`, same date.
- **Class:** V2 â€” release-gate exit, and the fix is an app-wide default change whose blast
...
wrap-safe minimum then lets the overflowed due time win â€” but **only when some other
registered timer is already â‰¥ 1 ms overdue** (the Î´ trigger, which is the intermittency) â€”
...
### Layer choice â€” Option 2, app-wide default. One seat.
...
- **Not Option 1 (per-control `ShowDuration` on the GearCard path).** That fixes the test,
  ...
  overdue tick â€” dozens exist and none are special. A hand-maintained list of "the tooltips
...
- **Not Option 3 (dispatcher defense).** The overflow is in WindowsBase
  (`DispatcherTimer.Restart` / `UpdateWin32Timer`) â€” not our code. A defense is either
  reflection into private dispatcher state (fragile across runtime updates) or a watchdog,
  and a `DispatcherTimer` watchdog **dies with the same shared Win32 timer it would guard** â€”
  ...
  Î´ precondition has nothing to select.
- **Option 2:** one `OverrideMetadata` giving `ShowDuration` a bounded app-wide default.
  ...
  of never â€” a "default" no design ever chose (`int.MaxValue` is "forever" by accident).

### Seat PR-3 â€” decomposition

1. **`src/EQBuddy.UI.Shared/ToolTipPolicy.cs`** â€” framework-free, the `TextRenderingPolicy`
   precedent: the bounded ShowDuration in ms (**recommend 30 000; executor may pick within
   15 000â€“60 000 and log it in `DECISIONS.md`**), with a doc comment carrying the overflow
   arithmetic and the Î´ â‰¥ 1 ms trigger so the constant can never be "tidied" back up.
   Unit tests: positive, â‰¤ 60 000, and the overflow-safety claim stated as arithmetic.
2. **WPF applier** â€” one call in `App.OnStartup` beside `WineText.ApplyIfNeeded`
...
3. **Trap 42 binds this exactly, and the plan does not trust the happy reading.** An
   ...
   `ShowDuration` is *not* an inherited property â€” `PopupControlService` reads
   `ToolTipService.GetShowDuration(owner)` directly, so a metadata default should apply
   where trap 42's inheritance case did not â€” but "should" is the word trap 42 exists for:
...
4. **Mechanism regression test** â€” lift the diagnosis's 49-line repro into
...
- The freeze is intermittent (needs Î´ â‰¥ 1 ms at tooltip open), so a single green e2e run is
...
- #357's instrument stays armed after the fix â€” a recurrence names itself with a minidump.
...
and #357 rebases onto fixed main â€” its e2e then goes green on the merits rather than on
...
plan: **PR-3 outranks OE-6/OE-5 PR-1** under Soft max â‰¤3 â€” it is the release gate.

### WhatsNew / severity â€” plan questions for Helm, not decided here

1. Two candidate player-facing facts: (a) the defect â€” *"EQBuddy could stop updating (clock,
   ...
   edge â€” tooltips now dismiss after ~30 s. Recommend one `WhatsNew.json` entry in the
   release that ships it, worded to (a); no reporter to credit â€” CI found it (worth saying
   so in the entry rather than crediting nobody silently). Whether this earns its own
   release / `v1.99.19` is release posture â€” Helm's ask to carry, David's go.
2. **Upstream:** the 49-line repro is a clean dotnet/wpf bug report. Filing publicly under
   the project's name is consequence-list #3 â€” David's door, routed by Helm if judged worth
   it. Not part of PR-3.
...
- **Shot offline:** n/a â€” no staged screenshot.
- **Column budgets:** n/a â€” no new strings on any surface.
- **What clamps it:** `MainWindow*.xaml.cs` glob is at its ratchet limit â€” PR-3 makes no
  ...
  WPF types).
- **Must-list rows on this surface:** none â€” no surface is reshaped.

## 2026-09-07 ~2:50 AM CT â€” Owner LOCK seats named: OE-5 buff-timer data path Â· OE-6 first-run Setup (Fable, on `631bd7f5`)

Executes the Helm-signed #355 owner PRODUCT LOCK (~8:55 PM CT 09-06; `HELM.md` PR #355
entry) and Bevel's Setup pre-design (PR #356 tip `52981085` â€” unmerged, read from the
branch). The `FABLE.md` stub riding unmerged PR #355 (the "To: Bevel" one-liner ask) is
ANSWERED by that pre-design and by this naming â€” whoever lands #355 should treat its stub
as spent, not pending. Two seats, not three; soft max â‰¤3 respected.
...
mechanism + fix + green on main/rebased tips â€” Helm un-parks). Building is NOT parked:
...
its "no PR-3" line â€” zero `MainWindow*` contact), so the old "timers behind density" serial
assumption is dead; they can land in either order. OE-6 shares only the Settings
neighborhood â€” whichever of OE-6 / any Settings seat lands second rebases, nothing more.

### OE-5 â€” Buff-timer data path: dump-assisted duration resolution (LOCK A)

- **Priority:** `ready` â€” Core-only, parallel worktree, no screen. Merge parked under the
  release gate like every OE tip.
- **Class:** `V2` â€” not for size but because the two obvious wrong moves are wrong for
...
- **Already shipped â€” the finding that shapes the whole seat: LOCK (A) parts (1) and (2)
  are ALREADY BUILT.** `Core/BuffTracker.cs` is land/fade live (cast window â†’ cast-on-you
  ...
  (`MainWindow.xaml.cs:1425`, per-tick at `:2662` â€” "the countdowns ARE the content")
  already draws the countdowns with the `est` label. **This seat is exactly the LOCK's two
  OPTIONAL sources, (3) and (4) â€” an accuracy upgrade to a working path, not a build.**
- **Plan:**
  - **PR-1 â€” `/outputfile spellbook` rank/known match.** Core reader (`SpellbookFile`,
    ...
    line; nothing reads it today â€” grep-verified). Consumption: narrow
    `BuffState.Candidates` to spells the character actually knows, and where the catalog
    carries a rank-resolved duration, use the KNOWN rank's base. **Spellbook â‰  timer
    source (locked):** it never starts, ends, or times a countdown â€” it only resolves
    WHICH spell a landing was. **The log outranks the dump:** a stale spellbook that lacks
    an observed landing must never erase or shrink it â€” the candidate set stays. Learned
    durations outrank anything dump-derived. Same PR adds the fourth
    `HomeReadout.Readiness()` row (the ONE place, per Bevel's source-of-truth rule â€” Home
  ...
  - **PR-2 â€” inventory focus extend.** Read the ALREADY-auto-imported inventory for
    duration-focus items and apply the extension to wiki-base estimates only â€” **never to
    ...
    overshoots every countdown). Focus-item â†’ extension% is game truth: harvest it where
...
- **Bevel pre-design: no, because** there is no presentation PR â€” the roster's presentation
...
- **What clamps it:** `MainWindow*` ratchet at zero headroom â€” untouched by design (no
...
- **Checked:** `BuffTracker.cs` full read; `OutputfileAutoImport.cs`; `RenderBuffs` call
  ...
  **Hypothesis, labelled:** whether `BuffDurations.json` carries per-rank durations is
  UNVERIFIED â€” if the harvest is rank-less, PR-1 degrades gracefully to known-spell
...
- **Decided without asking** (â†’ `DECISIONS.md` when taken): (1) the `est` label survives
  dump resolution â€” only a learned (fade-taught) duration drops it; a dump-resolved base is
...
- **Out:** any live buff-bar invent (locked out); OE-4's density work (its own seat,
  untouched); the owner's original countdown-vs-actual complaint â€” `BEVEL.md` item 1's
...
### OE-6 â€” First-run Setup / `/outputfile` guide (LOCK B) â€” **TAKEN 2026-09-06**
...
shipped unchanged** â€” one data source (`ReadinessRows`, shared with Home), no fifth
`SettingsTab`, auto-launch as a fact about the dumps. **One correction to the seat, filed
in `FABLE-FEEDBACK.md`:** the shot note has the batch profile backwards â€” it stages NO
...
priority kick. Then OE-6 and OE-5 PR-1 in parallel worktrees under soft max â‰¤3 â€” OE-6
first if serial (smallest, owner-visible, zero gate-file contact). OE-4 stays `ready` in
lane W, independent of both.

---
## 2026-09-06 ~6:38 PM CT â€” Mobile companion north star (later; not OE-1)

Owner direction: mini-bar glance model copies to phone/tablet later (no obstruct PC session); Home guidance stays mobile-accessible with game data; proactive quest-loot notifies (e.g. epic piece â†’ â€œwant to track?â€) post-glance. **Do not expand OE-1 for mobile.** No second mobile door. See `HELM-FEEDBACK.md` this date.

---
## 2026-09-06 ~6:36 PM CT â€” Owner product lock: Home stays guidance hub (not metrics)

**Owner lock (David ~6:36 PM CT):** Mini-bar = live trackers (OE-1 glance). **Home stays** the guidance hub (quests regular/sky/epic, gear lookup, next-place XP/gear guidance from inventory + DPS/capabilities) â€” not a second metrics dashboard.
...
iterates the same way as the software (observe â†’ diagnose â†’ change â†’ verify).

## When this file is in play

**V2â€“V3 only.** Cross-cutting architecture, significant refactor, ambiguous root cause,
security/privacy/migration, complex parallel decomposition.

Fable 5 writes the plan. Helm last-looks. **Claude executes it** â€” unless the plan carries a
...
**Approval by exception, not by gate** (David, 2026-08-22). The old shape â€” Fable plans,
David marks `approved`, Claude executes â€” had him reading every plan in full to say yes to
work the release gate already protected him from. The first two plans through here were
approved without a word changed.

**V0â€“V1 does not belong here.** Cosmetic, mechanical, localized, straightforward work
...
This is standing process, not a V2â€“V3 plan item. Do not stub it as a work item.
...
- **Class:** `V2` or `V3` (if you cannot say why it is not V0â€“V1, it does not go here)
...
- **Bevel pre-design: yes / no, becauseâ€¦** â€” required on any plan with a presentation PR.
...
- **Shot offline: yes / no** â€” for any staged screenshot. `shoot.ps1` is NOT offline by
  default, so a "not read yet" prediction for an unseeded wiki page is wrong before it runs.
- **Column budgets: <the fixed widths this touches>** â€” for any plan that puts a new string
...
- **Guards run eight times** â€” a new test that guards a fix is not green until it has passed
...
- **What clamps it: <the stored setting's other readers>** â€” for any formula that takes a
...
- **Must-list rows on this surface: <the `GameCommandsTests` / `ImportReportReachesASurfaceTests`
  rows>** â€” for any plan that reshapes a surface carrying one. "Defer to the window's scroller"
...
- **Decided without asking:** the implementation calls the plan made that could have gone the
  other way, one line each â€” these go to `DECISIONS.md` when the item is taken.
...
## GearCard e2e tick-freeze / app-stopped-ticking â€” RELEASE-GATE investigation (Fable, 2026-09-06 ~8:55 PM CT, on `a93e09e7`)
...
tip. Blind re-runs are exhausted (4Ã— across main + branch) and STOPPED by Helm â€” this is not
a flake to outlast, it is the thing in front of every merge.

- **Priority:** `ready` â€” with one ordering rule that is Helm's, not mine: PR-1 (instrument)
...
- **Class:** V2 â€” ambiguous root cause. Two hypothesis families fit every observation
  ...
  four rounds on exactly this ambiguity; the lesson it bought â€” *ship the instrument before
  the third theory* â€” is the plan.
- **Source:** Helm sign `HELM.md` / `HELM-FEEDBACK.md` 2026-09-06 ~8:45 PM CT; PR #352
  comment (~8:43 PM CT). CI: run `34071753526` (main `68d7d072`, #350 merge â€” **no OE-3
  code**, so not OE-3-reachable) and `34072733350` + re-run (#352 tip `4c86ae27`); empty
  re-trigger `4594877a` the same. 274/1 and 275/1 â€” always this one test.

### Evidence (checked, this seat â€” from `gh run view 34071753526 --log-failed`, verbatim)

- Failing wait: `lootSummaryLen` to reach 29; last seen **18** â€” aborted by the harness
...
- **No `error.log (tail):` section in the artifact.** `AppHarness.Artifacts()` folds it in
  ...
  `DispatcherUnhandledException` handler logs everything it swallows â€” so **no
  dispatcher-delivered exception fired**. That kills "RefreshUi throws and the app handler
  eats it" outright.
- **`WidgetDump.MaybeWrite` wraps the entire dump build â€” including `PaintOneMoment`'s
  satellite paints â€” in a try with a bare `catch { }`** (`WidgetDump.cs:115` â€¦ `:486`). An
...
- `companionPumpTicks=24` beside `tick=5`. Both are DispatcherTimers (50 ms at `Send`
  priority vs 1 s at default/`Background` â€” `MainWindow.xaml.cs:514/522`); a free dispatcher
...
- Timing: tick 4â€“6 is when `DebugHooks` opens the Gear & Loot window and the shell at
  `ApplicationIdle` â€” both are OPEN in the frozen dump. And this is the only test in the
...
### Hypotheses (labeled as such â€” the instrument decides, not this list)
...
- **F2 (slightly favored): a silent per-tick exception inside `MaybeWrite`'s try.**
  ...
  the file freezes at the tick the throw began â€” which matches the freeze landing exactly
...
- **F1: dispatcher starvation/livelock above `Background` priority.** A layout/measure storm
  (SizeToContent widget + shell + satellite on a 1024Ã—768 runner â€” trap 12's geometry
...
- **F3 (named to be dismissed by the same instrument): persistent share-violation on the
  `debug.txt` write** â€” same silent catch, same frozen-file signature. Unlikely to persist
  30 s, but the PR-1 catch-logging discriminates it for free.

### Plan

**PR-1 â€” INSTRUMENT (V1-sized, its own worktree and PR; never on #351/#352/#353):**
...
1. **The dump's catch stops being silent.** On throw: `App.LogError(ex)`, plus a
   ...
   write is inside its own try â€” a logger that can throw is the bug it reports.
2. **The harness captures the frozen process before giving up.** In
   ...
   F1's sub-causes definitively â€” it is trap 33/49's "ship the instrument before the
...
3. **No heartbeat thread.** A second writer to one file from another thread is trap 4/13's
   ...
   *Verification:* PR-1 merges under the standing gate â€” its own tip's `build-and-test` +
   ...
   apply â€” PR-1 claims no fix.

**Phase 2 â€” DIAGNOSE.** Local repro loop on a Windows seat (the single test Ã—20; it takes
...
`FABLE-FEEDBACK.md` + a Helm wake â€” **not** straight into a fix.

**Phase 3 â€” FIX (Opus, only after Fable reviews the diagnosis and Helm last-looks).**
...
loosening `StallTimeout`, no weakening or deleting the test â€” the liveness abort is the
...
on main and on the rebased OE tips â†’ Helm un-parks the OE merges. Rebasing #353/#351 is
...
- **Shot offline:** n/a â€” no staged screenshot.
- **Column budgets:** n/a â€” the new strings go to `error.log`/`debug.txt`, not a surface.
- **What clamps it:** n/a â€” no persisted setting is a plan input.
...
- **Already shipped / must not fight:** the trap-56 machinery (`PaintOneMoment`,
  `surfacesBehind`, `tick`, the liveness abort) â€” PR-1 extends it, never bypasses it; the
  trap-61 screen lock (the repro loop takes it); `Artifacts()` already folds error.log â€”
...
- **Checked:** HELM.md sign + HELM-FEEDBACK ~8:45 entry; run `34071753526` `--log-failed`
  (frozen dump quoted above); `EndToEndTests.cs:39â€“104`; `AppHarness.cs:245â€“275, 564â€“577`;
  `WidgetDump.cs:82â€“130, 486`; `App.xaml.cs:129â€“143`; `MainWindow.xaml.cs:514â€“526,
  550â€“552, 587â€“593, 2377â€“2379, 2675`. Everything not in that list is a hypothesis and is
  labeled one.
...
## Owner Evolved seats â€” OE-1â€¦OE-4 (Fable, 2026-09-06 ~11:30 PM CT, on `e2cf4a07`) â€” OWNER-OVERRIDE kick order

**This list SUPERSEDES the #347 sign's "Seat order (Fable after merge)"**
(`HELM-FEEDBACK.md` 2026-09-06 ~6:05 PM CT: Open EQBuddy â†’ xp tooltip â†’ buff density â†’
...
#347 sign stands unchanged â€” only the order Opus is kicked in moves. The four seats, in kick
order:

1. ~~**OE-1 â€” mini-bar expand** (Opus, lane W, HAS SCREEN) â€” first ship DPS â†’ HPS â†’
   ...
   lifted to `BreakoutHost.cs` to pay the ratchet. Both of its labelled hypotheses held â€”
   **no Bevel question is owed on a fourth state.** Feedback in `FABLE-FEEDBACK.md`.
2. **OE-2 â€” Open EQBuddy / shell recover** (`must-fix`) â€” parallel lane or next.
3. ~~**OE-3 â€” xp-chip tooltip** (ETA + level).~~ **TAKEN 2026-09-06 and built**
   ...
   3,839** â€” the one argument the widget owed was paid for by a `TrackedLevel` member
...
2. ~~**OE-2 â€” Open EQBuddy / shell recover** (`must-fix`) â€” parallel lane or next.~~
   **TAKEN 2026-09-06 and built** (`claude/oe2-open-eqbuddy-20260906`, its own worktree, no
   screen). The item is deleted per the take-then-delete contract; what it planned is the
   widget's `Open EQBuddyâ€¦` context-menu row through `ShellHost.OpenDoor`, with the shell
   window's ownership moved to `ShellHost.Window` to pay for the one line of glue out of a
   ratchet that had none. **`ArchitectureTests.Hotspots` is untouched, so lane W keeps its
   baseline line to itself.** Feedback in `FABLE-FEEDBACK.md`.
3. **OE-3 â€” xp-chip tooltip** (ETA + level).
4. **OE-4 â€” buff wrap-chip density** (timers stay `waiting`).

~~Do NOT kick OE-2 ahead of OE-1.~~ **Spent â€” OE-1 is built.** OE-3 and OE-4 are serial in
...
`MainWindow*` ratchet came down to **3,839** with OE-1â€™s lift and again has ZERO headroom
(4,222 of 4,222), so OE-4â€™s roster lift is not optional and OE-3â€™s tooltip has to land in
`HudBarView`/`UI.Shared` rather than in the widget.

### OE-2 â€” Open EQBuddy / shell recover: build the already-named door (`must-fix`)

- **Priority:** `ready` â€” parallel with OE-1 in its own worktree, or the next kick after.
- **Class:** `V1` â€” named here for kick order per the owner override, not because it needs a
...
- **Source:** `BEVEL.md` item 3, Helm-signed #347 item 3: the one-row premise for deferring
  the door expired at SR-5 (rail is all seven rooms); closing âœ• the `ShellWindow` currently
  strands the player until restart (`ShellHost.Show`'s only caller is `ApplyEnvHook`,
  gated on `EQBUDDY_SHELL`).
- **The trap-59 floor:** the door must exist on a DEFAULT profile â€” a context-menu row at
  ...
  coordinate with lane W (OE-1 owns `HudBarView` while it runs) â€” the context-menu row
...
### OE-4 â€” Buff roster wrap-chip density

- **Priority:** `ready` â€” after OE-3 in lane W (touches `MainWindow`'s `RenderBuffs`).
- **Class:** `V1.`
...
- **Scope:** wrap the always-on buff roster (`RenderBuffs`, `MainWindow.xaml.cs:1406-1519`)
  into a multi-column chip grid â€” `WrapPanel` per trap 25, never a horizontal `StackPanel` â€”
...
- **What clamps it:** the `MainWindow*` ratchet (one line of headroom) â€” lift the roster
...
- **Out:** ANY buff-timer code â€” the timer complaint stays `waiting` on the owner's
...
### Checked â€” what Fable actually read on this tip

`BEVEL.md` items 1â€“4 in full including the owner interview locks 1â€“10; `HELM.md` through the
...
this file's SA-1â€¦SA-4 struck notes (ratchet 4,283/4,284.5; `HudChipRowWindow` precedent;
...
(1) the peek/pin interaction fits as transient-Inline on `ThemeHost` without a new state â€”
if the executor finds it needs a fourth state, that is a Bevel question before it ships, not
after; (2) `HudChipRowWindow`'s slaving is reusable as-is for the under-bar panel â€”
verified in-PR before OE-1's hosting is final.

### Decided without asking â€” for `DECISIONS.md` when taken

- **OE-3's level readout goes on the tooltip, not the Progress header** â€” the sign allowed
  either; the tooltip pairs it with the ETA in one hover.
- **OE-2's recommended control is the context-menu row** â€” it is the only choice that is a
  trap-59 door on a default profile AND avoids a lane-W file collision while OE-1 runs.
- **OE-3/OE-4 serial behind OE-1 in lane W** â€” could have been parallel worktrees; they
  share `HudBarView`/`MainWindow` and the ratchet, so parallelism there is merge-conflict
  spend, not speed.

---

## Surface A â€” HUD Edit mode: the F2 multi-PR decomposition (Fable, 2026-09-05, on tip `d4092028`)

- **Priority:** `ready` â€” **after Helm signs this decomposition** (the last-look ask is in
  `HELM-FEEDBACK.md`; K10's own kick prompt says plan-only until F2 is signed). Once signed,
  each SA PR below follows the standing per-PR Helm last-look loop exactly as #299â€“#326 did â€”
  no further blanket authorization exists or is needed. **No `needs-david:`** â€” everything
  here is inside the #324 sign (Helm, 2026-09-05 ~1:30 PM CT, six items) and the signed spec
  Â§0/Â§3; no consequence-list door is touched. Serial in lane W (K11), one screen owner.
- **Class:** `V2` â€” the long pole by the E-3 plan's own name (I-8). Three always-on-top
  ...
  V0â€“V1 because the obvious reading of I-8 ("MiniStats seeds the HUD") is the thing the
  signed pre-design just ruled OUT â€” an executor without this plan would build the
  mini-dashboard again and call it a HUD.
- **Source:** Bevel's B3 pre-design (`BEVEL.md` 2026-09-05, PR #324, Helm-signed ~1:30 PM CT
  â€” the six signed items are restated below so no SA PR needs to re-derive them);
  `docs/BEVEL-v2-staging-critique.md` Â§0/Â§3 (the signed HUD spec); `FABLE.md` I-8; the K10
  kick. B3 stays in `BEVEL.md` as the executors' reference â€” its Â§1 per-key fate table is
  the authority; the condensed copy below is a cross-reference, not a second list.
- **Bevel pre-design: yes â€” B3 (#324).** One residual: B3 named the Edit-mode *verbs* and
...
- **Shot offline: yes** â€” every staged state is fixture-log + settings driven, no wiki
  fetch anywhere in Surface A. Predictions written before shooting (trap 23), and the
  **BATCH** runs, not `-Shot` (trap 53) â€” SA-2 deletes two windows, which is trap 53's
...
- **Column budgets:** the HUD is `SizeToContent`, so trap 12 binds every readout:
  ...
  widths â€” `PerfReadout` is the worked example. No new string enters an existing
...
- **What clamps it (all verified this pass):**
  - **The WPF ratchet is `4106` with zero headroom** â€” 4,516 lines against 4106 Ã— 1.1 =
    4,516.6 (`ArchitectureTests`). Every SA PR must be net-non-positive in the
    `MainWindow*` glob **or lift a surface out** â€” and SA-1/SA-2 are designed as lifts, not
    as edits that squeeze under the bar.
  - **`MiniStats` has readers beyond the bar.** The minimized-breakout gate
    (`MainWindow.xaml.cs:3530â€“3536`) opens a breakout only when the kind is not in
    `DisabledBreakouts` **and** its star key is in `MiniStats` â€” so stripping `dps`/`hps`
...
- **Must-list rows:** no `GameCommandsTests.SurfacesNeedingACommand` or
  ...
  copy, no import report lives on a Surface A surface â€” checked both curated lists by
...
- **Already shipped â€” what this must not fight:** nine-card widget (W1/#314); the standing
  per-item HUD-subtraction gate (rooms pre-design Â§2: room landed â†’ chip/room shipped for
  review â†’ screenshot parity, per surface); `ChipStackPlan`/`SlowChipText` as tested chip
  logic (both survive, re-consumed); `AlertWindow`'s toast + placement-drag;
  `AlertSurface`/`AlertTab` scaffolding (zero consumers â€” B4 fact; **spent at I-11, not
...
1. **`MiniStats` does not migrate.** The flat ten-key mini-dashboard retires with the card
   ...
   always-on collapsed numbers â€” and promotion removes their toggles, which is a
   player-visible change the What's-new must say in those words.
2. **One HUD chip row** â€” consolidation, not extension. `SpawnChipsWindow` and
   `MezChipsWindow` retire; Watch-fire and buff-expiring chips are **net-new UI**, not a
   port (nothing visual exists for either today â€” grep-verified by B3).
3. **Edit verbs re-scoped for one row:** Place = order in the row (not x/y â€” the signed
...
4. **B4 facts ride along as facts only** â€” full Settings IA stays I-11.
5. **Sequencing (1) collapsed numbers (2) chip-row consolidation (3) Edit mode (4) star
   retirement per-item** â€” signed as an offer to F2, not a plan lock.
6. **Out of pass:** pet-idle chip question (open, unruled); card cuts; player door;
   `v1.99.19`; Play Console; I-11.

### SA-1 â€” TAKEN 2026-09-05, built and green (Claude, lane W)

Its eight steps are struck rather than left to be re-read as pending â€” the take-then-delete
...
- `UI.Shared/HudGlance.cs` (the trio and the one swap, unit-tested with no window, every
  ...
  purely from event totals over TWO windows â€” 30 s dominance, 5 s resume â€” so the snapshot
...
- **The bar is out of `MainWindow` and in `EQBuddy/HudBarView.cs`**, a view class, with
  ...
  tree** â€” SA-2 inherits zero headroom again, exactly as this plan predicted, so its two
...
- The migration is `AppSettings.MigratePromotedHudStats`, run-twice tested through the whole
  chain AND through the real `Load` â€” see the note below, which SA-2 needs.
...
### SA-2 â€” One chip row: fold spawn + mez, retire two windows and eight settings

1. **`UI.Shared/HudChipRow.cs`** â€” the merge decision: which families are present
   (spawn + mez now, SA-3 adds two), family order (default mez first â€” combat-urgent, the
   ...
   unchanged.
2. **Hosting â€” the one place this plan amends B3's letter, flagged in the Helm ask.** The
   row renders in a companion window **slaved to the HUD's position every tick** â€” shown
   ...
   a fullscreen game â€” trap 12/#173's exact mechanism. The slaved companion keeps every
   ...
   still. **Visibility: whenever chips exist, in BOTH HUD states** â€” today's chips are
...
3. **The chicklet lifts as a control** (`EQBuddy/HudChip.cs` or a `DesignSystem` addition):
   countdown, drain-gauge fill, DUE flip, click-to-dismiss â€” from the two windows' render
...
4. **Retired:** `SpawnChipsWindow.xaml(.cs)` (215 lines), `MezChipsWindow.xaml(.cs)` (174),
   their `MainWindow` construction sites (`:2451`, `:2477` â€” net-negative in the glob,
   ratchet down same commit), and **eight settings**: `SpawnChips{Left,Top,Bottom,GrowUp}`,
   `MezChips{Left,Top,Bottom,GrowUp}` â€” removed outright, not kept for round-trip (the
   windows they position no longer exist; JSON load ignores unknown keys; logged in
   `DECISIONS.md`). **`ChipScale` STAYS** â€” it is the player's chip-size choice and the row
   obeys it.
5. **`ChipStackAnchor` + `ChipAnchor.cs` + `ChipStackAnchorTests` delete with the
   windows** â€” their entire subject is per-window geometry persistence (the trap 2/#122/
   ...
   the surface it guarded â€” trap 57's precedent for how to record that).
6. **Fold obligation (traps 20/26/46):** enumerate every control and interaction the two
   windows carried and say where each went â€” the known ones: click-to-dismiss (â†’ the chip
   control, unchanged), Options-open placement mode and its draggable placeholder
   (`ChipStackPlan.Placeholder` â€” dies with placement; the Options affordance that
   summoned it must not dangle), grow-up/grow-down (dies with free placement â€” the row has
   one growth direction), mez-duration editor and spawn toggles in Options (rules, not
   geometry â€” untouched). The PR carries this list in its description.
7. **E2E + shots:** `hudChips=` fact family (per-family count, due flags, row visible)
   ...
   `docs/screenshots/` and the docs first â€” trap 21) with spawn timers + a mez chip
   seeded and the picture predicted in writing before it runs.

### SA-3 â€” TAKEN 2026-09-05, built and green (Claude, lane W)

Struck rather than left to be re-read as pending â€” the take-then-delete contract. What
...
- **`HudChipFamily` is four**: `Mez, Spawn, WatchFire, Buff`, and `DefaultOrder` is already
  ...
  moved to `order:`).
- **`UI.Shared/WatchFireLedger.cs`** â€” one chicklet per RULE (not per firing), 30 s linger,
  dismiss, and **the `AlertBanner` gate lives there** as `Record(TrackedRule, â€¦) â†’ bool`, so
  it is assertable. `HudChipRow.WatchChips` / `BuffChips` are the two builders.
- **The buff threshold is `AppSettings.BuffWarnSeconds`, not a pinned constant** â€” still no
  ...
  and `_slowTracker` take â€” which is better for this, since the row already asks trackers.
...
- The `buffs` `MiniStats` key is untouched, as scoped â€” it retires with the card under SA-R.
...
### SA-4 â€” TAKEN 2026-09-05, built and green (Claude, lane W)

Struck rather than left to be re-read as pending â€” the take-then-delete contract. All five
...
- **A family the stored order OMITS is APPENDED, not dropped** â€” the one place the plan's
...
- **Edit mode is a placeholder per FAMILY, not affordances on live chips**, and the row is on
  ...
  Entry is the `Edit HUDâ€¦` context-menu row; `EQBUDDY_HUDEDIT=1` is the hook, `hud-edit` the
  shot.
- **The mute toggle is a TICK, not a bell** â€” this app's bell is its SOUND vocabulary and mute
...
- **`hudChipOrder=` is read off the ROW** (families in the order drawn), `hudMuted=` off the
  ...
  against the 4,284.5 ceiling â€” no baseline bump**, so SA-R inherits the same one line of
  headroom the last three passes did.
- **`PinWatchChips` is still the one switch SR-4 left for this lane, and SA-4 did NOT
  reconcile it** â€” checked rather than assumed. It is the master switch for watch chips in the
  ...
  `MutedChipFamilies` would be exactly the repurposing B3 Â§3 forbids. It retires with the mini
  bar under SA-R, not here.

### SA-R â€” per-key star retirement: a template, NOT an authorization
...
table (condensed from B3 Â§1, which remains the authority):

| Key | Retires with card | Destination that must exist first |
|---|---|---|
| `kills` | Kills & Drops | Live (session kills half â€” shipped) |
| `loot` | Gear | Gear room + **the ordinary-loot toast** (net-new; belongs to THIS retirement, not to SA-2/3 â€” named so nobody bolts it in early) |
| `motes`, `money` | Motes / Progress | Progress Wealth (shipped) â€” note `motes` has TWO writers today (B3 Â§1) |
...
Template per retirement PR: destination shipped + reviewed â†’ screenshot parity â†’ star
writer removed + cell removed + migration strips the key + `HiddenSections` handling for
the card â†’ `DeadSettingTests` / must-list re-check per move (traps 20/26/34/43) â†’
What's-new "X is now Y" naming origin AND destination â†’ Helm last-look. `xp`/`dps`/`hps`
never appear in this series â€” SA-1 already retired their stars by promotion.

### Sequencing and lane

SA-1 â†’ SA-2 â†’ SA-3 â†’ SA-4, **serial, lane W** (they share `MainWindow`, the row, and the
...
2. **The SA-2/SA-3 split** of signed step (2): consolidation is a parity refactor with
   zero new behavior; net-new chips are new behavior with new staging â€” different review
...
`docs/BEVEL-v2-staging-critique.md` Â§0 + Â§3 in full; `MiniBarPresentation.cs` in full
(`Order`, `Names`, `Icons`, `Cells`, the buffs-never-draws comment);
`MainWindow.xaml.cs` `MiniStatOrder:116`, `StarButtons:3438â€“3446` (six keys â€” dps/hps/pet/
procs/buffs/motes), `SyncStarsFromSettings:3451`, chip construction `:2451`/`:2477`,
`AlertTile:2800`, the mini-bar loop `:3683`, the breakout gate `:3530â€“3548`;
`AppSettings.cs` chip fields (six position `:437â€“449` + two GrowUp `:217â€“218` + `ChipScale
...
`SessionStats.cs` `CurrentDps` memo block `:1665â€“1680`, `CharacterName:138`, snapshot
fields `:2083â€“2093`; `ArchitectureTests` ratchet row (4106; 4,516 vs 4,516.6);
`WidgetDump.cs` (zero mini/chip facts â€” grep); `scripts/shoot.ps1` `mini-bar`/`mini-tour`
...
for SA-3 â€” implied by the Buffs breakout, not read; verified in-PR before SA-3 is scoped
final. (2) The chicklet render code lifts cleanly as a control â€” the two windows are thin
(215/174 lines), believed not proven. (3) The 19 `MiniStats`-naming files beyond the two
live gates are comments/blockers, not readers â€” SA-1's in-PR enumeration proves it.

### Decided without asking â€” to `DECISIONS.md` when taken

- **The mini bar lifts out of `MainWindow` in SA-1** rather than being edited in place â€”
...
- **Chip row hosted as a position-slaved companion** rather than in the widget's visual
  tree (trap 12) â€” flagged to Helm rather than silently decided, but the default is mine.
...
- **Eight chip-geometry fields removed outright**, not kept-for-round-trip â€” the surfaces
  they position are gone and 2.0.0 is local-only; `SpawnSound`'s keep-for-round-trip
  precedent predates the one-lane world.
- **`ChipStackAnchor`/`ChipAnchor` deleted with tests + trap 2 tombstone** â€” could have
  kept them as dead guards, which trap 45's "an exemption nobody can see" logic argues
  against.
- **SA-3 ships pinned default thresholds, no Options rows** â€” tuning waits for a player
  ask; the alternative (a settings row per family) is I-11's surface arriving early.
- **Nudge-reorder, not drag-reorder, for Place** â€” cheap and testable; drag can arrive
  later without changing the setting's shape.
- **Loot toast deferred to the `loot` retirement PR** â€” it is that gate's destination, not
  Surface A core.

---

## I-11 Settings room â€” the F3 multi-PR decomposition (Fable, 2026-09-05, on tip `6336933c`)

- **Priority:** `ready` â€” **after Helm signs this decomposition** (the LIVE ASK is in
  `HELM-FEEDBACK.md`; #331's soft â€” *"Fable may decompose I-11 after this sign"* â€” authorizes
  the plan, and each SR PR below then follows the standing per-PR Helm last-look loop exactly
  as #299â€“#335 did). **No `needs-david:`** â€” everything here executes the six-item #331 sign
  (four tabs Look / Alerts / HUD / Behavior; "HUD" not "Cards & windows"; gear checklist
  import â†’ Gear; the out-list); no consequence-list door is touched.
- **Class:** `V2` â€” cross-cutting two-host architecture. Not V0â€“V1 because the obvious
  reading â€” "build a Settings page in the shell and copy the controls over" â€” is exactly the
...
- **Source:** Bevel's I-11 Settings IA (`BEVEL.md` 2026-09-05, PR #331, Helm-signed ~5:10 PM
  ...
  describing the character"*); PR #335 (the Options-gap V0â€“V1, SHIPPED â€” consumed as-is
  below, never re-opened). The IA stays in `BEVEL.md` as the executors' reference, the same
  contract B3 runs under for Surface A.
- **Bevel pre-design: yes â€” I-11 (#331).** Two residuals Bevel named ride inside this plan
  rather than as separate work: the vocab sweep before shell land (structural â€” see the ban
  clamp below) and the **Theme color-picker â‰  banned "theme"** caveat, written into SR-1 so
  no mechanical rewrite misfires on the wrong token (trap 34's shape).
- **Shot offline: yes** â€” every staged Settings state is settings + fixture-log driven;
  nothing on these tabs fetches the wiki. Predictions written before shooting (trap 23), and
  the BATCH runs, not `-Shot` (trap 53 â€” no window is deleted or renamed in this series, but
  SR-5 adds shots and the batch is the only thing that proves the rest).
- **Column budgets:** none in the fixed-width sense â€” `OptionsWindow` and the shell are both
  resizable. The one strip rule: **the Alerts sub-tab strip carries `AlertSurface` badges, so
  it is not fixed-width â€” `WrapPanel`, never a horizontal `StackPanel` (trap 25, the exact
...
- **What clamps it (verified this pass):**
  - **`OptionsWindow.xaml.cs` is a ratcheted hotspot: baseline 1547, file at 1,578** â€” about
    123 lines of ceiling left (1547 Ã— 1.1 = 1,701.7, `ArchitectureTests:222`). Every SR lift
  ...
  - **Trap 13 as a constructor contract:** both hosts' `OptionsViewModel` instances wrap the
    ...
    `_main.PersistSettings` â€” what `OptionsWindow.xaml.cs:25` already does). A room that
    called `AppSettings.Load` for itself would hold a second snapshot and clobber wholesale.
  - **The Â§4 terminology ban binds every string a shared block carries.** The ban exempts v1
    `OptionsWindow`, but one block serving two hosts has ONE string set, and it must pass in
    shell scope â€” so lifting a block IS that block's vocab sweep. "Theme" (the color picker)
    survives; the "widget"/"card"/"breakout"/"mini pill" copy Bevel Â§5 verified does not.
  - **`PinWatchChips` has two live readers in lane W's own files** (`HudBarView.cs:298`, the
    minimized-breakout gate at `MainWindow.xaml.cs:3534`) plus `WatchPinMigration` â€” which is
    why it is EXCLUDED from the lift (SR-4 item 4) rather than carried into the new tab.
- **Must-list rows:** `GameCommandsTests.SurfacesNeedingACommand`'s Gear row is untouched â€”
  the â§‰ lives on `GearCardView`, which SR-2 adds to and never un-pins.
  `ImportReportReachesASurfaceTests` carries `LastInventoryImport â†’ Gear` (untouched).
  **Hypothesis, labelled:** the EQ Legends Tools checklist import has NO import-report row
  today â€” SR-2 checks both curated lists by subject and, if confirmed, ADDS the row naming
...
- **Already shipped â€” what this must not fight:** `ShellPage.Settings` + `BelowTheGap` + the
  rail machinery (Settings absent from `Landed` â€” it joins in SR-5's own commit, the
  registry's stated rule); `AlertSurface`/`AlertTab` (built, zero consumers â€” **spent at
...
`OptionsWindow.xaml(.cs)` into host-neutral view classes â€” new files, the
`OptionsCardsView`/`QuestsView` precedent â€” each of which builds its own controls, takes
...
host constructs its own instances (trap 45 â€” no control ever crosses hosts), both wrap one
`AppSettings` (trap 13), and the room's dump facts are the blocks' facts re-keyed through
`ShellDumpFacts.Prefixed("shellSettings", â€¦)` (trap 58). The rejected alternative â€” building
the room fresh beside a live `OptionsWindow` â€” is two copies of ~40 control wirings drifting
until retirement day: #210's mechanism with a bigger surface.

### SR-1 â€” TAKEN 2026-09-05, built and green (Claude, lane D)

Its four steps are struck rather than left to be re-read as pending â€” the take-then-delete
contract, SA-1's and SR-4's precedent.

**What landed** (`EQBuddy/SettingsLookView.cs` 377 lines, `EQBuddy/SettingsBehaviorView.cs`
443 lines; `OptionsWindow.xaml.cs` 689 â†’ 393 and `OptionsWindow.xaml` 372 â†’ 226, baseline
lowered in the same commit):

- **Look** â€” the colour theme picker and its Custom rows (hex boxes + 16 swatches), the four
  size/opacity sliders and their live values, the alignment grid + spacing, the cursor ring.
  **Behavior** â€” EQBuddy Mobile pairing + the sounds switch, the three hide-when rules and the
...
- **One thing the window still does for a block, and it is named in both files:** ROUTING a key
  ...
  nothing inside the panel has focus when the gesture lands â€” the press reaches the WINDOW.
...
- **The vocab sweep ran and "Theme" survived, as an EXEMPTION ROW rather than as silence.**
  `ShellTerminologyTests.Exempt` was empty; it now has exactly one row, naming Bevel I-11 Â§5 and
  why the ban's `\bthemes?\b` is the wrong sense here. Reworded: "Widget size" â†’ "EQBuddy size",
  "Whole-widget opacity" â†’ "Overall opacity", the three "Hide the widgetâ€¦" boxes, the alt-tab
...
- **Two UI.Shared word modules joined `ShellStringSources` too**, which the plan did not
  ...
  no tier could see them â€” the block's own scan reads `MobileAlertSounds.Label`, an identifier,
  and learns nothing. `AltTabPolicy.TaskbarWarning` was reworded at its source ("the widget" â†’
...
- **Owed and NOT done here:** the `options-window`-family re-shoot and the E2E suite. Both need
  the screen and lane W's SA-4 held it â€” the E2E run refused the lock by name (pid 45284), which
  is trap 61's interlock working rather than a failure. 37 source-level assertions stand in.

### SR-2 â€” TAKEN 2026-09-05, built and green (Claude, lane D)

Its four steps are struck rather than left to be re-read as pending â€” the take-then-delete
contract, SR-1's and SR-4's precedent. **SR-3 is next in the serial and carries no part of
this.**

**What landed** (`GearCardView.cs` 348 â†’ 515, `GearChecklistPresentation.cs` +87,
`GearChecklistImporter.cs` +26; `OptionsWindow.xaml.cs` 393 â†’ 337 and `OptionsWindow.xaml`
226 â†’ 219, baseline lowered in the same commit):

- The three buttons, their handlers and the status line are above the list on
  `GearCardView`, beside the â§‰ copy of `/outputfile inventory` and the import report â€” one
...
- **The mutations went further than the plan asked, into Core** â€”
...
- **The live defect the move would have created, found by following the plan's own trap-26
  duty:** `GearChecklistPresentation.EmptyRoute` â€” the empty state, the surface's whole
  route â€” named "Options â†’ Cards & windows â†’ Import gear listâ€¦" and would have gone on
...
- **Item 3's hypothesis, answered: NO row is owed, to either list, and both reasons are
  ...
  `AutoImportOutcome`s â€” dumps EQBuddy reads BY ITSELF and could act on silently â€” and this
...
- Four executor calls, logged in `DECISIONS.md`: Clear asks first (the move changed the
  risk, not our minds â€” a mis-click costs the ticks, which only EQBuddy holds); Clear is
  hidden rather than disabled (trap 17 â€” no disabled visual in this style); the heading and
...
### SR-3 â€” TAKEN 2026-09-05, built and green (Claude, lane D)
...
`OptionsCardsView.cs` it replaces; `OptionsWindow.xaml` 219 â†’ 173 and `OptionsWindow.xaml.cs`
337 â†’ 326, baseline lowered in the same commit):

- The panel list and its "no longer on the widget" companion, the mini-dashboard tick boxes,
  the floating-window tick boxes, and all three strays â€” host-neutral, taking
...
- **The three strays are the rows that mattered, for a reason specific to this tab.** Their
  ...
  control does not carry when it is rebuilt in code â€” trap 20's polarity arriving through a
...
- **The rename made three OTHER surfaces stale, which item 2 has no line for.** The âœ• tooltip
  on every floating window, the alert banner when one is dismissed and the `CoreLog.Error`
  beside it all said *"Options â†’ Breakout windows"* â€” #219's mechanism inside one sentence, the
  ...
  lines in `MainWindow.xaml.cs` â€” zero net, mandatory: that file is one line under a
...
- **Prove-fails run.** 47 of 49 `SettingsHudBlockTests` assertions fail against the pre-lift
  ...
  new `ShellTerminologyTests` rows fail there â€” `MiniBarPresentation` was already clean, so its
  row is vacuous today and guards those labels going forward.
- **Owed and NOT done here:** the `options-window` family and `options-cards` re-shoots â€” both
...
### SR-4 â€” TAKEN 2026-09-05, built and green (Claude, lane D)

Its six steps are struck rather than left to be re-read as pending â€” the take-then-delete
...
1,569 â†’ 689, baseline lowered in the same commit):
...
- **`PinWatchChips` did not lift** â€” asserted in both directions, so SA-4 finds one switch.
...
- **Two executor calls, logged in `DECISIONS.md`:** the slow alert sits with the shared
  ...
  control** â€” item 5 fixes v1 at five tabs, so a strip here would have had no consumer until
...
### SR-5 â€” TAKEN 2026-09-06, built and green (Claude, lane S)
...
`ShellPages` gains Settings in `Landed` and a `Rooms` row; `OptionsWindow.xaml.cs` 326 â†’ 348;
`tests/EQBuddy.Tests/SettingsRoomTests.cs` and 5 new E2E rows):

- **It is a COMPOSITION, not a move, a lift, a build or a merge** â€” the first room of its kind,
  and the reason the four blocks came out first. Four tabs against the window's five (Bevel Â§2:
...
- **Item 5 needed a half the plan did not name, and it is the half that made the comparison
  ...
  `shellSettings*` â€” so `optionsHudPanels` beside `shellSettingsHudPanels` is a comparison
  rather than a collision (trap 58). It cost one word in lane W's file
  (`private` â†’ `internal` on `_optionsWindow`, ZERO net lines, which matters at one line of
...
- **Item 3's check found one thing that is NOT nothing, and it is now a retirement blocker.**
  `Release()` is empty (the blocks hold no timer, token or watcher â€” the armed hotkey recorder
  is a string field and the â¤´ "âœ“" revert is a self-stopping one-shot). But `MainWindow.OnOptions`
  pairs `AlertTile.EnterPlacement()` with the WINDOW's `Closed`, and a room has no `Closed` â€”
...
- **Item 4's fallthrough created an obligation the plan did not state, so it is asserted:**
  ...
  `settings:cards` answers HUD as an alias (the v1 tab tag â€” nothing renders it, and an old
...
- **Item 6 landed the word "HUD" structurally**, which SR-3 recorded as still owed.
  ...
  is still not swept, still on #335's sign, and the room renders it verbatim â€” it is Bevel's to
...
- **Item 7's staging was smaller than the plan expected, and the reason is SR-2**: the gear
  ...
  **The four new rows were shot under the screen lock and each was reviewed against its
  written prediction; the FULL batch did NOT complete** â€” the session running it died partway
...
**SR-1 â†’ SR-2 â†’ SR-3 â†’ SR-4 â†’ SR-5, serial in one seat.** SR-1â€¦SR-4 all touch
`OptionsWindow.xaml(.cs)` â€” D-lane files per the E-3 lane table; SR-5 is S-lane files
...
`MainWindow*` or anything lane W owns â€” the SR and SA series are file-disjoint by
construction** (the one shared subject, `PinWatchChips`, is handled by exclusion), so
SR-1â€¦SR-3 run beside SA-2/SA-3 with no coordination beyond the mailbox. Screen: SR PRs that
...
runs the full `shoot.ps1` batch first** â€” the #332 duty, still owed.

### Out â€” matching the #331 sign, restated so no SR PR drifts
...
- **No re-opening #335** â€” the Retired list is consumed as-is (SR-3 re-hosts, never
  redesigns); future HUD cuts keep adding their rows per the standing rule.
- **No re-litigating the Â§4 ban or the "HUD" name** (signed items 1â€“2).
- **No mobile Settings surface** â€” the phone's âš™ Screens picker is its own subject, outside
  the IA's scope.

### Checked â€” what Fable actually read on this tip

`BEVEL.md` I-11 Â§0â€“Â§6 in full; `HELM.md` #331/#332/#333/#334/#335 signs and Live Holds
...
`:21â€“120` and the gear import block `:1497â€“1558`; `GearRoom.cs:1â€“80` (the two-host view
pattern and the star-refusal comment); `ShellWindow.RoomFor` (`:303â€“323` â€” rooms take
...
`WatchPinMigration.cs`); `ImportReportReachesASurfaceTests:43` (`LastInventoryImport â†’
...
import-report row â€” SR-2 checks by subject; (2) `BuildRulesEditor`/`BuildBuffSetPanel` lift
cleanly as blocks â€” believed from the `OptionsCardsView` precedent, the 1,578-line file was
not read end-to-end; (3) callers of `OptionsWindow.SelectTab`/`OptionsTab` beyond the
window's own persistence â€” enumerated in SR-1 before the lift, not this pass.

### Decided without asking â€” to `DECISIONS.md` when taken

- **Blocks over fresh-build** â€” could have built the room fresh and let retirement delete
  the duplication; #210 is why not.
- **v1 `OptionsWindow` keeps its five-tab arrangement re-hosting the blocks** â€” could have
  collapsed v1 to four tabs too; that reshapes a shipped surface for no player benefit
  before the room is even reachable.
- **`PinWatchChips` excluded from the lift** â€” could have moved it; the SA-4 collision and
  the spec's presence-split are why not.
- **SR-2 before SR-3**, so the HUD block never carries the gear import.
- **`SettingsSurface` in Core** rather than a room-private tab list â€” map, don't translate.
- **`settings:crowd`-style fallthrough** rather than a three-level address grammar.
- **Shell tab named "HUD" while v1 keeps its label** â€” the ban's own scope line, applied.

---

## Evolved opt-in telemetry â€” heartbeats, own backend, public metrics (Fable, 2026-09-05, on tip `3ec285aa`)

- **Priority:** `someday` â€” **captured for later sequencing, on David's explicit instruction.**
  ...
  which is its own consequence-list conversation. **No `needs-david:` line** â€” David is the
...
- **Class:** `V2` â€” privacy/security is this file's charter in as many words, and this is
  the **first feature in the product's history that sends anything off the player's
  machine**. Not V0â€“V1 because the payload allow-list, the backend's retention/delete
...
- **Source:** David, 2026-09-05, verbatim requirements: (1) default-OFF opt-in anonymous
  telemetry; (2) heartbeats only â€” install id / app version / OS, **no** logs, character,
  chat, paths, or EQ account data; (3) own backend with retention + delete path; (4) public
  README/repo-main marketing metrics â€” unique users, concurrent + peak concurrent, version
  mix; GitHub download totals may show separately and **must be labeled downloadsâ‰ uniques**;
  (5) Evolved requirement for later sequencing â€” not folded into live shell/nav lanes.
- **Already shipped â€” what this must not fight:**
  - **`SECURITY.md` Â§"Zero telemetry" is a self-enforcing promise**: *"If you ever catch
    EQBuddy sending something this page doesn't list, that is a vulnerability â€” report it as
    one."* That sentence is the best thing on the page and it stays â€” the telemetry ship
  ...
  - **`README.md:43` "Zero telemetry, always contribution"** â€” a public promise. It changes
    to the charter's wording ("no telemetry by default") **only in the release that ships
    the client**, never before. `README.md:535`'s "no telemetry â€” nothing leaves your
  ...
  - **`LEGACY-V1.md:60` "Nothing expires, phones home, or switches itself off" â€” stays true
  ...
  - **The charter already permits exactly this shape**: `EQBuddy-v2-Project-Guide-Requirements.md`
    Â§2.2 says "no telemetry **by default**" (not "never"), and **NFR-PRIV-002** â€” *"Telemetry,
    if ever added, must be opt-in and separately documented"* â€” reads like it was written for
  ...
  - **E-1's local-only mechanism** means the client can ship fully live during the local
    phase â€” the only installs that exist are David's, so "dark launch" needs no extra flag.

### The requirement, pinned (TEL-001 â€¦ TEL-006)

- **TEL-001 â€” Consent.** Telemetry is OFF on every install, forever, until the player turns
  ...
  OFF stops sends **and destroys the local install id** â€” re-enabling mints a fresh one, so
  opting out is also an identity reset. Trap 47 binds: **one policy module decides
  consent + cadence, and every send path goes through it** â€” never two code paths deciding
  an off-machine question â€” and the periodic timer's epoch is "time of opt-in", never
  `DateTime.MinValue`.
- **TEL-002 â€” Payload.** Exactly three fields: `installId` (random GUID minted at opt-in â€”
...
- **TEL-003 â€” Cadence, and what "concurrent" means.** One heartbeat shortly after launch
  ...
  **peak concurrent** = max over 10-minute buckets; **unique users** = distinct ids
  ...
  numbers honest â€” publish them beside the numbers.
- **TEL-004 â€” Backend, retention, delete.** Our own backend, no third-party analytics.
  Raw heartbeats retained **90 days** then deleted by scheduled job; aggregates (counts
  only, no ids) kept indefinitely. **IPs are never persisted or logged** â€” transport sees
  ...
  raw row for that id. (Aggregates need no scrub â€” they contain no ids.)
- **TEL-005 â€” Public metrics.** README/repo-main shows: unique users, concurrent now, peak
  concurrent, version mix â€” fed live from the backend's public `metrics.json` (badges/
  fetch), not by a bot editing the README on a clock. GitHub download totals may appear
  **separately, labeled** â€” the shipped wording must say downloads count fetches, not
  people, and point at the uniques number as the honest one.
- **TEL-006 â€” Scope freeze.** Heartbeat only. No crash reporting, no feature-usage events,
  no session stats, no error strings, ever, under this plan â€” any of those is a NEW plan
...
### Decomposition â€” four PRs plus one Bevel pass, lane `TEL`

**TEL is its own lane** â€” disjoint by construction from W/S/D/T (no `MainWindow`, no
...
1. **TEL-PR1 â€” requirement page** (docs only, this repo): `docs/v2/telemetry.md` carrying
   TEL-001â€¦006 verbatim plus the payload JSON, cadence, retention table, and delete
   contract â€” NFR-PRIV-002's "separately documented", written before the code exists so the
   code is written to the page and not the reverse. Includes DRAFT (clearly marked
   unshipped) replacement copy for README Â§log-only and SECURITY.md Â§Zero telemetry, so
   TEL-PR4 is a copy-move, not a composition under release pressure.
2. **TEL-PR2 â€” backend** (own PUBLIC repo, recommended `DranakCorps-bot/eqbuddy-telemetry`):
   smallest thing that works â€” Cloudflare Worker + D1: `POST /heartbeat` (validate, upsert),
   `POST /delete`, cron rollup (10-min buckets â†’ daily aggregates â†’ public `metrics.json`),
   ...
   client's is â€” a player can read exactly what the endpoint stores. Free tier; the moment
   this needs a paid plan it is a **David door (consequence item 4, money)** â€” named here so
   it is asked then, not smuggled.
3. **TEL-PR3 â€” client** (this repo): `UI.Shared/TelemetryHeartbeat.cs` (pure policy:
   consent, id lifecycle, cadence, payload â€” unit-tested, no network); one thin sender the
   ...
   the local-only phase â€” the endpoint's only traffic is David's installs.
4. **TEL-PR4 â€” the public face** (this repo, **at channel-open only**): flip README and
   ...
   consequence item 3 (public, under the project's name) â€” Helm signs the copy, and it
   rides the channel-open release David already gates, which is why it carries no
   `needs-david:` of its own.

- **Bevel pre-design: yes, before TEL-PR3** â€” the toggle, the consent copy, the status
...
- **Shot offline: yes** â€” TEL-PR3 adds an Options shot of the toggle in both states; the
  fixture has no network, so the staged state is OFF + "never sent", predicted before
  shooting (trap 23).
- **Column budgets:** none â€” new Options rows, no fixed-width surface. The status line
...
- **What clamps it:** `AppSettings.Save` writes the whole file (trap 13) â€” the id mint on
...
### Sequencing vs E-3 â€” the answer to "when"

**Not now, and not in the shell/nav lanes.** Nothing in TEL blocks or is blocked by
I-1â€¦I-17; the E-3 kick sequence (K1â€“K11) does not change. Recommended slot: TEL-PR1 and
TEL-PR2 any time an idle seat exists after Surface A (I-8) is underway â€” they touch no
product surface and no screen. TEL-PR3 after the Bevel pass, in whatever settings surface
exists at kick time (Settings room I-11 if landed, else Options â†’ Behavior â€” the executor
...
dashboard of one machine â€” the sequencing is the honesty.

### Checked â€” what Fable actually read on this tip

`SECURITY.md` Â§Zero telemetry in full (the self-enforcing sentence is verbatim);
`README.md:38â€“49` and `:525â€“545` (both "no telemetry" claims and their scopes);
`LEGACY-V1.md:60`; charter Â§2.2 non-negotiables and Â§18.3 NFR-PRIV-001â€¦004;
...
comfortably covers heartbeats at any plausible player count â€” believed from published
limits, not load-tested; TEL-PR2 verifies before relying on it. (2) shields.io endpoint
badges cache acceptably for "concurrent now" â€” if their cache is too coarse, the README
...
- **Cloudflare Worker + D1** as the recommended stack. Could have been Azure (where signing
  ...
  if reality disagrees â€” the contract is TEL-004, not the vendor.
...
## E-3 completion â€” the parallel build-out plan (Fable, 2026-09-05, on tip `d55de151`)

- **Priority:** `ready` â€” with per-lane gates NAMED inline. The plan itself is the
  ...
  last-look. **No `needs-david:` line** â€” every kick below is inside signed rulings or the
...
- **Class:** `V2` â€” complex parallel decomposition, which is this file's charter in as many
...
- **Source:** David via the kick prompt â€” *"parallel not serial"* â€” plus `HELM.md`'s
  eleven E-3-era signs (#299â€“#306), Bevel's HUD-subtraction pre-design (`BEVEL.md`
...
- **Bevel pre-design: per lane, and three lanes ARE Bevel work** â€” see the kick sequence.
...
- **Shot offline:** every subtraction and room PR runs the `shoot.ps1` BATCH, not one
  `-Shot` (trap 53), with predicted counts written first (trap 23) â€” e.g. cut W1's
  prediction is a nine-card widget. Screen work is MUTEXED â€” see concurrency.
...
- **What clamps it:** the WPF ratchet is `4123` with **zero headroom** (4,535 lines against
  4123 Ã— 1.1 = 4,535.3 â€” verified in `ArchitectureTests` this pass). Every MainWindow-touching
...
- **I-1 Â· Quests card subtraction (HUD subtraction cut 1).** Bevel pre-design filed with
  Helm ask ~7:55 AM CT. Diff per Bevel Â§2: `quests` leaves `OverlaySections.Catalog` and
...
- **I-2 Â· Room-level empty-state wrapper.** Helm-signed 2026-09-04 ~11:15 PM CT; **built by
  zero of the six rooms** (Bevel Â§5 restates it). Stops being deferrable the moment I-1
  merges â€” a new profile then sees the un-wrapped Quests room as the entire Quests
  experience.

**Own-ask items already ruled "own ask" by signed entries:**

- **I-3 Â· World's Drops tab** (Kills & Drops split, destination 1 of 4; Live Â§1 + Bevel Â§6).
- **I-4 Â· History this-session half** (Live Â§1).
- **I-5 Â· World `misc` card subtraction (cut 2)** â€” **DONE 2026-09-05.** Bevel ran both
  checks (BEVEL.md, tip `d4092028`): parity holds by construction (one `TravelsView`, two
  owners) and the `Worldâ€¦` row is structurally permanent â€” and check two established that
  ...
  Ratchet 4106 â†’ 4100.
- **I-6 Â· E-2d Wine/CrossOver knobs** â€” ruled at #277 (*"drop three Options knobs; keep
  `TextRenderingPolicy` + `WineText`"*), parked since; the three settings verified still in
  `AppSettings.cs:89â€“110`. Keep `WineFonts.cs` + `TextProbeWindow.cs` per the signed plan.
- **I-7 Â· E-2e v1 feature disposition table** (`docs/v2/v1-feature-disposition.md`) â€”
...
- **I-8 Â· HUD Edit mode / Surface A** â€” the signed plan's "PR after the host": DPS/HPS
  glance chips, editable deadline chips, `MiniStats` star rehoming (loot/xp/money/motes,
  deaths if I-5 lands), `HiddenSections` â†’ HUD-content-only migration, `MiniStats` seeds
  ...
  Motes, Progress â€” Bevel's inventory table; the eighth is Kills & Drops) and, behind them, every window retirement.
  Multi-PR by construction; needs its own Bevel pre-design (the chips are pixels) and its
  own Fable decomposition against that pre-design. â†’ **Both exist now:** B3 (#324,
  Helm-signed 2026-09-05 ~1:30 PM CT) and the F2 decomposition above (SA-1â€¦SA-4 + SA-R).

**Blocked / parked, with the gate named so nobody re-derives it:**

- **I-9 Â· v1 window retirements** (`ProgressWindow`, `QuestsWindow`, `GearLootWindow`,
  `WorldWindow`) and the two `SurfaceOwnershipTests` exemptions â€” blocked on I-8 star
  rehoming + card cuts; Bevel Â§4: retiring a window is separate from and later than
  subtracting its card.
- **I-10 Â· Kills & Drops split remainder** â€” Search lookup (waits I-7), Gear "what dropped
  for you" (own ask, later). The card itself stays until 3 of 4 destinations exist.
- **I-11 Â· Settings room** â€” `ShellPage.Settings` exists, not in `Landed`, held by the
  stated decision ("a room whose whole job is not being a launcher"). Needs a Bevel
  Settings-IA pre-design; design can run now, build waits. â†’ **Both halves exist now:**
  the Bevel IA (PR #331, Helm-signed 2026-09-05 ~5:10 PM CT) and the F3 decomposition
  above (SR-1â€¦SR-5); build starts when Helm signs F3.
- **I-12 Â· Player door â€” OFF.** Not designed, not implemented here. Bevel Â§6's coupling
  stands: HUD subtraction and the door meet at release time. When it opens it is a
  consequence-list conversation (roadmap/release posture) â€” a real ask, later, not a lane.
- **I-13 Â· Progress "see Live" pointer â€” CLOSED** (signed omit, #306). Named so no lane
  reintroduces it.

**Infrastructure / process:**

- **I-14 Â· `shoot.ps1` intermittent full-batch look â€” TAKEN, DIAGNOSED AND GUARDED** (T1 lane,
  ...
  `shoot.ps1` stands down the first seat's in-flight fixture app mid-settle â€” the failing row is
  ...
  states it in one line while the ask beside it asked whether the harness needed a look. Â§4's
  *"enforced by kick order, not by tooling"* is now enforced by tooling: a batch-held lock file
  ...
  it â€” **the readiness wait was satisfied by the widget rather than by the shot's own window**, so
  ...
  (a) the batch was NOT run â€” SA-1 held the screen, and running it is the collision itself, so
  ...
  verify); (c) `docs/screenshots/quest-tracker.png` is stale for the second round (880Ã—658 vs a
  committed 880Ã—868), and it sits with the 17 committed illustrations #306 measured as no longer
  matching what `main` renders â€” a re-shoot lane, not a harness lane.
- **I-15 Â· Empty-profile harness/shot** for true never-seen room empties â€” #303 ask 2 named
...
- **I-16 Â· Shell terminology scanner â€” TAKEN AND BUILT** (PR #323, 2026-09-05, T3 lane).
  `ShellTerminologyTests`: eight facts plus nineteen curated file rows, prove-failed against
  five seeded violations on the real tree. It answers Bevel's Â§6 ask 6 as well as the item â€”
  **the shell's player-facing strings are NOT reachable from one place**, so it is three
  ...
  literals a curated file list scans, and the ban list itself pinned to Â§4's table in both
...
- **I-17 Â· Fable H4 last-look of the executed E-3 diffs (PRs #299â€“#306).** H4 exists for
  E-0/E-1 only; five shell PRs have shipped since without one. One read of the merged range
  is the same move that caught the âœ¦ regression the whole suite missed.

### 2. Dependency graph â€” what serializes, what does not

```
Helm sign (HUD ask + this plan)
  â”œâ”€ W1 Quests card cut â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â–º W2 World misc cut (needs I-5 checks + Bevel/Helm)
  â”‚        â”‚                              â”‚
  â”‚        â””â”€â”€â”€â”€â”€â”€â–º (ratchet freed) â”€â”€â”€â”€â”€â”€â”´â”€â–º W3+ Surface A series (needs Bevel HUD
  â”‚                                            pre-design B3 + Fable decomposition F2)
  â”‚                                            â”€â”€â–º remaining card cuts â”€â”€â–º I-9 retirements
  â”œâ”€ S1 empty-state wrapper â”€â–º S2 World Drops tab (needs Bevel B1 + Helm)
  â”‚                         â””â–º S3 History this-session (needs Bevel B2 + Helm)
  â”œâ”€ D1 E-2e disposition table â”€â–º Search index (later)
  â”œâ”€ D2 E-2d Wine knobs (cite #277)
  â”œâ”€ T1 shoot.ps1 batch look Â· T2 empty-profile harness Â· T3 terminology scanner
  â”œâ”€ B1/B2 Bevel pre-designs (Drops, History) Â· B3 HUD Edit pre-design Â· B4 Settings IA
  â””â”€ F1 Fable H4 of #299â€“#306 Â· F2 Surface A decomposition (after B3)
```
...
parallel by construction â€” disjoint files, or no files at all.
...
(`QuestsView` is the precedent) rather than editing `MainWindow` â€” that keeps the widget
lane's exclusive lock real, and it is the direction the ratchet pushes anyway.

| Lane | Owns exclusively | Must not touch |
|---|---|---|
| **W (widget)** â€” W1 Quests cut, W2 misc cut, W3+ Surface A | `MainWindow.xaml`, `MainWindow*.xaml.cs`, `OverlaySections.cs`, `BreakoutWindow.*`, `EndToEndTests.cs` widget facts | `ShellWindow`, `*Room.cs` |
| **S (shell)** â€” S1 wrapper, then forks S2/S3 | `ShellWindow.xaml.cs`, `ShellHost.cs`, `RoomEmptyState.cs`, `*Room.cs`, `ShellPages.cs`, `ShellHostTests.cs`; S2 adds a new `DropsView` file, S3 owns `HistoryWindow` | `MainWindow*` (lift, don't edit) |
| **D (docs/settings)** â€” D1, D2 | `docs/v2/`, `OptionsWindow`/`OptionsViewModel`, `AppSettings` Wine rows, `DeadSettingTests` | any window file |
| **T (tooling)** â€” T1â€“T3 | `scripts/shoot.ps1` batch logic, harness profile seeding, new terminology test file | product `src/` |
...
trivially; channel files follow the existing hygiene â€” **channel commits go to `main`
...
lanes building .NET in their own worktrees (own `obj/`/`bin/` â€” trap 18 stays per-tree), one
...
a last-look, and five simultaneous asks serialize inside Helm's mailbox anyway â€” a queue in
...
`HELM-FEEDBACK.md` last-look ask exactly as #299â€“#306 did.

- **K0 (Helm, now):** last-look **two** asks in one sitting â€” Bevel's HUD-subtraction ask
  (~7:55 AM CT, pending) and this plan's. One Helm session, two signs.
- **K1 (Opus, worktree `lane-w`, HAS SCREEN):** W1 â€” Quests card subtraction per Bevel Â§2.
  Ratchet down same commit; batch shot with predicted nine-card widget; PR + ask.
- **K2 (Opus, worktree `lane-s`):** S1 â€” empty-state wrapper across all six rooms +
  `ShellWindow` centering. No screen â€” CI e2e; screenshots ride the next S-lane PR or
...
- **K3 (Sonnet, Bevel):** B1 World-Drops pre-design, B2 History-this-session pre-design,
  and the two I-5 checks â€” one Bevel session, three entries, one combined Helm ask.
- **K4 (Opus, worktree `lane-d`):** D1 â€” E-2e disposition table (docs only, cites the
  signed E-2e section verbatim as its spec). Then D2 â€” E-2d in the same lane, asking Helm
  with the #277 citation (a formality ask, not a re-ruling).
- **K5 (Fable, this seat or next):** F1 â€” H4 last-look of #299â€“#306 merged range;
  findings are V1 items for the next loop per the standing contract.
- **K6 (Opus, `lane-s` after S1 merges + B1 signed):** S2 â€” World Drops tab (new
  `DropsView` lift; `WorldRoom`; no `MainWindow` edits).
- **K7 (Opus, fourth seat or `lane-d` after D2, after B2 signed):** S3 â€” History
  this-session, respecting the signed Home/Live boundary (no combat fields by construction).
- ~~**K8 (Opus, `lane-w` after W1 merges + I-5 checks + Helm):** W2 â€” World misc card cut.~~ **DONE 2026-09-05** â€” own PR, HELM-FEEDBACK last-look filed. Widget is eight cards.
- **K9 (Sonnet, Bevel, after W1+S1 merge):** B3 â€” HUD Edit mode / Surface A pre-design
  (chips, Edit mode, star destinations, `MiniStats` seeding). B4 Settings IA may ride along.
- **K10 (Fable, after B3 signed):** F2 â€” Surface A multi-PR decomposition into this file.
...
- **T-kicks (any idle seat, screen-mutexed):** ~~T1 batch look~~ **DONE 2026-09-05** â€” own PR,
  diagnosed as the cross-seat collision Â§4 predicted and guarded in the harness; the batch
...
page â€” the only door-shaped things here (I-12, channel-open) are parked, not asked.
...
`EQBuddy-Evolved.md` in full; `ShellPages.cs` (`ShellPage` enum â€” no Search member,
Settings not `Landed`; `Landed` = six), `GearRoom.cs`/`WorldRoom.cs`/`LiveRoom.cs` headers
(star-writer blockers verbatim); `AppSettings.cs:89â€“110` (three Wine knobs live â€” E-2d not
...
(#210, #153 â€” both pre-Evolved, neither in scope); absence of any terminology scanner in
`tests/`.

**Hypotheses, labelled:** (1) S2's Drops content can be lifted without touching
`MainWindow` â€” the Drops view's current home was not read this pass; if it is
MainWindow-owned, S2 coordinates one factory-lift commit with lane W rather than editing in
place. (2) Two concurrent .NET builds on David2026 are comfortable â€” inferred from build
...
- **S1 (empty-state wrapper) is its own PR before the content forks**, not folded into W1 â€”
...
- **The terminology scanner needs no pre-design** â€” it enforces an already-signed ban list.
- **Channel commits land on `main` directly**, generalized from the #306 round's practice.

---

## EQBuddy Evolved â€” LOCAL-ONLY development start (owner GO 2026-09-04 ~2:52 PM CT)

- **Priority:** `ready` â€” E-0 and E-1 may start immediately. E-2 is gated on E-0 closing the
  #275 LEGACY checklist (Helm, 2026-09-04 11:50 AM CT: Avalonia removal waits on the Phase 0
  gate). E-3 is gated on E-2 plus a Bevel nav pre-design. **No `needs-david:` line** â€” see
...
- **Class:** `V3`. It cuts a supported platform out from under live users, it changes what
  ...
  folder and every v1 install takes it within six hours. Not V0â€“V1 because the obvious
  version of "develop v2 locally" â€” just build it â€” is the thing that leaks.
- **Source:** owner GO via Helm 2026-09-04 ~2:52 PM CT (1-shot as much as possible; Evolved
  ...
  `docs/v2/EQBuddy-v2-Project-Guide-Requirements.md` rev 1.1 Â§Â§3 / 16 / 19 / 20 / 23 / 26;
...
- **Already shipped â€” what this must not fight:**
  ...
  - `UpdateChecker.GitHubLegacyReleasePage` resolves from the **running build's own tag**, by
    design (`UpdateChecker.cs:39â€“57`). Do not "fix" it into a literal.
  - Bevel's IA is the signed destination and it is **not** a work order â€”
  ...
  - **Bevel's staging pass #2 (`103d8fec`, 2026-09-04 ~3:05 PM CT) landed while this plan was
    being written and is folded in below.** Two consequences: **Â§6 door 2 of the signed critique
    is RETIRED, not waiting** â€” LEGACY-002's notice copy shipped in `v1.99.18` and is kept
    verbatim, so **do not schedule a voice pass on it** (that would be the #228 class: rewriting
    shipped player-facing text for no player benefit). Doors 1 and 3 stand. And its Â§1/Â§2 are
...
- **Bevel pre-design: E-0/E-1/E-2 no; E-3 YES and it gates E-3's first pixel.** E-0 to E-2 are
  ...
  string they touch â€” the legacy notice â€” was designed in P0-2 and is unchanged). E-3 is a new
...
- **Column budgets:** none for E-0/E-1/E-2 â€” no new string enters an existing fixed-width
  ...
  rule** â€” `SizeToContent` means a timer-driven string is a geometry change on an always-on-top
...
- **What clamps it:** the new `<Version>` is read by four separate consumers with four
  ...
  â€” which **fails outright on `2.0.0-alpha.1`**, because the regex requires `</Version>`
...
- **Guards run eight times.** Every new guard here (`evolved-channel-guard.ps1`, the
  ...
  runs before its PR is called green, **and is proven to fail on the pre-change tree** â€” a
...
close the hole â€” it closed the other one.**

`release.ps1:96â€“97` copies `EQBuddySetup.exe`, its `.sha256` and the portable zip into
...
local-first by design â€” `Check(folder)` reads that exe's `FileVersionInfo`, `IsNewer` compares
...
in the story. The script's own comment at line 133â€“136 states this plainly â€” *"The OneDrive
copy above is a SEPARATE channel â€¦ so a prerelease still reaches the family's widgets"* â€” and
...
- `release.ps1:153` ends with `/SILENT`, and `installer/EQBuddy.iss` uses one `AppId` and
  ...
  install in place** and inherits its profile â€” `settings.json`, `history.db`, archives. The
  installer's `EQBuddy.previous.exe` rollback (#158) gives back the binary and not the profile.
- `install-local.ps1` is already the correct local loop and says so â€” *"Touches neither
...
repo already knows this shape â€” `release.ps1` has no `-SkipSign` on purpose, because a
...
### E-0 â€” Close the Phase 0 gate â€” **TAKEN AND DONE, 2026-09-04 (Claude)**

Left as a stub rather than deleted, because E-2's gate is defined in terms of it. The four PRs
and their evidence:

- **E-0a â€” PR #288, merged, Helm-signed.** Re-pinned all six asset links + both tag links in
  `LEGACY-V1.md` and the README from `v1.99.17` to `v1.99.18` and deleted the "planned / not
  published yet" prose. Added `legacy-notice-guard.ps1` **check 4** â€” every `v1.<n>.<n>` those two
...
- **E-0b â€” PR #289, merged, Helm-signed.** `legacy-v1` cut at `v1.99.18` = `dbcfb3a1`; branch
  ...
  ATTEMPTING the deletion â€” the tag rule probed on a scratch tag, never on `v1.99.18` itself.
- **E-0c â€” PR #290.** `LegacyNoticeRenderTests`: the notice is PAINTED, from a hand-built
...
- **E-0d â€” PR #291.** README + `docs/FeatureGuide.md` truth pass, `options-cards.png` re-shot,
  and the illustration lock written into `CLAUDE.md` as a standing rule. Two stale menu items the
  plan had not listed turned up by reading the menu XAML (`Quest trackerâ€¦`, `Spawn timersâ€¦`).

**#275 is ticked end to end** with evidence inline, and the deferred halves moved into a new
"Release-time rows â€” due at channel-open" section. Helm's confirmation of that checklist is the
E-2 gate; E-2 has NOT started.

---

### E-1 â€” Make local-only a mechanism â€” **TAKEN AND DONE, 2026-09-04 (Claude)**
...
file. Three commits, in the plan's load-bearing order â€” **the refusal landed before the bump.**

- **Commit 1 â€” `scripts/evolved-channel-guard.ps1`.** Sibling of `legacy-notice-guard.ps1`,
  ...
  a pull request). Proven to fail on the pre-change tree â€” 11 problems naming lines 14, 96, 97,
  98, 142, 153, 154. Eight consecutive green runs after.
- **Commit 2 â€” `release.ps1` refuses 2.x** unless `-EvolvedLocal`, before the publish. That
...
- **Commit 3 â€” `<Version>` â†’ `2.0.0`** (numeric, per decision 3), the LEGACY-007 `WhatsNew`
  2.0.0 Legacy Linux/macOS section linking `releases/tag/v1.99.18`, and `install-local.ps1
  -Evolved` â€” build, sign, run **portable** from `dist\publish` on `%AppData%\EQBuddy Evolved`.
...
**both widgets ran side by side** â€” portable `2.0.0.0` and installed `1.99.18.0`, each on its own
...
invocation first â€” so it could never fire. Four lines moved; both reachable now.
...
About line as `2.0.0 (Evolved local Â· <short sha>)`; the numeric version is a machine contract,
and the machines are `UpdateChecker`, Inno Setup and three guards. **Re-decide the public number
at channel-open.** *Executed as written â€” the four consumers were re-read before the bump and
all four still match that regex.*

### E-2 â€” Phase 1: subtract the platform (gated on E-0 complete)
...
> **SETTLED â€” `release-assets.yml` runs from the TAG's own tree, not from `main`.** The
...
> **SETTLED â€” LEGACY-001's asset half.** All three non-Windows artifacts plus both Windows
> artifacts are attached to `v1.99.18`. The `fail-fast: false` matrix worry did not bite.

**E-2a â€” Test disposition, and the prerequisite that reorders it.** `docs/TestPlan.md` Â§5
records the WPF layer has no unit tests, so `tests/EQBuddy.Avalonia.Tests` â€” **21 test files
plus `GlobalUsings.cs` and `TestAppBuilder.cs`** â€” is the repo's only rendering coverage that
...
5. **`AppThemeTests.EveryCatalogThemeAppliesCleanly` is a catalog guard wearing an Avalonia
   ...
   Decide it explicitly â€” a WPF twin is cheap and E-3 is about to add a shell full of new
   surfaces to every palette.
6. **NEW SINCE THIS PLAN WAS SIGNED (E-0c, PR #290): `LegacyNoticeRenderTests.cs` joined the
   lane â€” the spine is 24 files, not 23** (re-verified 2026-09-04 ~4:25 PM CT, Fable review).
   It is the painted proof of LEGACY-002 â€” the notice REACHES a control, trap 42's claim â€”
   and it lives on the project this phase deletes. Give it an explicit row. Recommended
   disposition: **accepted loss, with the reason written down** â€” the surface it proves ships
...
**E-2b â€” the shared-suite scanners.** **20 files in `tests/EQBuddy.Tests` name
`EQBuddy.Avalonia`, not the 18 the prior plan listed** â€” `LegacyPlatformUpdatePolicyTests` and
...
- `SurfaceOwnershipTests`' two curated exemptions (Gear & Loot, Kills & Drops) each name the PR
  ...
  so ownership does not matter"** â€” trap 45 is about a method that returns a long-lived UI object
...
**E-2c â€” pipeline and deletion, in this order and not mixed.** `ci.yml`: drop
...
`build-and-test`**, a few lines above the render-test step being deleted â€” it survives,
verbatim, and so does `check.ps1`'s `evolved` stage when the `avalonia` stage and `-Quick` go.
`EQBuddy.slnx`: drop both Avalonia rows. `check.ps1`: drop the `avalonia` stage â€” and `-Quick`
...
written into `docs/Architecture.md` â€” that doc edit belongs **in** the deletion commit, not in a
follow-up.

â†’ **Trap 53 lives here.** `scripts/` matches windows by **title**, and three stale titles left
the whole `shoot.ps1` batch unreachable past shot 37 for six days across four releases. Before
any window is deleted, renamed or folded, grep `scripts/` for its **title** â€” the class name is
...
**E-2d â€” Wine/CrossOver: already ruled, do not re-open.** Helm's #277 sign-off settled it â€”
...
call (traps 40â€“42, several rounds each). `WineOverlay.cs`, `scripts/crossover/` and
`MacOverlayLevel` go with the platform. **`WineFonts.cs` and `TextProbeWindow.cs` are not named
by that ruling** â€” `TextProbeWindow` is the `--textprobe` diagnostic that finally separated three
...
**E-2e â€” the v1 feature disposition table, with Bevel's IA as the destination authority.**
Output to `docs/v2/v1-feature-disposition.md` (charter Â§21.1), one row per feature:
name Â· today's door(s) Â· v2 room Â· class (Keep / Merge / Replace / Advanced / Remove) Â· why Â·
...
- The destination column is **not** invented here. `docs/BEVEL-v2-staging-critique.md` Â§2 is a
  ...
  notice voice pass, is retired â€” the shipped copy stays.) Do not re-litigate them and do not
  drag #250, #251 or the 320-cap into this table.
- **`Options â†’ Cards & windows` is a routing exercise, not a cleanup.** Bevel pass #2 Â§4 read the
  tab and found five jobs on it: overlay cards (â†’ nothing; the shell nav replaces them), the gear
  checklist import (â†’ **Gear**; an import workflow is a domain action), the 12 mini-dashboard
  checkboxes (â†’ **HUD, edited on the HUD**), the eight breakout toggles (â†’ **Live** boards and
  **HUD** chips), and two genuine settings stranded among them. Four of the five are deletions
  with a destination. **The rows get routed, not carried** â€” and the tab currently prints
...
**E-2 gate (charter):** no v2 requirement is blocked on non-Windows desktop parity â€” provable
mechanically from the removed CI jobs plus a disposition table with no row whose blocker is a
non-Windows desktop.

---

### E-3 â€” Phase 2 PR 1: the shell host (gated on E-2 + Bevel nav pre-design)
...
are already shell pages that ship as their own windows â€” Quests, Progress, World, Gear & Loot.
v2's shell is **two genuinely new pages (Home, Search) and a navigation host over four surfaces
that already exist as hosted views.** Charter Â§16.4's "MainWindow should lose responsibility,
...
still carry `SurfaceOwnershipTests` exemptions â€” they come later, and moving them is what
removes those rows.

**Four seams; three already exist.**

1. **A surface factory per host** â€” every page builds its own instances; no host interface
   returns a `Control` it did not just create (trap 45).
2. **Framework-free presentation** â€” `UI.Shared` is **95 files** and already holds
   ...
   `TextRenderingPolicy`, `LegacyPlatformUpdatePolicy`. **Charter Â§16.3's `EQBuddy.Presentation`
   IS this project.** Do not rename: Â§16.3 says the boundary matters more than the namespace, and
...
3. **The mobile projection is the parity proof.** `CompanionProjection*` reads the same shared
   ...
   again** â€” and #210's direction surprised people, because the *phone* was ahead.
4. **Navigation â€” the one new seam.** Smallest thing that works, per Â§16.5: a `ShellPage` enum, a
   ...
   spelling on day one â€” which is what WORLD-006 asks for.

**The consumer-grade bar, made concrete.** These are acceptance criteria for E-3, not aspirations,
and they come from Bevel Â§4 and Â§8 plus the charter's Definition of Done:
...
- **No unexplained empties.** Every first-run and no-data surface uses the inventory-dump voice:
  ...
  â§‰ copy from `UI.Shared/GameCommands` â€” and **`GameCommandsTests.SurfacesNeedingACommand` gets a
...
- **No implementation vocabulary on screen** â€” Bevel's ban list: card, card key, breakout, theme,
...
- **Settings is settings.** The âš™ does not list windows that have a nav item (Bevel gate 4). This
  is the single loudest piece of v1 debt â€” #219's reporter opened *Cards & windows* looking for
  Motes and found a deletion.
- **Every illustration of our own UI is a capture with a recipe, or it does not ship** â€”
...
- **Evolved's first run is designed, not ported.** Do not carry the 8-page tour or its assets
  forward. Charter UX-011 describes the replacement â€” contextual, self-clearing readiness that
  *"never make[s] a permanent onboarding checklist another navigation destination"* â€” and Bevel's
  locked Phase 2 Home (identity Â· readiness Â· recent session Â· deep links) is that surface. A tour
  ...
  â†’ **THE ONE CARVE-OUT, AND IT IS A DOOR: tour page 1 is not a tour page.** It is consent to
  **empty the player's log files**, asked before they know what EQBuddy is, and trap 47 is what
  happened the last time two code paths disagreed about it â€” StrIIker-TV ticked the box and lost
...
shape, and a fold may only name keys that are **no longer cards** â€” checked against
...
**Two migration rules from Bevel pass #2 Â§5, carried verbatim because they are presentation calls
...
1. **A v1 player's hidden card must never become a hidden ROOM.** *"I hid Combat"* meant "keep
   ...
   from people's products on upgrade â€” #219/#233 industrialised. **`HiddenSections` translates to
   HUD content and to nothing else.** Charter DATA-002 explicitly permits that.
2. **`MiniStats` is the one v1 setting that IS a HUD statement** â€” the best evidence we will ever
...
â†’ **One grep before Phase 2 wires either** (Bevel's hypothesis, unverified by it and by me): the
phone's `âš™ Screens` picker is a *second* per-device store of "which surfaces do I show". If the
shell's room list and the phone's picks are not built from one definition, trap 38's shape â€” a
sticky payload whose memo records the wrong thing â€” has an obvious second home.

**HUD (Surface A), for the PR after the host.** The HUD is `MainWindow` minus what the shell
takes: log tail wiring, live snapshot, mini/expanded state, chip windows, tray and context menu â€”
...
move**, or the freed room refills. Chip placement (LIVE-008) has **three actors, not two** â€”
...
**change `AppSettings.DisabledBreakouts` defaults, do not delete** â€” David uses the damage
breakout.

**E-3 gate (Bevel Â§8, and it is a product gate not a ratchet):** a player can find every retained
...
Charter Phases 3â€“6 (CharacterProfile, golden corpus, Gear +0â†’+10 math, Home recommendations,
Search depth). #250, #251, #240 / the 320-cap, #264. #261 / #262 â€” do not open. Any public
...
artifact rot. Product-direction debate â€” the charter wins. Renaming `UI.Shared`. Any framing of
...
### Doors considered and refused â€” why there is no `needs-david:` line
...
- **Removing Linux and macOS support** â€” already the owner-approved charter (Â§3.1, Â§16.2) and
  Helm's 2026-09-04 11:50 AM CT posture ruling. Settled, not open.
- **Re-pinning the public legacy links** â€” executing Helm's #284 ruling verbatim ("re-pin on
  publish"). A factual correction to an approved page, not an announcement.
- **The Evolved version number and the local install loop** â€” implementation; Â§26 lists the
  legacy branch name and the bridge version as agent decisions, and this is the same class.
- **Refusing to publish 2.x from `release.ps1`** â€” it *protects* the release go rather than
  exercising it. Opening the channel is the door, and this plan explicitly does not open it.
- **A separate Evolved profile directory** â€” protective of David's own data and squarely inside
  DATA-003's intent. It changes nothing about what the app does with a player's files.
- **Wine/CrossOver boundary** â€” Helm ruled it on 2026-09-04 ~12:05 PM CT. Cite, do not re-ask.

**Two things that WOULD be doors, both deliberately kept out of this plan:**

- **Opening the Evolved channel** â€” the first public 2.x release, its number, its announcement.
...
- **The auto-empty consent** (tour page 1). Bevel named it and did not open it; neither does this
  ...
  `needs-david:` line â€” consequence-list item 8. Naming a door and refusing to walk through it is
...
absence of `legacy-v1`, PRs #276â€“#287, and issue #275's body and checklist; `CLAUDE.md`;
`HELM.md` Holds + Retired + the sixteen newest sign-offs; `FABLE.md` item shape and the Phase 0â€“2
...
`EQBuddy.slnx`; the `release-assets.yml` run history via `gh run list`; README lines 64â€“86; file
...
**Hypotheses, labelled â€” not verified:**
...
2. **The `evolved-channel-guard` live-folder check can resolve the update folder from PowerShell
   ...
   invoke the C# instead â€” two sources for one rule is trap 4.
...
### Decided without asking â€” for `DECISIONS.md` when this is taken
...
- **`release-assets.yml` is deleted rather than guarded** â€” settled by observation, not doc.
...
- **`UI.Shared` is not renamed to `EQBuddy.Presentation`.** Churn outweighs clarity (Â§16.3).
...
- **`HiddenSections` migrates to HUD content only; `MiniStats` seeds the HUD.** Adopted from
  Bevel rather than decided here â€” the default (translate hidden cards to hidden rooms) is the
  one that would have deleted features from people's products on upgrade.

---

## EQBuddy v2 Phase 0â€“2 â€” technical decomposition (charter Â§25 item 6, issue #275)

- **Priority:** `ready` â€” **for Phase 0 only.** Phase 1 is written here in full and is
  **BLOCKED**: Helm's 2026-09-04 11:50 AM CT ruling says Avalonia removal waits on the
  ...
  staging**, not buildable work â€” its first PR waits on Bevel's IA.
  **License constraint (owner 2026-09-04):** Evolved/v2 = proprietary ARR; published 1.x stays MIT;
  ...
  implement license code in Phase 0 â€” docs PR #276 owns `LICENSE-EVOLVED.md` + PRODUCT/LEGACY/README.
- **Not `needs-david`.** Every call below is inside the owner-approved charter; the two
  places a line could have been drawn differently are marked **â†’ Helm** and named, not
...
- **Class:** `V3` â€” a platform line is being cut under live users, the update channel is
  ...
  and nothing we ship afterwards can reach it. Not V0â€“V1 because no single answer finishes
  it and the obvious version ("stop building the tarball") is wrong for a reason only
  visible with the whole system in view â€” see P0-2 rule 4.
- **Source:** `docs/v2/EQBuddy-v2-Project-Guide-Requirements.md` rev 1.1 (owner-approved
  2026-09-04) Â§Â§3 / LEGACY-001â€¦007 / 16.2â€“16.6 / 19 / 25 item 6; `HELM.md` Â§"EQBuddy v2
  Phase 0 / #275 â€” Windows-only posture", signed Helm 2026-09-04 11:50 AM CT;
  `HELM-FEEDBACK.md` same stamp; issue #275.
- **Bevel pre-design: Phase 0 YES but small; Phase 2 YES and it gates everything.**
  Phase 0's only player-facing surface is the LEGACY-002 notice â€” its wording, and the one
...
- **Shot offline: N/A for Phase 0.** The notice appears only when the live feed reports a
  major-2 release, which `shoot.ps1` has no way to stage â€” the fixture profile has no
...
- **Column budgets: 320 px, wrapping â€” so the budget is HEIGHT, not width.**
  `MainWindow.xaml:87` `NormalRoot Width="320"` and `MainWindow.cs:115`
  `_normalRoot = new() { Width = 320 }` â€” both lanes. Both banners already wrap
  ...
  **Trap 12 is live here:** both widgets are `SizeToContent`, so a string that fails to
  wrap is a geometry change on a transparent always-on-top window over a fullscreen game â€”
...
- **Guards run eight times.** The new policy tests are pure functions with no clock, no
  network and no file, so flake is unlikely â€” but `LegacyPlatformUpdatePolicyTests` and the
...
- **What clamps it:** `AppSettings.LegacyFinalNoticeAcknowledged` is a **new** setting with
  ...
  `_lastUpdateCheck`, which is **not** persisted â€” see the epoch note in P0-2.
- **Must-list rows on these surfaces:** none on the update banner today â€”
...
### PHASE 0 â€” protect v1 users before cutting platforms (`ready`)

Charter Â§19 Phase 0. Four PRs, in this order. **P0-4 is last and it is irreversible in the
useful direction: once `legacy-v1` exists at the bridge commit, Phase 1 is unblocked.**

#### P0-1 â€” Bridge release mechanics (LEGACY-001, LEGACY-004)
...
1. **`release.ps1` gains `-Prerelease`** (passes `--prerelease` to `gh release create`).
   ...
   `releases/latest` excludes prereleases and drafts â€” so a v2 milestone published as a
   ...
   â†’ **Second belt, free:** `ParseRelease` does `Version.TryParse(tag.TrimStart('v','V'))`
   and returns `null` when it fails, so a tag shaped `v2.0.0-beta1` offers nothing even if
   it *were* marked latest.
   â†’ **Hypothesis, not fact:** I read GitHub's documented behaviour for `releases/latest`,
...
4. **Version number:** the charter refuses to hard-code one and so does this plan. Current
   ...
   P0-3, with its own `WhatsNew.json` entry â€” and that entry is the **only** in-app
...
#### P0-2 â€” LEGACY-002: stop non-Windows v1 chasing v2
...
This is trap 47's shape exactly â€” *never let two code paths decide one destructive
question* â€” with "destructive" replaced by "may we tell this player to install something
...
Output: a small **record**, not a bool â€” a bool is what produced #93.
`(bool ShowUpdateOffer, bool ShowFinalLegacyNotice, bool RecordAcknowledgement, string? BrowserTarget)`.

Rules:

1. **Windows â†’ unchanged, in every case.** The policy returns "behave as today" and the
...
2. **Non-Windows and `info.Latest.Major >= 2`** â†’ never `ShowUpdateOffer`.
   - automatic path (`manual: false`): show the notice **iff** `!acknowledged`, and
     `RecordAcknowledgement`. After that, nothing.
   - manual path (`manual: true`): **always answer.** A player who opens Help â†’ Check for
...
3. **Non-Windows and `Latest.Major < 2`** â†’ today's behaviour, byte for byte. A later
...
4. **The browser target under rule 2 is the FINAL LEGACY RELEASE, never `latest`.**
   ...
   falls back to `UpdateChecker.GitHubLatestPage` â€”
   `https://github.com/.../releases/latest` â€” which is *the v2 release page* the moment v2
   ...
   negative: `DoesNotContain("releases/latest", target)` for every non-Windows Ã— major-2
...
`Known` row â€” check that it stays that way rather than assuming it.

**Affordance â€” the one thing for Bevel.** The Avalonia banner is a single clickable
`Border` (`_updateBanner`, `PointerPressed â†’ OnUpdateBannerClick`); the WPF one is
...
legacy release page", "I have read this", or both?** Recommendation â€” both, because a
...
- `LegacyPlatformUpdatePolicyTests` (in `EQBuddy.Tests`) â€” the full matrix: 4 platforms Ã—
  {1.99.x, 2.0.0} Ã— {manual, automatic} Ã— {acknowledged, not}, plus the two negatives
...
- A **source scanner** in `LogJanitorPolicyTests`' shape, asserting that all six call sites
  ...
  names â€” *tick / menu / policy*, not `manual` vs `not` (trap 49: a two-actor model is how
...
#### P0-3 â€” Support matrix and transition docs (LEGACY-006, LEGACY-007)

- **New `LEGACY-V1.md` at the repo root** â€” charter Â§24 names this file. Contents: the
  ...
  independently â€” **v1 / published 1.x (MIT) only**, worded as an invitation and **not** as a
  commitment by David to maintain anything. **EQBuddy Evolved / v2 is proprietary
  (All Rights Reserved)** â€” no use/copy/modify/redistribute/fork without David's written
...
- **`README.md`** â€” lines 37â€“38 currently describe a cross-platform build that "tracks
  ...
  â†’ **Lines 620â€“628 are the CREDITS block. Credit stays; status changes.** Someone editing
  ...
  â†’ `README.md:556â€“583` describes building and releasing the Avalonia lane; that is
  developer documentation and it becomes `legacy-v1` documentation.
- **`docs/FeatureGuide.md` Â§"Updates" (line 657)** describes the update flow to players and
...
- **`docs/TestPlan.md`** â€” new rows for the LEGACY-002 behaviours, marked Auto where the
...
- **LEGACY-007 is a v2-release-time obligation, not a Phase 0 edit.** It lands as a
  ...
  â†’ **Optional, cheap, and offered rather than assumed (â†’ Helm):** a sibling of
  ...
  â†’ **If it is written, mind trap 54.** `whatsnew-guard.ps1`'s first run reported 111 of 129
  ...
  `[Console]::OutputEncoding = [Text.UTF8Encoding]::new($false)` â€” and per the tooling note,
  Windows PowerShell 5.1 defeats even that, so a red first run is a host difference until
  `git diff` says otherwise.

#### P0-4 â€” Final legacy tag and `legacy-v1` branch (LEGACY-005)
...
- `git branch legacy-v1 <bridge-tag>` then `git push origin legacy-v1`. Exact name is an
  implementation decision (charter Â§26); `legacy-v1` is what the charter suggests and there
...
- Protect the tag and the branch through GitHub settings so LEGACY-004 is enforced by the
  platform. **â†’ Helm** â€” repo settings are Helm's sequencing call, not a consequence-list
...
#### PHASE 0 GATE â€” and how it is actually proved
...
1. **Unit** â€” `LegacyPlatformUpdatePolicyTests`, the full matrix and both negatives.
...
2. **Scanner** â€” all six call sites route through the policy; a seventh fails the build.
3. **Rendered** â€” the Avalonia headless harness (`WidgetRenderTests`' constructor pattern,
   ...
   â†’ **Known limit, stated rather than papered over:** `UpdateChecker.CheckGitHubAsync`
   ...
   plus the policy and leave the fetch uncovered â€” which is where it already is. **Prefer
...
4. **Real, and this is the one that counts** â€” publish the first v2 milestone as a
...
generic release page â€” which is precisely why LEGACY-007 exists, why the `-Prerelease`
...
### PHASE 1 â€” subtract platform debt (**BLOCKED on the Phase 0 gate â€” do not start**)

Charter Â§19 Phase 1 and Â§16.2. Written now so Phase 0 is executed with the destination in
view; **no Phase 1 PR opens until Helm confirms the LEGACY checklist on #275 is complete.**

#### P1-1 â€” Disposition of the 23 Avalonia test files, BEFORE any deletion

**This is the finding that reorders Phase 1.** `docs/TestPlan.md` Â§5 records that the WPF
...
`workflow_dispatch` â€” so it is also the only rendering coverage that runs on a push.
Charter Â§16.2's "remove Avalonia-only tests from v2" is correct as direction and, executed
...
- `AppThemeTests.EveryCatalogThemeAppliesCleanly` is a **catalog** guard wearing an Avalonia
  coat â€” it is the thing that would notice a broken palette, and it has no WPF twin. Decide
  it explicitly.
...
guards nobody runs â€” which is worse than deleting them honestly.

#### P1-2 â€” The 18 shared-suite tests that scan the Avalonia source
...
single-lane must-list (still valuable â€” trap 34 and trap 43 are both single-lane guards) or
...
narrow silently â€” that is the failure mode of every guard in the trap list.
...
PR that removes them â€” those PRs are Phase 2 work (see the sketch) and the exemption list
...
#### P1-3 â€” CI and release pipeline

- **`ci.yml`** â€” delete the `build-avalonia-linux` job and the "Run Avalonia render tests"
  step. Unambiguous: this workflow triggers on `push`/`pull_request` and runs from the
  branch. Do this **after** P1-1, not with it.
- **`release-assets.yml`** â€” **verify before deciding, and I did not.** The open question is
  ...
  - if the tag's own copy runs â†’ **delete the file** on v2 mainline; legacy tags keep theirs
    and can be re-published forever, satisfying LEGACY-004;
  - if the default branch's copy runs â†’ **do not delete it.** Guard the job instead
...
- **`EQBuddy.slnx`** â€” drop both Avalonia rows.
- **`scripts/check.ps1`** â€” drop the `avalonia` stage (line 64); the `-Quick` switch loses
  its reason for existing and should go with it rather than becoming a no-op flag.
- **`scripts/release.ps1`** â€” `-Prerelease` lands in Phase 0 (P0-1); noted here so the
...
#### P1-4 â€” Wine/CrossOver: name the boundary before deleting (**â†’ Helm**)

**Fact, and it is the opposite of the intuition:** 40 files mention Wine or CrossOver, and
the load-bearing ones are on the **Windows** lane â€” `src/EQBuddy/WineText.cs`,
...
players running **`EQBuddy.exe` under CrossOver on macOS** â€” traps 40, 41 and 42, which cost
...
**settings** in the v2 UI"; Â§16.2 removes "Wine/CrossOver **UI accommodations** from v2
Windows UI".

**Recommendation:** remove the three Options **knobs** â€” that is what UX-010 names â€” and
...
the line falls inside it â€” scope, which Â§21.2 gives Helm. It is deliberately not decided
...
#### P1-5 â€” v1 feature inventory and classification
...
- **One row per feature:** name Â· today's door(s) Â· v2 domain (Home / Live / Progress /
  Gear / Quests / World / Search / Settings) Â· class (Keep / Merge / Replace / Advanced /
  Remove) Â· why Â· **what writes it**.
- **Every `Remove` and `Merge` row names what stops writing.** Traps 20, 26 and 43 are one
  event told three times â€” a surface folds, the data survives, the write path does not â€”
...
- **Trap 30 applies to the tooling:** `scripts/shoot.ps1` enumerates `BreakoutKind` by hand
  and `scripts/` matches windows by **title** (trap 53 â€” three stale titles dark-lit the
  whole batch for six days). Any row that renames or removes a window is a `scripts/` grep
  before it is a code change, and the batch â€” not a single `-Shot` â€” is what proves it.
- **Output** goes to repo markdown (charter Â§21.1) â€” propose
  `docs/v2/v1-feature-disposition.md` â€” with per-feature GitHub issues carrying the work
  state.
...
### PHASE 2 â€” v2 shell and HUD: architecture sketch (for Bevel's parallel staging)
...
| Kills & Drops theme (`CreatureWindow`, `DropsCardView`) | folds into Live / World per P1-5 â€” *not* a top-level domain |
...
| â€” | **Home** and **Search** are the only genuinely new pages |
...
charter's Â§16.4 ("MainWindow should lose responsibility, not just lines") has already begun
â€” `QuestChecklistView`, `WorldWindow` and the Progress fold are that work.

#### Four seams â€” three of them already exist
...
1. **A surface factory per host.** `MainWindow.NewProgressSurfaces()` and the trap-45 rule:
   *a method that returns a long-lived UI object is a transfer of ownership wearing a
   ...
   `SurfaceOwnershipTests`' two remaining exemptions are removed **by** this phase â€” the
...
2. **Framework-free presentation.** `UI.Shared` (94 files) already holds
   ...
   `TextRenderingPolicy`. **Charter Â§16.3's `EQBuddy.Presentation` IS this project.**
   *Recommend not renaming* â€” Â§16.3 says the boundary matters more than the namespace, and
...
3. **The mobile projection is the parity proof, and it survives Phase 1 untouched.**
   ...
   `SurfaceParityTests` asserts it. That is charter Â§16.6 already enforced. **The standing
   rule for every shell page: if a page hand-rolls a rule the projection reads from
   `UI.Shared`, that is #210 happening again** â€” and #210's direction surprised people,
   because it was the *phone* that was ahead.
4. **Navigation â€” the one genuinely new seam.** Smallest thing that works, per Â§16.5 (no
   ...
   `EQBUDDY_EXPAND` already takes `progress:raids` â€” a `page:room` deep link, on both lanes
...
- **The HUD is `MainWindow` minus what the shell takes.** Target responsibilities after
  ...
  context menu â€” and **no card rendering**. `ArchitectureTests`' hotspot entry is a glob
...
- **Chip placement (LIVE-008) has three actors, not two.** `MezChipsWindow`,
  ...
  state belongs in `UI.Shared` beside it. **Put the participants in the test names** â€”
...
Rewrite Core for aesthetics (Â§16.1). Introduce DI, a message bus or a plugin host (Â§16.5).
Start before Bevel's IA is signed. And the shell's **first** PR is a host with **one**
existing page moved into it â€” the World fold is the precedent this repo already has (five
...
exists that is a test, not an opinion â€” and it is worth writing as one, because "every
...
Phases 3â€“6. #250. #240 / the 320-cap. #264. #208 (live hold â€” do not open). Play Console,
signing and production secrets. Product-direction debate â€” the charter wins (Helm,
11:50 AM CT). Avalonia removal in the session that takes Phase 0. Tagging. Merging.

### Checked â€” what I actually read
...
**Hypotheses, labelled as such â€” none of these were verified:**
...
2. Which ref supplies the workflow file for a `release: published` event (P1-3). **Do not
   act on either branch of this until it is checked** â€” the wrong guess costs LEGACY-004.
...
- **`ParseRelease` + policy tests, and the wire fetch stays uncovered.** Could have made
  ...
  been the defect.
- **`UI.Shared` is not renamed to `EQBuddy.Presentation`.** Â§16.3 permits either; churn
  outweighs clarity.
...
- **Recommend keeping `TextRenderingPolicy` while removing the three Wine Options knobs**
  â€” raised to Helm as a scope question rather than settled here, because it lands on people
  running the supported Windows artifact.

---

## The WORLD theme â€” Travels & Deaths + Map, Spawns, Travel, ZoneShare become one theme

- **Priority:** **DONE 2026-08-27 â€” all five PRs executed, merged, staged in 1.99.13.**
  ...
  the amendment signed (Helm 2026-08-26 9:06/9:07 PM); PR 2â€“4 on PR #244 (execution report:
  `CLAUDE-FEEDBACK.md` 2026-08-27 â€” WorldWindow both lanes, the three standalone windows
  ...
  (was: `ready` for PR 0 and PR 1; PR 2â€“PR 4 gated on Bevel's pre-design answers. No
  `needs-david:` â€” the theme choice was David's, 2026-08-27, question tool, over Alerts.)
- **Class:** `V3` â€” four surfaces Ã— two lanes fold into one window; the heaviest pair in
  ...
  whole system in view â€” see the phone decision below.
...
- **Bevel pre-design: YES, and it gates the presentation PRs.** Filed alongside this plan
  (its standing preference â€” before the design, not after). The six questions are in
...
- **Shot offline: mostly yes.** Spawns/Routes/Travels tabs stage from the fixture log and
  seeded stores. **The Map tab is the known-hard one** â€” it needs zone geometry; the mobile
  ...
  bucket **and says so** (trap 22/23 â€” no silent coverage claim). `map-window`,
...
- **Column budgets:** the launcher line replaces the `MiscHeader` "N deaths" StatValue in a
  320-wide card header â€” headline parts only, tabs carry detail (the Progress clipping
...
- **Guards run eight times:** `drag-verify` on the new WorldWindow, and the Avalonia
  expand â†’ pop out â†’ close â†’ expand sequence test.
...
| `ZoneShareWindow` | 278 | 274 | Core collaborators only â€” needs NO lift |
| misc card body | ~6 lines + XAML | ~76 (`BuildMiscSection`) | the smallest fold yet |
| Ratchet | 4,613 / 4,214 / cap 4,635 â€” **22 lines** | 5,402 / 5,229 / cap 5,751 â€” ~349 | keep-if-it-fits |
| `LogParser` | 933 / 853 / cap 938 â€” **5 lines** | â€” | constraint: this plan must not touch it |
...
1. **The seam WPF needs already exists on the other lane, and it was written to be copied.**
   ...
   `MapWindow`'s reach into `main` and it is exactly that member set (`SpawnPoints` Ã—9,
   `Settings` Ã—4, `SpawnTimers` Ã—3, `CurrentZoneName` Ã—3, `CurrentSnapshot` Ã—2, and one
...
2. **`SpawnsWindow` needs only `Settings` + `PlayAlertSound`** beyond its shared VM.
   `TravelWindow` needs `ZoneGraph` + `CurrentZoneName` + `Settings` â€” inside `IZoneHost`.
3. **`ZoneShareWindow` was built for this day** â€” *"no MainWindow reach-back, so an
...
4. **`AlertSurface` already exists in Core and its `Spawns` tab is CONFIGURATION** â€” its
...
5. **Phone:** `map` and `spawns` are first-class on the wire (`CompanionSurfaces.cs:14-15`,
   ...
   zone-share**, and `CompanionMapMarker` plots only the 'you' marker and camp pins â€”
...
6. **Everything that must re-route, enumerated** (trap 29's lesson taken in advance): four
   ...
   (`:4368-4380`); the per-tick `_mapWindow.MaybeRefresh()` (`:2531`); `StarDeaths` â€” the
   only `MiniStats` writer for `"deaths"` (trap 20/26).

### Architecture

**Core â€” `WorldSurface`** (recipe step 1), the fifth sibling of `ProgressSurface`:
`WorldTab { Map, Camps, Routes, Travels }` with Themes.md's labels â€” Map Â· Camps & timers Â·
Routes Â· Travels. Wire keys reuse names that already exist where they exist: `map`,
`spawns`, `travel`, and `misc` for Travels. `TabForKey` aliases: camps/timers â†’ Camps,
routes â†’ Routes, travels/deaths â†’ Travels.

- **`ThemeCardKey = "misc"` â€” decided deliberately, the thing the ask flagged.** Every
  ...
  at all** â€” the step Themes.md calls "where silent data loss lives" simply does not run.
...
- **`InlineModeFor`, initial table (Bevel may move any row):** Travels = **Full** (the
  current card body â€” deaths, zones, markers â€” small player-driven lists); Map, Camps,
  ...
  engineering default.
- **`LauncherSummary(zone, zonesVisited, deaths, runningTimers)`** â†’ e.g.
  `Crushbone Â· 4 zones Â· 2 deaths Â· 3 timers`, parts omitted when empty.
  **Counts, never countdowns â€” in the launcher AND the tab badges.** A countdown in either
...
**Core â€” `TravelPlan`** (recipe step 2 for Routes): a pure module over
`ZoneGraph.Distance` â€” from, destination, hops, the step list, and the two wordings both
...
**PR 1 â€” the view lifts, both lanes, zero product change.** `MapView`, `SpawnsView`,
...
**PR 2 â€” `WorldWindow`, both lanes; the standalone windows retire.** `EqSegmentedStrip`
tabs (wraps), per-host surfaces via the factories. `MapWindow`, `SpawnsWindow`,
`TravelWindow` are **deleted** (the Gear & Loot precedent â€” a window kept "just in case" is
...
home in the same PR â€” including a **Drop marker button on the Travels tab** (the action
...
entries are left inert (the `BreakoutKind.Progress` ruling â€” a migration would be code to
...
**PR 3 â€” the theme card.** `WorldThemeCard` on both lanes (fifth instance of
`ThemeCardView`/`ThemeCardPanel` â€” the machinery needs nothing new). The misc XAML section
converts to the theme shell (the `ProgressSection` shape); `StarDeaths` moves into the
WorldWindow's Travels tab header with the surface it stars (trap 26 â€” list every control
of the old card and say where each went); tutorial page, Options row, `AbsorbedTitles`
("Travels & Deaths Â· Zone map Â· Travel route Â· Spawn timers are in here now" â€” those are
the words someone scans for); glance wordings in UI.Shared so the Avalonia card says the
same sentences.

**PR 4 â€” the phone.** The REAL parity gap, confirmed: **Travel and markers, not map or
spawns.**
- **`map` and `spawns` stay separate first-class surfaces â€” decided, and it is the
  opposite of the desktop fold on purpose.** They are the glance-while-camping surfaces
  (first in display order); the map is the one `fills` surface (trap 9 â€” its container
  contract is unique); and a tablet propped on a desk showing map AND timers at once is
  the product's uncontested ground. Parity is by shared module (steps 1â€“2), not by
  matching chrome â€” folding the phone to match the desktop would DELETE simultaneity from
...
- **New `travel` surface** reading `TravelPlan`; destination picked on the phone via the
  ...
  ticks (trap 8) â€” and it must NOT ride the map's sticky-payload machinery (trap 38:
...
- **Session marker pins on the phone map**, and a **drop-marker action**
  ...
  machine). Mirror whatever the map fingerprint does about ages before adding pins â€”
  hypothesis (b) below.
...
- **Ratchet, keep-if-it-fits:** WPF has **22 lines**. PR 1 and PR 2 must be net-negative
  ...
  paragraph** â€” the file's own history says these numbers rot.
- **LogParser at 5 lines is a tripwire, not a task.** Deaths, zones, locs and markers are
  ...
  `LogParser.cs` has taken a wrong turn â€” stop and re-read the plan.
...
- **Trap 20/26 in both directions:** the fold list (finding 6) is the checklist; PR 2 and
  ...
  PR notes. `DeadSettingTests` cannot see a lost WRITER whose setting has other writers â€”
...
- Unit: `WorldSurface` table tests (keys, aliases, absorbed list, inline table);
  `TravelPlan` (route, already-there, no-route wording â€” with one negative each, trap 39);
  `ThemeHost` is already generic and tested.
- Avalonia: `WidgetRenderTests` twins; the expand â†’ pop out â†’ close â†’ expand sequence for
...
survivors â€” their homes are a different decision); the all-time stats view; ZoneShare on
...
equivalent for the bell preview â€” verify before widening `IZoneHost`; (b) the map
section's fingerprint already excludes the 'you' marker's age â€” mirror its mechanism for
marker pins, verify in `CompanionSnapshot`; (c) `MapWindow`'s layer toggles hold no
settings writers that die with the window â€” the trap-26 list settles it; (d) the mobile
...
- **Priority:** **DONE 2026-08-28 â€” all three PRs executed, merged, staged in 1.99.14.**
  PR 1â€“2 on PR #248 (Helm-authorized 4b33824; execution report `FABLE-FEEDBACK.md`
  ...
  windows' hand-rolled right-click-clear was Verified-blind â€” the arithmetic is
  `QuestLedgerStore.ClearCount` now, scan-guarded. Flipped DONE in place so a grep for
  `ready` cannot find shipped work (third time this shape has needed it â€” the flip belongs
  in the executor's merge commit, not the next reader's pass).
  (was: `ready`, Helm-gated on plan last-look; PR 3 gated on Bevel pre-design â€” both
  gates were satisfied and all three PRs merged before this line was updated.)
- **Class:** `V2`, confirmed â€” the fix is to change which STORE is authoritative for a number
...
- **Source:** #241 DasGud, 2026-08-26 7:40 PM CT ("Showing I have 4 Sphinx Claws but
  unfortunately I have noneâ€¦ one Mithril Bands when I have zero and 15 Izah runes instead of
  my 17"). Claude's stub (2026-08-27) and the `SCRIBE-FEEDBACK.md` trace, both verified here.
  Nothing posted, no promise made to him.
- **Bevel pre-design: YES, and it gates PR 3 only.** PR 1â€“2 change numbers, not sentences.
  The questions for Bevel (executor files them verbatim in `BEVEL-FEEDBACK.md` when taking
  the item â€” this session could not, per the planning-only scope): (1) what the have-count
  MEANS now that it has two possible sources, and whether the detail pane says which one it
  used ("verified from your inventory dump, 2h ago" vs "log tally â€” EQBuddy can't see
...
- **Shot offline: yes, with staging.** Seed a `<Character>_<server>-Inventory.txt` beside the
  ...
  the fixture log. Predict the counts BEFORE shooting (trap 23) â€” the whole point of the shot
  is three numbers whose values the plan dictates. Hypothesis (c) below if the harness can't
  reach the folder.
- **Column budgets:** the detail-pane provenance line is an `IconLine` (a Grid, wraps â€” trap
  14 already paid for); the import Summary clause rides the existing Gear-card report line on
  a 320-wide card â€” keep it to the shape "Â· quest counts trued (3)", no item names.
- **Guards run eight times:** the new store tests are deterministic (tests call `Flush()`,
  never the debounced `Save()`), but the rule is the rule â€” eight consecutive green runs on
  `QuestLedgerStoreTests` before PR 1 is done.

### What I verified in source (every stub claim re-checked; two new findings)

1. **The stub's mechanism is right.** Have = `Entry.Total = max(0, Looted + Manual âˆ’
   Consumed)` (`QuestLedgerStore.cs:33`); `Consumed` is written only for crafts/merges,
   destroys, and named vendor sales (`SessionStats.cs:905â€“935`); hand-ins are invisible.
   `QuestMatcher` and the ledger contain no reference to inventory.
2. **The window already contains BOTH answers, one tab apart** â€” the stub missed this, and it
   ...
   `QuestsWindow.xaml.cs:688â€“726`, Avalonia twin `:1685`). The **mine/zone/all** views, the
...
3. **Stub question 5 is ANSWERED, and it is a confirmed defect, not a hypothesis.** A Sky
   Test turn-in â€” from the Sky tab's button AND from the quest detail's toggle, both lanes â€”
   routes to `SkyCompleteToggle.MarkTurnedIn` (`QuestsWindow.xaml.cs:1342â€“1358`, Avalonia
   `:2241â€“2253`), which writes `SkyQuestCompleted` + item `Acquired` flags and **never
   touches the quest ledger**. `RecordCompletion` â€” the path that consumes â€” is reachable
   only for non-Sky quests. So the âœ” click that `Consumed`'s own doc comment promises
   ("that stays the âœ” click") **does not exist for Sky Tests.** DasGud's 4â†’0 and 1â†’0 are
   this, mechanically; the 15â†’17 is off-log acquisition, which only a dump can see.
4. **The reconcile seam already runs end-to-end.** `LogParser.cs:184` parses
   `Outputfile Complete:` â†’ `OutputfileEvent` â†’ `SessionStats.cs:841` forwards â†’
   ...
   The launch replay re-offers announcements chronologically every start â€” the exact
   machinery a replay-safe reconcile needs is already exercised daily.
5. **Absence in a dump means zero.** The dump enumerates every slot â€” worn, bags, bank â€”
...
6. **The quest is real catalog data.** "Beastlord Sky Test: Windhowl/Spirit Render" is a
   ...
   Rune Izah are rows sky-023â€¦026 in `SkyQuestDefaults.cs:42â€“45`. The turn-in list is not
   disputed and is not wiki-data â€” per Helm, this item never touches the wiki.
...
widget card and the phone projection â€” five chances to miss one, and the phone would need a
...
**PR 1 â€” Core: `ReconcileInventory`.** `Entry` gains `Verified int` + `VerifiedAt DateTime`
(old files parse; no `CountingRulesVersion` bump â€” the reset machinery is for wrong log
counters, and these start at zero). `Total = max(0, Verified + Looted + Manual âˆ’ Consumed)`.
New `QuestLedgerStore.ReconcileInventory(characterKey, counts, writtenAt)`:

- **No-op** when `writtenAt` â‰¤ the per-character `LastInventoryReconcile` watermark (any
  trigger is idempotent â€” replay, relaunch, manual refresh), and when `counts` is empty
  (the `SetUnlockedClasses` bad-parse precedent: an empty dump must not erase knowledge).
- For the union of (dump items the `TrackFilter` admits) âˆª (the character's existing
  ...
  events bounce.
- **`Manual` is deliberately superseded** â€” the dump is the game's own statement of
...
- **Clamps move or the fix breaks the âœ”:** `SetManual`'s floor and `RecordCompletion`'s
...
**Where it runs: inside the ingest, at `case OutputfileEvent`, in log order** â€” not in the
...
entries. `ImportReportReachesASurfaceTests` gets its row â€” that must-list exists precisely
for this.

**PR 2 â€” the âœ” that was promised.** `SkyCompleteToggle.MarkTurnedIn` gains the ledger, the
character key and the turn-in needs, and consumes the split quest's items â€” the same list
...
**The achievements import deliberately does NOT consume** â€” it is a statement about
...
**PR 3 â€” provenance, Bevel-gated.** The detail pane's status area says which source fed the
counts and how old the anchor is; the no-dump state names the command with a â§‰ copy â€” which
...
Bevel wants the sentence there too, it rides the wire (trap 32 â€” no page-side literal;
...
1. **Precedence:** the dump overrides, at its write time, for every admitted item â€”
...
2. **Staleness:** solved structurally, no cutoff rule. The reconcile applies at the dump's
   position in the log TIMELINE, so a three-week-old dump cannot override this week's loot â€”
...
3. **Bags vs bank:** total possession counts, as the held tab already counts it. The
   "could I hand it in right now" split is real but separable â€” `Entry.Location` supports
...
5. **Two completion paths:** the explicit turn-in click consumes (PR 2 â€” the missing
...
- **The one place a user statement is machine-overwritten:** a Manual count meaning "my
  ...
  wins â€” the ledger is per-character and a turn-in needs the items on this character â€” and
...
- **Ratchet, keep-if-it-fits:** the MainWindow cost is a few wiring lines per lane, but
  Helm's 1.99.13 ruling stands â€” **the next loop that touches `MainWindow.xaml.cs` takes
...
- **Double-subtraction is impossible by construction** â€” a âœ” consume followed by a dump
  reconcile overwrites rather than accumulates â€” but the TEST for it still gets written.
- **LogParser is not touched.** The announcement regex has existed since 2026-08-20. A PR
  that finds itself editing `LogParser.cs` has taken a wrong turn â€” stop and re-read.

### Decomposition

- **PR 1 â€” Core:** `Verified`/`VerifiedAt`, `ReconcileInventory` + watermark, ingest
...
- **PR 2 â€” consumption:** `SkyCompleteToggle` (+ Epic twin per hypothesis (b)), all four
  call sites via the one helper, tests. `docs/TestPlan.md` rows for both PRs.
- **PR 3 â€” provenance (Bevel-gated):** detail-pane source line + no-dump â§‰ nudge, both
  ...
  with whichever release carries PR 1 â€” the counts moving IS the player-noticeable change.

### Verification

- **DasGud's triple as a unit test, verbatim:** ledger looted {Sphinx Claw 4, Mithril
  Bands 1, Wind Rune Izah 15}; dump {Izah: 17}; after reconcile â†’ 0 / 0 / 17. If this
...
- **Clamps:** a âœ” after a reconcile lowers Total (fails on today's clamp â€” run it against
...
- **The check David can do himself, at level 29, no endgame needed:** loot any quest item,
  ...
  â€” then type a manual +1 and watch it survive. This item is UNUSUAL in being fully
...
#243's leftover-item audit (parked, Helm, different ask â€” no fold); anything wiki-facing
(this is the "about the player, not the world" side of the line â€” no pack, no edit link);
...
completion state in `SkyQuestCompleted` and possession in the ledger â€” two facts, not two
...
`SessionStats.cs` ingest (`:905â€“935` consumed sites, `:841` OutputfileEvent, `:473`
forward); `LogParser.cs:179â€“184, :752`; both `QuestsWindow`s' held/mine/detail/
`ToggleCompleted` paths; both MainWindows' `OnOutputfileWritten` and `LatestInventory`;
`CompanionQuestSource.cs` in full; `SkyQuestDefaults.cs:42â€“45`; the `SCRIBE-FEEDBACK.md`
trace; `HELM.md`'s 2026-08-27 ruling. **Hypotheses, labelled:** (a) the rewardKey â†’
split-quest turn-in needs (with quantities) are reachable from `SkyQuestDefaults`/
`QuestChecklistLayout` without a catalog instance in hand â€” verify before widening
`MarkTurnedIn`'s signature; (b) the Epic toggle has the same non-consuming gap AND epic
catalog quests surface ledger have-counts â€” check both before mirroring PR 2; (c) the
shoot fixture can stage a dump beside its Logs parent â€” if not, PR 3's shot goes in the
manual bucket and says so; (d) the held tab needs no change and agrees with the ledger
near dump time â€” confirm once PR 1 is in, as its own sanity check.
...
â€” Fable 5, 2026-08-27 (from Claude's 2026-08-27 stub, whose mechanism survived
...
- **Priority:** `ready`. No `needs-david:` â€” he already chose "add shots for the easy ones"
...
- **Fable's ruling (2026-08-24): ALL of this is V1, including the `Prime` per-run-content
  work â€” no decision in it is outside the executor's, and if David answered any one question
  ...
  not a fraction â€” the fraction model is WHY appended content was unreachable.**
- **Source:** the 2026-08-24 doc audit. README embedded `gear-locker.png` (a standalone
  ...
  2026-08-16), plus `widget-compact.png` â€” a **v1.51-era** capture with an "Update v1.51.0 is
...
adding duplicates: compact widget â†’ `widget-cards`, Sky â†’ `sky-checklist`, mini â†’ `mini-bar`,
breakouts â†’ `damage-breakout`, options â†’ `options-mez`, and the freed row â†’ `raids-card`.
Regenerable coverage went **5/24 â†’ 11/24**. The three stale originals are still on disk,
unreferenced, deliberately not deleted â€” a live Reddit post may hotlink them.

### `history-progress` â€” attempted, reverted, and here is why
...
nowhere** â€” this is the useful part of the attempt:
...
2. **No session may be selected** â€” picking one replaces the charts with the detail pane.
3. There must be `dings` or cumulative AA to plot, **across more than one session**.

**DONE 2026-08-24 (Claude) â€” Route B worked on the first try, and your correction is why.**
My "same log PATH = same archived row" was wrong; `Checkpoint` adopts on
`(Server, Character, StartUtc)`. Verified in `SessionRepository.cs` before building on it â€”
...
**`Lines`** (per-run content, appended INSIDE that window rather than onto a shared tail â€”
...
Prediction written first and met: 846Ã—553, filter `Aludra (test)`, three sessions dated
Aug 21/22/23, and **"Level 22 â†’ 24 (Aug 21â€“Aug 23, 3 dings)"**. One miss worth recording â€”
...
earned, cumulative â€” 9 total"**, which is what README's caption has always promised.
`history-progress.png` is retired in favour of the regenerable `history-charts`.

**README regenerable coverage: 5/24 â†’ 12/24 â†’ 15/24 (2026-08-26, Claude).** Three more
landed in one pass: `options-behavior` (a tab-key `Set` like its siblings â€” it was always
...
`spawn-circles.png` and `travel-window.png` now depict retired windows â€” the
`gear-locker.png` class of stale, not slow rot â€” the moment 1.99.13 ships. The surfaces
...
real â€” `ZoneShareWindow` survived the fold untouched, its door now on the Map tab.
...
is unreachable at any fraction below 1.0 â€” and repeated same-character primes gave one session,
...
2026-09-02 â€” new trap 53. Titles now match `EQBuddy World`; all three re-shot and committed
(`spawns-window` is the World â–¸ Camps tab at Runnyeye, prediction written first and held).
`zone-map` re-shot honestly as the **no-maps-folder empty state**, because the throwaway
profile has no maps folder â€” a real state, and NOT a replacement for `map-window.png`.
README's two un-regenerable World captures were re-captioned in the repo's own "X is now Y"
form (Zone map â†’ World â–¸ Map, Travel route â†’ World â–¸ Path, plus the Camps and Map-tab
...
- `map-window` / `spawn-circles` need a **maps folder** in the throwaway profile.
  ...
  regenerable â€” on a machine with the game installed, which `shoot.ps1` already requires in
...
- `travel-window` needs `EQBUDDY_TRAVEL` to take a DESTINATION zone the way `EQBUDDY_SPAWNS`
  ...
  no settings-backed one. **Check `ArchitectureTests.Hotspots` first** â€” `MainWindow` had 16
  lines of headroom on 2026-09-02.
- Unrelated finding while reading `TravelView.cs:74`: the route list draws a ðŸ“ **emoji**,
...
free â€” both are ordinary windows the existing hooks nearly reach. The map/mobile/travel ones
...
## Window height follows content â€” CLOSED 2026-08-25 (Fable): shipped in 1.99.11's NC-grab design, harness-verified

The V2 item is done, by a third design neither candidate anticipated: 5b0f331 retired the
`ContentRendered` pin and takes ownership at `WM_NCLBUTTONDOWN` on a resize border â€” the
player's actual grab, so no attribution rule is needed at all â€” with persistence gated on
...
â€” the design never interprets `SizeChanged`. Residual: Item info stays excluded by a
...
- **Priority:** **DONE 2026-08-26 (Claude), staged in 1.99.12 â€” all three PRs, executed to
  the plan with two deviations worth your last-look.** PR 0: `respawn-diff.py` ran live and
  its first report is committed â€” all 3 trusted Crushbone timers are MISSING from the wiki,
  ...
  `RespawnSuggestion` with your exact bar (3 / Â±15 % / â‰¥90 s), the three-way compare, the
  pack row (`RowKind.RespawnObserved`) and export block, both lanes, staged shot predicted
  first and matched. **Deviation 1: `RespawnSuggestion` lives in Core, not UI.Shared** â€”
  ...
  never-loosens rule REJECTS for the countdown** â€” your "the three places a gap is accepted"
...
A mega-thread we host would be a **second source of truth competing with eqlwiki** â€” the
...
bound, and only sometimes that â€” the mob may have been up for an hour before anyone looked.
...
wiki?** The loot pack already has an answer to the analogous question worth copying â€” the
...
3. Triggered and raid-instanced entries have no cycle at all â€” they must never be suggested,
...
pack never publishes anything itself â€” the player opens the edit link, reviews, and saves.

### Plan â€” Fable 5, 2026-08-22
...
a new surface, not a timer-submission tool â€” the pack pattern applied to a second kind of
...
1. **The wiki's idiom is a free-text field per creature, and it is sparse.** `{{Namedmobpage}}`
   ...
   NOTHING on Eldrig the Old and Lockjaw â€” the field is absent, not empty. There is also a
...
2. **We do not hold a SAMPLE today, only the tightest value.** `SpawnOverride.RespawnSeconds`
   is the minimum gap ever accepted, with `Learned`/`Sighted`/`Imported` flags â€” one number, no
...
3. **The engine already knows which observations are honest.** `LearnFromRekill` accepts a gap
   ...
   (`NeverLeftSince`), the gap is â‰¥ 90 s, and the entry is not trusted/multi-spawn;
...
4. **The catalog already holds verified timers the wiki lacks.** Entries with `trusted: true`
   ...
   first contribution and needs no player at all â€” a diff report, not a feature.
...
**Core â€” `SpawnCycleLedger`** (`spawn-cycles.json` beside `spawn-overrides.json`): per
`server|zone|name`, a list of `Cycle(DurationSeconds, Kind: Rekill|Sighting, At)`, capped at
the last 20. Written by `SpawnTimers` at the three places a gap is ACCEPTED today â€”
`LearnFromRekill`, `LearnFromSighting`, `LearnDiscovered` â€” and nowhere else, so every gate
...
**Core â€” `EqlWikiMobs.Parse` reads `respawn_time`** into `MobInfo.RespawnField` (raw text;
`""` when absent).

**UI.Shared â€” `RespawnSuggestion`** (pure, unit-tested â€” the whole honesty bar lives here):
- **Bar:** at least **3** cycles, all within **Â±15 % of their median**, median â‰¥ 90 s.
  ...
  â€” the `SuggestRarity` rule verbatim.
- **Never** for `IsTriggered`, `RaidInstanced`, or `MultiSpawn` entries (no cycle / sibling
  noise), whatever the ledger holds.
- **Wording:** the wiki's own â€” minutes under an hour ("22 min"), hours ("6 hours"), days
...
- **Three-way compare:** wiki field (from `RespawnField`), catalog value, observed. Suggest a
  ...
  the spread; when it agrees, say "wiki already says 22 min â€” nothing to add" (the KnownDrops
  line for timers); when it disagrees, phrase it as the stat block does â€” *compare, don't
  overwrite* â€” with the cycle list in the edit summary.

**UI.Shared â€” `WikiContribution.BuildExport` and `WikiPackPresentation`** gain a respawn
...
windows already draw whatever the presentation returns. **Bevel pre-design: yes** â€” the row's
...
**Script â€” `scripts/harvests/eqlwiki/respawn-diff.py`** (flags only, like the rest of the
...
- **The bar is the product.** Too low and EQBuddy becomes the source of wrong timers on the
  shared reference â€” uniquely wrong, the thing the match-the-wiki rule exists to prevent. Too
  high and nothing is ever suggested. 3 / Â±15 % is a starting point logged as a decision;
...
- **Trap 20:** the ledger is a new file that only the engine writes and only the pack reads â€”
...
- **Long timers:** a 3-day boss needs nine days of one player camping to reach the bar. That
  is correct â€” under-suggest â€” and PR 0 is how those reach the wiki instead.

#### Decomposition

- **PR 0 â€” `respawn-diff.py`** + its report in the weekly refresh; no app change. One-off
  run now, human pastes what it finds.
- **PR 1 â€” Core:** `SpawnCycleLedger`, the three write points, `RespawnField` parse;
  `SpawnTimerTests` (a cycle is recorded exactly when a gap is learned and never otherwise:
  instance, triggered, placeholder-started, cross-stay, import â€” one negative each);
  `EqlWikiMobsTests` for the field. No UI.
- **PR 2 â€” UI.Shared + pack:** `RespawnSuggestion` + tests (bar, agreement, wording both
  idioms, three-way compare); the pack section and row kind; `WikiPackPresentationTests`;
  both pack windows unchanged or near it; staged shot; `docs/TestPlan.md` Â§3 rows; What's-new
...
#### Decided without asking (â†’ `DECISIONS.md`)

Creature field, not the list page; bar = 3 cycles within Â±15 % of median; median suggested,
...
- **Priority:** **DONE 2026-08-26 (Claude), staged in 1.99.12 â€” both PRs, executed as
  ...
  **The addition: your double-counting risk was real and one exclusion was not enough.**
  The staged shot's first run caught it â€” `ActiveRowId` is set by the FIRST checkpoint, so
  ...
  by row id AND by identity `(server, character, session start)` â€” Checkpoint's own
  adoption rule â€” with the failing case as a test. `MobHistoryTests` (9 cases incl. a
...
not be relaxed â€” the fix is to stop throwing away the evidence that would satisfy it.
...
This is the **all-time stats direction (#168 / #159)** â€” a query over archives already on disk,
...
1. **The data is on disk, in full.** `SessionRepository` stores every session's complete
   ...
   Zone)` â€” the exact record the pack consumes. No log replay, no new collection.
...
2. **`BuildExport` takes `MobObservation(MobSummary, lookup)`.** So pooling produces
   ...
   no change â€” the 10-kill bar is then met by twelve kills across three sessions because the
...
4. **Session `Zone` on a `MobSummary` is the kill zone** â€” the #65 fix â€” so pooling keys on
...
2. **"Since" filter: no filter, and the scope line shows the earliest date pooled.** A retune
   ...
   a visibly bimodal rate, that is the moment to add a filter â€” not before.
3. **Per-session vs all-time toggle: none.** The reporter is right that a smaller sample never
   makes a better wiki edit. Drops by Creature keeps the LIVE view â€” "is this camp worth it" is
   a different job and stays session-scoped; only the pack pools.

#### Architecture

**Core â€” `MobHistory.Pool(IEnumerable<StatsSnapshot> sessions, StatsSnapshot? live)`** â†’
...
(âˆ’1 stays "never seen"); factions unioned with hit counts summed; `LevelMin`/`LevelMax` the
...
several fake snapshots asserting the pooled counts â€” the reporter's own test description.

**Core â€” `SessionRepository.MobRows(server?, character?)`**: the `ProgressSeries` probe
...
**UI.Shared â€” `WikiPackPresentation.ScopeLine`** becomes the pooled form: *"12 kills of 4
creatures across 3 sessions Â· Dranak and Flossie on freeport Â· 2026-07-30 â†’ today"*. One
sentence, and it is the sentence that makes decision 1 and 2 honest. **Bevel pre-design:
yes** â€” this line is the surface's whole claim about itself. **Column budgets:** none (the
line wraps). **Shot offline:** no for the wiki caches (seed them); the `wiki-pack` shot must
also seed `history.db` with two or three sessions so the pooled numbers are visible â€” the
...
as it does for live ones â€” with the 2-in-flight cap, so a long history does not burst eqlwiki.
...
- **Trap 8 / perf:** the pool is recomputed on a signature, never per tick; the DB probe is
  the cheap one. A profile with hundreds of sessions is the case to measure â€” stage it.
- **Double counting:** the live session's checkpointed row AND the live snapshot â€” exclude
...
- **PR 1 â€” Core:** `MobHistory.Pool` + `PoolScope` + fixture tests (sum, fold, extremes,
  unknown-stays-unknown, (name, zone) keying, live-row exclusion); `SessionRepository.MobRows`
  with a test over a temp DB. No UI.
- **PR 2 â€” pack:** both windows on the pooled source; `ScopeLine`; the Drops footer hint;
...
revisited only on evidence); spawn cycles (the other item â€” a cycle needs stay evidence the
...
## Avalonia theme bodies need a seam â€” PR A DONE, PR B DONE 2026-08-26
...
- **Priority:** **DONE.** PR A executed 2026-08-22; **PR B executed 2026-08-26 (Claude),
  ...
  `progress:raids`); the expand â†’ pop-out â†’ close â†’ expand sequence is a headless test now
  â€” INCLUDING the window tab-change via a real click and the hand-back assertion â€” because
  ...
  written prediction (strip wraps, Experience body inline, â§‰ on the header). One defect
  ...
  theme window's freshly painted rows with an empty snapshot â€” the consumer only ever
  wanted the open edge. The CLAUDE.md `EQBUDDY_EXPAND` parity note is updated.
- **Class:** `V2`, unchanged.

### PR A â€” DONE, and what it actually cost

Option (a), as planned. `EQBuddy.Avalonia/IWidgetCard.cs` (seam, `ICardContext`,
`ProgressSurfaceSet`), `CardParts` for the shared row builder, and five views â€”
...
settings the rest of the app reads. **369 lines out of `MainWindow.cs` (5,598 â†’ 5,229);
baseline lowered to 5,229 in the same commit.** All 271 existing Avalonia tests pass
unchanged â€” the "tabs draw what the cards drew" claim carried across the seam.
...
2. **`SurfaceOwnershipTests` found the same hand-off on TWO more lanes** on its first run:
   ...
   is 1.99.4's release-on-close mitigation, not safety** â€” the day one of them expands in
...
### PR B â€” Inline themes PR 1, Avalonia half â€” STILL PLANNED

Unchanged from the plan: `ThemeCardPanel` mirroring `ThemeCardView`, `EQBUDDY_EXPAND=progress:raids`
honoured, the expand â†’ pop-out â†’ close â†’ expand sequence test, a `WidgetSheetTests` shot with
the prediction written first, landing with the What's-new line the WPF half already has.
**Nothing blocks it now** â€” `EveryHostGetsItsOwnProgressSurfacesAndTwoCanLiveAtOnce` is the
...
expand, change tab in the window, close, expand. **Runnable HERE â€” it does not need a Linux
...
## Inline themes â€” expand in place, pop out on request â€” PR 2 DONE 2026-08-26
...
- **The Avalonia half was the seam plan's PR 2/PR 3 lifts done together**: `KillsCardView`
  now exists on the Avalonia lane (and closed a quiet drift â€” the widget's kills panel
  ...
  is EMPTY** â€” with a positive-half test asserting the factories exist.
...
- The `_gearChecklistDirty`/`_inventoryDirty` flags died with their consumers â€” per-host
...
- Glance wordings are Bevel's, in UI.Shared (`CreatureTheme.DropsGlance`,
  ...
  three) â€” a prediction miss the first test run caught.
...
**PR 3 (Quests) DONE the same day â€” the whole Inline themes item is COMPLETE.** Item 7's
hypothesis verified: the checklist RENDERING lives inside each QuestsWindow (no view class
to instantiate twice), so the inline Full rooms are a new REDUCED body â€” `Core/QuestInline`
...
conservatively** â€” flagged in the pending Unlocks review ask. All four theme windows now
raise `TabChanged`; all four hosts hand the room back and forth; the â†—-arrow assertions
became chevron-era assertions per the plan's own "do not keep â†— on the collapsed row".
...
## Inline themes â€” expand in place, pop out on request

- **Priority:** **DONE 2026-08-26 (Claude) â€” all four themes, both lanes, staged in
  1.99.12; the DONE headers above carry the execution record.** Flipped in place so a
  grep for `ready` cannot find a plan whose work has shipped (eqbuddy-fb's catch â€” the
  other three items closed on this line and this one had only gained a header).
  (was: `ready` â€” David answered the one question that was his, 2026-08-22, asked with
  the question tool: build it as Bevel ruled it â€” expand in place, pop out on request;
...
- **Class:** `V2` â€” four themes Ã— two UIs, a host-ownership rule a compiler cannot see (a
...
- **Bevel pre-design: YES, before PR 1's screenshots** â€” specifically the expanded card's
  ...
  over the game.
- **Shot offline: no** for Kills & Drops (the Drops tab reads the wiki â€” seed every fixture
...
1. **The launchers are four `SectionLink` buttons whose KEYS are already the theme keys**
   (`kills`, `loot`, `quests`, `progress` â€” `MainWindow.xaml:457/512/534/603`, `SectionMap()`
   at `MainWindow.xaml.cs:649`; Avalonia `_sections[...]` at `MainWindow.cs:999â€“1060`). The
   cards keep their keys, so **there is no settings migration in this item** â€” no
...
2. **The two UIs own theme bodies DIFFERENTLY, and the plan has to say so or the Avalonia
   ...
   `NewGearCard()`, `new LootCardView`, `new DropsCardView`, `QuestsWindow` its own) â€” two
   ...
   through `IProgressHost.ProgressTabBody(tab)` and sets `_body.Content = â€¦`
...
3. **`SectionScroll.MaxHeight` already caps the whole card stack** (`WidgetMetrics.
   ...
   widget off the screen â€” it scrolls inside the cap like any tall card. Per-theme height
...
4. **`BreakoutKind.Progress` exists on WPF only** (`BreakoutWindow.xaml.cs:14`; Avalonia's
   ...
   reflection â€” both need a look when the member goes.
5. **The phone is the prototype and needs nothing.** `index.html`'s `.qtabs` card-with-tabs
   reads the same Core tab keys (`index.html:1158â€“1180`). Parity is already by shared module;
   this item brings the desktop TO the phone's shape, not the reverse.
6. **E2E already pins the launcher LINE** (`EndToEndTests.cs:91â€“97, 196â€“224, 519â€“522`:
...
7. **`QuestChecklistView` is one control hosting Epic and Sky** (`MainWindow.xaml.cs` ctor,
   ...
   session â€” **hypothesis:** the General tab is a search box plus a detail pane with its own
   `DetailScroll`, which is exactly Bevel's "do not shrink-wrap a full window" case.

### Architecture

**Core (recipe step 1 â€” name it where the phone and both desktops can read it).**
Each theme's surface definition (`ProgressSurface`, `LootSurface`, `CreatureSurface`,
`QuestSurface`) gains `InlineMode InlineModeFor(tab)` â†’ `Full | Glance`. Initial table:

| Theme | Full inline | Glance inline (one line + â§‰ into the window) |
|---|---|---|
| Progress | Experience Â· Wealth Â· Faction Â· Raids | â€” |
| Kills & Drops | Kills Â· Drops | â€” |
| Gear & Loot | Loot Â· Wishlist | Inventory (a long list with its own filter bar) |
| Quests | Epic 1.0 Â· Plane of Sky | General (search + detail pane; Bevel's host rule) |
...
moves while you play â€” Experience, Kills, Loot, and for Quests whichever of Epic/Sky the
player's class has rows in (else Epic).

**UI.Shared â€” `ThemeHost` (framework-free, unit-tested; the "sum, not pixel" rule).**
A state machine per theme: `Collapsed | Inline | Window`, with inputs `ToggleCard`,
`PopOut`, `WindowClosed`, `OpenWindow(tab?)` (the âš™/hotkey/`EQBUDDY_*` openers), and a
`SelectedTab` kept for the session only. Invariants it enforces, and the tests assert:
- **One owner.** `PopOut` from `Inline` â†’ `Window` and the card collapses. `ToggleCard` while
  in `Window` does NOT draw a second copy: it brings the window forward (Avalonia cannot
  show the body twice; WPF could, and must not, for the trap-15 reason). `WindowClosed` â†’
  `Collapsed`, never back to `Inline` â€” the player closed a thing; do not re-grow the widget.
...
- **A Glance tab never paints a body.** Its inline content is the tab's one-line summary
  and the â§‰; `Render` is not called on the full view.
Both windows already expose `OpenX(tab)`; the state machine tells them when.

**WPF â€” `ThemeCardView` (one class, four instances; `EQBuddy/ThemeCardView.cs`).**
Replaces each `<Button Style=SectionLink>` with `<Expander x:Name="â€¦Section"
Style="Section">` + `<ContentControl x:Name="â€¦Body"/>`, the `MotesSection` shape, so
...
`DesignSystem.InlineIconButton("External", â€¦)` â§‰ that sets `e.Handled = true` â€” the
...
the window's, and the strip WRAPS â€” trap 25) over a `ContentControl` holding the selected
...
`ICardContext` + the factories, never `MainWindow` â€” the seam exists for this.

**Avalonia â€” `ThemeCardPanel` (`EQBuddy.Avalonia/ThemeCardPanel.cs`, beside
...
call. No other code path may assign a theme body â€” grep for `TabBody(` assignments and
...
pop-out"). `DisabledBreakouts` entries for `"Progress"` are ignored, not migrated â€” nothing
...
breakout carried and say where each went (its xp line â†’ Experience tab, already there).

**Tour and Options (recipe step 7).** The tutorial page "Cards that open windows" is now
wrong; rewrite it to "Cards that expand â€” and can pop out". `OverlaySections.AbsorbedNote`
stays as is (the absorbed titles are still inside). Options â†’ Cards & windows row tooltips
for the four keys say "expands in place; â§‰ opens its window".
...
- **Avalonia one-parent:** covered above; the `HandThemeBodyTo` funnel is the guard, and a
  ...
  again without an exception â€” the sequence that throws if any assignment bypasses the funnel.
- **Trap 12 (timer-driven size on X11):** a body that grows on a CLOCK inside an expanded
  ...
  Experience's xp line ticks every second â€” it was a card before the fold and is the same
...
- **Trap 16:** the â§‰ is an `InlineIconButton`, hit area `IconInlineHit`.
...
- **Trap 21:** `docs/screenshots/` already has `section-progress.png`, `progress-card.png`,
  ...
  (`EQBUDDY_EXPAND=1`) will now photograph four expanded themes â€” **predict it and re-shoot
...
- **Trap 1:** the â§‰ and the strip sit under the UI-scale transform; nothing here does
...
- **PR 0 â€” Core + UI.Shared, no UI.** `InlineMode` on the four surfaces with the table above;
  ...
  Â§3 rows.
- **PR 1 â€” Progress, both UIs.** `ThemeCardView` / `ThemeCardPanel` born here. The breakout
  retirement and the `mini-bar` list. E2E: pin BEFORE the move â€” `progressInline=0/1`,
  `progressTab`, `progressTabs`, `progressWindowOpen` into `EQBUDDY_EXPAND`; assert expand â†’
  pop-out â†’ close â†’ card collapsed. `WidgetRenderTests` twin. Tutorial page. Baselines
...
- **PR 2 â€” Kills & Drops and Gear & Loot.** Same class, two more instances; Inventory is
  the first Glance tab, so this PR proves the Glance body. Drops inline fetches the wiki â€”
  seed the fixture caches.
- **PR 3 â€” Quests.** The Glance General tab and the `QuestChecklistView` hosting question
  (item 7 above â€” verify before building; if the checklist cannot be hosted twice on WPF,
...
### Amendment â€” Fable 5, 2026-08-22, after PR 1 (WPF)
...
them, so PR 1 had nothing to move and the WPF file grew 4,424 â†’ 4,504 (131 of headroom against
...
- **WPF:** PR 2's first commit lifts the `EQBUDDY_EXPAND` dump block (~130 lines of pure
  string-building â€” a sum, not a pixel) into `WidgetDump.cs`, a class taking `MainWindow` and
...
- **Avalonia:** the seam item above IS the lift â€” Progress first (PR A), then Loot and Creature
...
(`ProgressWindow.TabChanged`, both lanes) â€” done on WPF, to be mirrored in PR B.
...
- Reporter confirmation on #228 after release â€” tell daetien-lab and joeymavity exactly
  what to click, and that Motes is also still its own card (#227) if they prefer that.

### Out of scope

The phone (it is the prototype); the World, Alerts and Live Meters themes (not built yet â€”
...
The four theme windows and their tab strips (`EqSegmentedStrip` everywhere â€” never
...
in full; `MainWindow.xaml:450â€“625` and `SectionMap()`; `MainWindow.xaml.cs` card seam,
`NewProgressSurfaces`, `NewGearCard`, the window openers; Avalonia `MainWindow.cs:990â€“1075`,
`AppTheme.cs` `SectionCard`/`SectionLinkPanel`, `IProgressHost.ProgressTabBody` and
`ProgressWindow.cs:255â€“275`; all four theme windows' strip/body wiring in both UIs;
...
(how `QuestsWindow` hosts `QuestChecklistView` and the General search) â€” verify in PR 3.
