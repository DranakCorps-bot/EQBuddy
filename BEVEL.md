...
## 2026-09-07 ~8:05 PM CT — Owner LOCK item 3: Edit HUD timers vertical stack + grow toggle — face design (Bevel)

**Priority:** `approved` (owner-locked, `HELM-FEEDBACK.md` ~7:36 PM CT); face pass only, hands to
Soft Opus once Helm signs this entry. Soft ≤3. Play Console OFF. Not needs-david.
**Place:** `src/EQBuddy/HudChipRowWindow.cs` (`_panel` orientation, `Park`/`ParkAtAnchor`,
`RebuildEdit`), `src/EQBuddy.UI.Shared/HudChipRow.cs` (`Placement`, `WrapWidth`'s vertical
mirror, `DefaultOrder`), `src/EQBuddy/HudEditChip.cs` (`Build`'s nudge chevrons), `src/EQBuddy.
Core/AppSettings.cs` (one new grow-direction bool — see §2), `src/EQBuddy.UI.Shared/
IconPaths.cs` (`ChevronUp` — does not exist yet, see §4), `src/EQBuddy/HudExpandWindow.cs`
(`SlavedOccupiedHeight`, `Reveal`'s "Grow down" — read for the cross-check in §3, not changed).
**Source:** OWNER LOCK, `HELM-FEEDBACK.md` 2026-09-07 ~7:36 PM CT, carried verbatim below.
**Checked:** full read of `HudChipRowWindow.cs`, `HudChipRow.cs`, `HudEditChip.cs`,
`HudExpandWindow.cs` (`SlavedOccupiedHeight`/`Reveal`), `AppSettings.cs` (the retired
`SpawnChipsGrowUp`/`MezChipsGrowUp` comment at :336 and the retired `SpawnChips{Left,Top,Bottom}`
comment at :607), `SettingsAlertsView.cs:651-663` (why the old grow-up tick-boxes left Options),
`CLAUDE.md` trap 2 (the SA-2 tombstone) and trap 49 (the three-actor placement model). Not run
against the live app — source-read face pass, same caveat as every pre-design in this file.

> **OWNER LOCK 2026-09-07 ~7:36 PM CT (carried verbatim):** 3) **Edit HUD — Spawn / Mez / Buff
> Timers like v1.** Stack **vertically** (not horizontal). Toggle grow **up or down**. Soft Bevel
> face (match v1 behavior) then Soft Opus implement. Cross-check OE-8 free-drag / expand chooser
> so this is Edit-HUD layout of timer kinds, not a fight with parked windows. Soft ≤3. Play
> Console OFF. Not needs-david.

---

### 0. What "like v1" is actually asking for, read against what SA-2 deliberately ended

v1 had **two** independently-placed, independently-persisted floats — `SpawnChipsWindow` and
`MezChipsWindow` — each its own vertical list of chicklets, each with its own saved x/y and its
own `GrowUp` tick-box (`AppSettings.cs`'s own retirement comment: *"#95's 'boss timers above mez
timers, each growing away from the other' was an answer to there being two independently-placed
stacks"*). SA-2 (Helm-signed 2026-09-05) folded both into **one** companion — `HudChipRowWindow`
— slaved to the widget every tick, laid out `Orientation.Horizontal`, with no saved position at
all. That fold is what retired #122/#152 (CLAUDE.md trap 2) for good: nothing here saves a
position, so nothing can walk up the screen across reopens.

**The lock's own last clause already resolves the scope question I would otherwise have to ask:
"not a fight with parked windows."** That rules out reopening the two-window, two-position
architecture — this is not "bring back `SpawnChipsWindow`/`MezChipsWindow`." What "like v1"
asks for, read narrowly, is the **visual/interaction shape** those two windows had — a vertical
list of chicklets, growing away from an anchor in a chosen direction — applied to the ONE
consolidated row SA-2 already built, not a second reopening of the position-persistence debt.
That is a smaller, safer change than #2 in the 2026-09-07 ~7:10 AM lock (free-drag), which
Bevel routed to Fable specifically because it reintroduced saved geometry; this one adds no new
persisted position, only an orientation and a direction, and both are computed the same way
every tick that `Placement` already is.

---

### 1. The layout: `WrapPanel` orientation flips, chicklet content does not

`_panel` (`HudChipRowWindow.cs:117`) is a `WrapPanel { Orientation = Orientation.Horizontal }`
today, capped by `MaxWidth = HudChipRow.WrapWidth(area.Width)` so a fifth chip wraps to a new
row instead of clipping off-screen (trap 25). The vertical read is the exact mirror, not a new
mechanism: `Orientation.Vertical`, capped by a `MaxHeight` computed from the available vertical
run instead of `MaxWidth` from the horizontal one — a `WrapPanel` and not a `StackPanel` for the
same reason trap 14/25 already state it: a stack measures infinite in the stacking direction, and
a long list would run off-screen with no ellipsis and no overflow, silently.

**Each chicklet's own content is unchanged.** `HudChip.Build` draws one row of icon + name +
clock per chicklet regardless of the panel's orientation — v1's two windows were exactly this
shape (one horizontal mini-row per timer, stacked as a vertical list), so flipping the container
orientation is the whole of the visual change; nothing about an individual chip's face moves.

**Family grouping survives the flip for free.** `Follow` already keeps `_row` in family order
(`DefaultOrder`: Mez, Spawn, WatchFire, Buff, or the player's own `HudChipOrder`) and draws
consecutive same-family chicklets together — a vertical list reads that grouping more clearly
than the horizontal wrap did, since a v1 player's mental model was "the spawn ones are a block,
the mez ones are a block," not an interleaved wrap.

---

### 2. The grow toggle: one setting, computed at layout time, not a saved position

**What it controls.** Today's list order (Mez → Spawn → WatchFire → Buff, or the player's
`HudChipOrder`) always appends **away from** the anchor point the same direction — effectively
"down" once vertical, since a `WrapPanel`/list lays out top-to-bottom by construction. "Grow up"
flips which end of the anchor the list extends from: the newest/first item in stored order sits
nearest the widget and the list grows upward from there, matching what v1's boss-timer stack did
when parked above the widget. This is a **read direction**, not a position — the anchor itself
still comes from `HudChipRow.Placement` (slaved) or the OE-8 park pair (parked); the toggle only
decides which end of the rendered list touches that anchor point.

**One setting, not per-family, not per-window** — matching the lock's plain English ("toggle grow
up or down") and the "not a fight with parked windows" instruction: one row, one direction, same
as today's one row having "a single growth direction" per the `AppSettings` retirement comment.
Reviving two independent `GrowUp` bools (one per family) would be re-opening exactly the #95
architecture SA-2 retired; a single `HudChipRowGrowUp` bool (default `false` = grows down, the
current behavior unchanged for every existing profile) is additive and keeps every untouched
profile's on-screen row identical to today's.

**Where it lives: Edit HUD, not Options — the same call SA-2 already made and documented.**
`SettingsAlertsView.cs:651-663` names exactly why the old two grow-up tick-boxes left Options
when SA-2 shipped: *"the control is the drag, and the way back is Edit HUD's 'Follow the HUD
again', which is where the row's other verbs already live."* A grow-direction toggle is the same
kind of verb (a live control over the row's own on-screen shape, not a durable preference about
sound or timing), so it belongs beside "Follow the HUD again," the mute ticks and the order
nudges in `HudEditChip`/`RebuildEdit` — not a sixth checkbox in Options → Alerts & chips.

**Face for the control:** a small two-state chicklet in the same shape as `HudEditChip.Unpark` —
icon + label + one clickable state, not two competing icons (trap 39's identity-must-be-real
lesson, and #148/#166's two-identical-shapes failure). Label reads the current state rather than
commanding an action, matching how the mute tick already reads ("muted"/live) rather than
"mute"/"unmute" as a verb: **"Stack grows: Down"** / **"Stack grows: Up"**, click to flip. Emblem:
`ChevronDown`/`ChevronUp` (see §4 — `ChevronUp` does not exist yet), the same vector language
`HudEditChip`'s nudge arrows already use, so the toggle reads as one more member of the row's
existing chevron vocabulary rather than a new one.

**Do not reuse the word "grow" bare — see §3.** Qualify it every place it appears near the mini-
bar ("the timer chip stack grows…"), because `HudExpandWindow.Reveal` already owns an unrelated,
owner-locked "grow down" (lock 10, the peek panel's reveal animation).

---

### 3. Cross-check against OE-8 (free-drag/park) and the expand bar (mini-bar chooser) — no fight, one real seam to close

**Free-drag/park:** orthogonal by construction, and this is the point of §2's "computed at layout
time, not a saved position" framing. `IsParked` (`HudChipRowWindow.Mode`) decides **where** the
anchor is; the new grow-direction bool decides **which way the list extends from** whatever that
anchor turns out to be. A parked row growing up and a slaved row growing down are both just
`Placement`/`ParkedPlacement` handed a taller or shorter rectangle to fit — no new field on the
park pair, no new write path, nothing OE-8's `HudDragGrip` needs to know about.

**The expand bar/chooser (`HudExpandWindow`, the mini-bar's peek/pin/pop panel) is a DIFFERENT
surface with its OWN, already-owner-locked "grow down"** (`HudExpandWindow.Reveal`, lock 10 — a
`ScaleTransform` reveal animation, not a list-append direction, and not user-toggleable). These
must not be described with the same bare word in copy or in a settings/tooltip string, or a
player reading "Grow down" in two different contexts will reasonably assume one control governs
both. Recommend the timer row's copy say "Stack grows down/up" (never bare "Grow down") to keep
the two apart in every surface a player can read them side by side (Edit HUD, any future
tooltip).

**The one real seam: `SlavedOccupiedHeight`.** `HudChipRowWindow.Park()` already asks the expand
bar how much vertical space it's claiming under the widget (`_main._hudExpandBar
.SlavedOccupiedHeight`) so the chip row parks below it rather than on top of it. A vertical chip
list is taller than today's one-line wrap far more often (every additional chip is a full row
now, not a few more pixels of wrap), and if the chip row's own grow direction is "up," it now
extends **toward** the widget and the expand bar sitting under it — the two can meet in the
middle for the first time. This is not a design question this pass needs to resolve (the "not a
fight with parked windows" instruction reads as "don't let me invent a collision," not "solve
every geometry case here"), but it is the one concrete implementation check I'd hand to Opus
directly: **when the chip row's own grow direction is "up," its placement arithmetic must treat
the expand bar's occupied height as space it cannot grow into, the same way it already treats the
widget itself** — `HudChipRow.Placement`'s existing `occupied` parameter is already exactly this
shape (widget height + bar height, summed, per the current code's own comment: *"the widget and
whatever is hanging off it are one block"*); growing up just means the chip row's own height now
has to be added to that same block when the auto-flip-above-widget rule decides whether the
COMBINED stack still fits above vs. below.

---

### 4. Two small build-time facts for whoever implements, named so neither is missed

- **`ChevronUp` is not in `IconPaths.cs` today** — only `ChevronDown`/`ChevronLeft`/`ChevronRight`
  exist. `ChevronDown` is `"M7.41 8.59 12 13.17l4.59-4.58L18 10l-6 6-6-6 1.41-1.41Z"`; the file's
  own convention for the left/right pair is "mirrored about x=12" (its comment at the
  `ChevronRight` entry) — the same construction gives `ChevronUp` as the vertical mirror of
  `ChevronDown`. Worth naming explicitly per trap 29's lesson (a control with no vector is
  invisible in a screenshot, not an error).
- **Edit HUD's nudge chevrons rotate with the panel.** `HudEditChip.Build`'s `canLeft`/`canRight`
  + `ChevronLeft`/`ChevronRight` nudge the stored order left/right today because the row reads
  left-to-right; once the panel is vertical the same two buttons read as reorder-up/reorder-down
  and should swap to `ChevronUp`/`ChevronDown` with matching tooltip text ("Move {family} up/down
  the stack") — the reorder MECHANISM (`HudChipRow.Nudge`, ±1 on stored order) does not change at
  all, only which two icons and words describe it.

---

**Not a hold, not needs-david.** No `src/` touched this pass. Filed to `HELM-FEEDBACK.md` as a
LIVE ASK carrying the OWNER LOCK verbatim, per this session's routing; Soft Opus implements once
Helm signs.
