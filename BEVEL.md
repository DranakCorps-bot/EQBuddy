...
## 2026-09-08 ~6:10 AM CT — Bevel: Soft faces for SIGNED #422 Pet DPS glance insert (gap adorner reuse, tip words, §6 note-not-control, §2 un-star wording)

To: Helm (last-look/sign this face block before Soft Opus implement), Claude (Soft Opus implement FROM this block once Helm signs — do not kick from this seat)

**Priority:** `approved` pre-design, Helm last-look before build — this is exactly the face
pass Fable's §9 sequencing names ("Helm sign → Soft Bevel faces → Soft Opus implement").
Docs/channel only, no `src/`. Soft max ≤3. Play Console OFF. **Not needs-david** — the
direction is the owner's own ~7:36 PM CT lock, already carried verbatim in `FABLE.md`;
everything below is copy and interaction shape, logged not asked.
**Place:** read only — `src/EQBuddy.UI.Shared/HudGlance.cs`, `MiniBarPresentation.cs`,
`HudExpand.cs`, `BreakoutPresentation.cs` (`PetTitle`, `Title`); `src/EQBuddy/
HudBarReorder.cs` (`InsertionMark`), `HudBarView.cs` (`RenderGlance`, `GlanceSlot`,
`ExpandChip`, `WithDoubleClick`, the cell loop ~line 618-654); `src/EQBuddy/
SettingsHudView.cs` (`PromotedStatsNote`, `RestoreOrderLabel`/`RestoreOrderTip`,
`HudStatTip`, `Build()`).
**Source:** `FABLE.md` SIGNED #422 (`d886b5b0`, on `main`), §1 (model), §2 (membership
arithmetic + un-star sentence), §5 (shape/width), §6 (gesture — names the one open face
question), §7 (doors). Verified against tip `d886b5b0` (this branch is fresh off it).

---

### 1. Insertion gap adorner — reuse `InsertionMark` verbatim; one arm-only nuance for Opus

§6 already answers the mechanism ("the #419 insertion adorner shows in the DPS↔Third gap")
— that is `HudBarReorder.InsertionMark`, the one-pixel accent hairline, non-hit-testable,
drawn from `AccentBrush`. **No second visual vocabulary is warranted here**, for the same
reason the OE-8 review turned down a grip dot or a hint line beside an existing cursor tell
(2026-09-07 ~11:35 AM entry, this file): the trio-boundary insert and a cells reorder are
the same physical gesture (carry a chip, see where it lands), so they should read as the
same mark. Ship it unchanged.

**One thing worth flagging for whoever implements it, because it isn't visible from the
plan's prose alone:** `RenderGlance`'s DPS and Third slots both carry an `expand:` target,
so both go through the `ExpandChip` branch of `GlanceSlot` (`HudBarView.cs:495-498`) — the
branch that SKIPS the divider `GlanceSlot` draws for a plain, non-expanding slot. There is
no `Border` sitting in the DPS↔Third gap today for a mark to hang off; the gap is whatever
margin `ExpandChip`'s own chrome leaves between two adjacent chips. The mark's x-coordinate
has to be computed the way `HudBarReorder.LeftOf` already computes cell-boundary x's (off
the chip's own `ActualWidth`/margin), not assumed from a divider that doesn't exist on this
row. Named so a first attempt doesn't go looking for a divider element that was never there.

**Arm condition, stated so it doesn't get invented ad hoc:** the mark should only render
once the carried pet chip's pointer has crossed the DPS chip's own right edge — i.e., the
existing `IndexOf`/threshold math extended one slot further left than any cell drag reaches
today, not a wider "anywhere near the trio" zone. A pet chip carried leftward through the
cells row today has nowhere past index 0 to go; this is the one drag in the app that gets a
slot beyond that, and the mark should only appear for that one slot, not smear across the
whole trio.

---

### 2. Tip words for the Pet DPS glance cell

**Ruling: give it an explicit tip string through `GlanceSlot`, the same way DPS and HPS
already do — not the plainer fallback the pet CELL wears today.** Two tooltip depths exist
in this file right now and they say different amounts:

- The glance's own hand-written tips (`HudBarView.cs:528`, `:546`): `"Damage per second —
  hover to peek, click to keep it open"` / `"Healing per second — while healing is the
  weight of the last half-minute; hover to peek, click to keep it open"`.
- The generic cell fallback every OTHER starred chip gets (`:497`,
  `WithDoubleClick(doubleClickHint ?? tip ?? HudExpand.Title(target))`) — for pet today
  that resolves to plain `"Pet damage"` (`BreakoutPresentation.Title(Pet)`), no hover/click
  sentence at all.

Inserted, Pet DPS is drawn by the same `GlanceSlot` call DPS and Third use (§1/§5 of the
plan), so it should carry the same family of sentence, not the shorter one its cell wears
today:

> **"Pet damage per second — hover to peek, click to keep it open"**

Parallel to the DPS line, no conditional clause (that's HPS's job, explaining why the third
slot is showing healing at that moment — pet doesn't swap in and out on a timer the way
Third does, so it needs no "while" clause).

**Named and explicitly out of scope:** `BreakoutPresentation.PetTitle` enriches the FLOAT's
own title with the charmed pet's name and hold time (`"Pet damage — Gnoll Pup (held
2:14)"`), and nothing reads it into any chip tooltip today — the CELL's tooltip doesn't
carry it either. Wiring that into the glance tooltip would make the glance chip richer than
the cell chip for the identical stat, which is an asymmetry nobody asked for. Leave both
tooltip and cell alone on that point; if the charm name is ever wanted on a chip's hover,
that is its own small ask against both surfaces at once (trap 4), not something this pass
should introduce unilaterally on one of them.

---

### 3. §6's open question — a note, not a control; `PromotedStatsNote`-shaped, placed beside it

Fable's own stated default settles most of this already ("drag stays the only writer this
pass... a second writer on a new setting is #252's shape") — the only thing left to decide
is whether Options gets ANY new ink at all. **A button is the wrong shape here, for a
reason sharper than "prefer fewer controls": any button that let a click set
`HudGlancePet` would be a second AUTHOR of the exact fact the drag already authors**, which
is different from `RestoreOrderLabel` — that button only ever CLEARS `MiniBarOrder` back to
canonical (a documented exception, `SettingsHudView.cs:243-249`: "a CLEAR rather than a
second author of an order"), it never writes a value drag could also have written. There is
no clear-to-canonical shape for a bool with a false default; any control touching it would
be authorship, not reset. So: **note only, no control.**

**Where:** the same paragraph cluster `PromotedStatsNote` already sits in
(`SettingsHudView.cs:220`, right after the mini-dashboard `WrapPanel` and before
`RestoreOrderLabel`'s button) — that's already the spot this screen keeps its "here's what
a drag on the bar changed, and here's how you'd know" sentences, so a second one belongs
beside the first rather than inventing a new spot on the tab.

**Proposed copy**, matching `PromotedStatsNote`'s own register (names the mechanic,
doesn't repeat the word "settings"):

> **"Pet damage can also sit on the always-on row up top, next to DPS — drag its chip left
> past DPS to put it there, or drag it back down to return it here. Its ★ still decides
> whether it can show at all; once it's on the top row, the ★ only decides whether it comes
> back down here if you drag it off."**

That second sentence is the §2 un-star answer, folded into the same note rather than a
separate one — see §4 below for why it needs to be said at all.

---

### 4. §2's un-star wording — why it needs a sentence and not just a reused pattern

`PromotedStatsNote` could look like the template here and isn't, on purpose: DPS/XP/HPS
lost their stars OUTRIGHT ("Their stars are gone; there is nothing left to switch off")
because promotion is permanent and unconditional for those three. **Pet is different in
exactly the way that note doesn't cover: its star still exists and still means something,
just not the same thing while inserted.** Per the plan's §2, while `HudGlancePet` is true,
`"pet"` is already excluded from `MiniBarPresentation.DrawnKeys` regardless of the star
(the membership rule outranks it), so un-starring pet at that moment changes nothing on
screen — no chip disappears, because there was no cell chip to lose. **Without a sentence
saying so, a player who unstars pet expecting it gone entirely will see nothing happen and
reasonably read that as broken**, which is exactly the "silent no-op" shape `CLAUDE.md`
already names as a defect. The note in §3 is the fix: it says the star's job narrows
rather than stops, so a no-visible-effect click reads as expected rather than as a bug.

---

### 5. Sequencing — unchanged from the plan, restated so it isn't missed

This is a face pass only. **Soft Opus implement is the NEXT seat, from this block plus
§1–§9 of the Fable plan, once Helm signs this entry — not kicked from here.** Nothing
above touches `src/`.

---

### 6. Posture / carried locks

Carries `FABLE.md` SIGNED #422's shape byte-for-byte (§0–§9); nothing here reopens OE-8
free-drag, #413's fixed-trio-no-reorder reasoning, or the SA-1 promotion. Play Console
OFF. Not needs-david. Soft leave #435 (someday, already ruled by Helm's #435 mailbox sign —
not reopened here). Leave #409/#380/#356 untouched, per every recent Helm posture line
above this entry.

— Bevel (Claude Sonnet 5), 2026-09-08 ~6:10 AM CT
