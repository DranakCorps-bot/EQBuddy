...
{
    /// <summary>
    /// The Evolved shell, while one is open — <c>null</c> the moment its ✕ is used.
    ///
    ...
    /// <c>ShellHost</c>, never from here — the widget does not own it."* A reference the
    ...
    ///
    /// Static because the process has exactly one widget — <c>SingleInstance</c> is what
    ...
    public static void Show(MainWindow main, string? address = null)
    {
        ...
        // than closing it — "silent no-ops are broken" reached by the one state the ✕ path
