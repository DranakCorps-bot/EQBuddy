...
public sealed class CompanionHost : IDisposable
{
    ...
    public string? PairingUrl =>
        _server is { } s && PairingAddress is { } address
            ? $"http://{address}:{s.Port}/#{_settings.CompanionToken}"
            : null;

    /// <summary>The address the QR and the URL actually carry. The player's pin
    /// (<see cref="AppSettings.CompanionPairingAddress"/>) when this machine still has
    /// that address, otherwise the best-ranked one. Null while stopped.
    ///
    /// <para>The fallback is the point: a pin is made on a laptop's home Wi-Fi and read
    /// again in a hotel, and a QR pointing at an address that no longer exists is a scan
    /// that silently reaches nothing — which is the bug <see cref="LanAddressRank"/> was
    /// written for in the first place (#264).</para></summary>
    public string? PairingAddress =>
        _server is { } s
            ? LanAddressRank.Resolve(
                [.. s.BoundAddresses.Select(a => a.ToString())], _settings.CompanionPairingAddress)
            : null;

    /// <summary>What the player PINNED, as saved — null when they never did. Distinct
    /// from <see cref="PairingAddress"/> on purpose: the picker has to show "Automatic"
    /// for a stale pin, and a UI reading the effective address would instead paint the
    /// fallback as if the player had chosen it.</summary>
    public string? PinnedPairingAddress => _settings.CompanionPairingAddress;

    /// <summary>Every address a device could be pointed at, best first — what the pairing
    /// window's picker offers (#264). Empty while stopped.</summary>
    public IReadOnlyList<LanAddressCandidate> PairingAddresses =>
        _server?.BoundCandidates ?? [];

    /// <summary>Pin the pairing URL to one of <see cref="PairingAddresses"/>; null or
    /// empty hands it back to the ranking. Saves immediately — the QR is regenerated by
    /// the caller from <see cref="PairingUrl"/>, and nothing restarts: every LAN address
    /// is already bound, so this only changes which one we PRINT.</summary>
    public void SetPairingAddress(string? address)
    {
        var value = string.IsNullOrWhiteSpace(address) ? null : address.Trim();
        if (string.Equals(value, _settings.CompanionPairingAddress, StringComparison.Ordinal)) return;
        _settings.CompanionPairingAddress = value;
        _settings.Save();
    }
