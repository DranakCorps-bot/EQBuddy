...
| `SpellCatalogDebouncedSaveLandsWithoutAFlush` — `debounced background save never landed` | 2026-09-12 PR #577, run `34692730242` (4307/4308) | `LearnedStoreDebounceTests.SpellCatalogDebouncedSaveLandsWithoutAFlush` | `ci/build-and-test` | **open** — observed-rerun. 10 s `File.Exists` wait for a background store write. The same run's `LandingSourceClaimsTests.TheFooterCarriesAQuietSupportLink` passed; tip is site-only plus this ledger row. **Do not expand product from this red.** |
