...
- **When quest/catalog data conflicts and cannot be resolved, match the wiki** (David,
  ...
  *uniquely* wrong costs trust in EQBuddy specifically, which is the whole point of
  ...
  Core/SkyQuestDefaults.cs (retired by DRA-47 into `Core/Data/GuideCatalog.json`'s Sky steps, where these entries' notes now live as `sources[].note`), which went the other way once and came back.
