# FABLE.md — archived entries (2026-Q3)

Rotated out of the live `FABLE.md` on 2026-09-21 by DRA-259 (DRA-144 F10), verbatim and
byte-for-byte. This is the first rotation `FABLE.md` has ever had: it was 501,593 B, 7x the
64 KiB policy and past its grandfather row of 478,113 B, at a measured 13.7 KB/day.

**What moved:** every `##` entry dated **2026-09-14 and older**, in their original
newest-first order — from `## 2026-09-14 ~7:45 PM CT — Fable: DRA-84 FARM GEAR — the pl…` down to `*No other items.*`, less the undated
operating charter described next. 470,386 B, 38 `##` entries.

**What did NOT move.** The live file keeps the 5 `##` entries dated 2026-09-15 and
newer. The four undated charter sections (`When this file is in play`, `How Fable reaches
Helm`, `How Claude calls Fable`, `Item shape`) were **moved to the top of the live file**
rather than archived — they are standing process, they are not dated, and they had been
sitting in the middle of this file where every date cut would take them. They are not
duplicated here.

**Three `###` sections below are ALSO re-pinned verbatim in the live file**, because live
code cites them by section number and `FABLE.md §n` must resolve: `### 4. Concurrency on
David2026` (the SCREEN mutex, "§4", six citers), `### §3 Updater vs separate download`
(TR-2, "§3", three citers) and `### 3. Visual tokens — Example 1 chrome mapped onto
Turquoise` ("plan §3, DRA-48", one citer — a shipped stylesheet). Their originals are left
in place here, in their own entries, so this archive stays a contiguous verbatim record.

Nothing here was reflowed, re-dated, re-worded or summarised.
