...
**Three rooms have landed** (PR 1: Progress · PR 2: Gear, World). All three are surfaces a
v1 fold had already unified, so each is a MOVE rather than a redesign — and each is a
SECOND host for a window that still exists, which is why so many rows below are about the
two agreeing rather than about either one being right.
...
| **No rail row for a room that has not landed** — a disabled row is an affordance that opens nothing. Rooms join `ShellPages.Landed` in the PR that lands them, and the rail draws that list | **Auto** — `ShellNavigationTests` (the landed three named, and the four that are not), `ShellHostTests` (`shellRail` equals the landed count); **Shot** — `shell-progress` / `shell-gear` / `shell-world` (three rows, no greyed four) |
...
| **Rooms are built on first arrival, not on open** — two of the three do real construction-time work (a ticking timer plus a ledger read; a scan of the game folder), and a shell opened to look at experience must pay for neither | **Auto** — `ShellHostTests` (`shellRooms=1` on a shell opened to Progress) |
