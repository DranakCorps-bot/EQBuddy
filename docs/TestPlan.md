...
| **A held Sky item is "no longer needed" only when EVERY class's reward that takes it is turned in** (#243, tvongaza) — a shared Wind Rune with one of six rewards done is not five-sixths of the claim, it is a different claim, and it goes in the separate `Other classes still want` band instead | **Auto** — `SkyLeftoversTests.ASharedRuneReachesBandAOnlyWhenEveryClassesRewardIsDone`, `TheShippedTableAgrees_HedaNeedsAllSixClassesBeforeItIsNoLongerNeeded` |
| **The "other classes" band is never produced without a class lens** — no lens is not a wildcard (#193's rule, one surface over): with no classes known there is no "other" to be other than. The "no longer needed" band is a claim about the game and is NOT lens-gated | **Auto** — `SkyLeftoversTests.WithNoClassLensBandBIsNeverProduced`, `BandAIsUnaffectedByTheClassLens` |
| **Another quest in the catalog wanting the item keeps it out of "no longer needed"**, and the band's hover says which quest — nothing here destroys anything, so the only cost this feature can impose is a wrong "you can free that". The split Sky Tests are the checklist and never veto their own items | **Auto** — `SkyLeftoversTests.AnotherQuestWantingTheItemKeepsItOutOfBandAAndIsExplained`, `TheSkyTestsThemselvesDoNotVetoTheirOwnItems` |
| A Reopen puts the item back out of the band on the next render (both bands read `SkyQuestCompleted` live), and a reward the #101 auto-grant guard refused still counts as open | **Auto** — `SkyLeftoversTests.ReopeningARewardTakesTheItemBackOutOfBandA`, `ARewardTheAchievementsGuardSkippedStillCountsAsOpen` |
| The row's words are said once for all three surfaces — `{Item} ×{held} · {where}`, with `where` collapsed to bags / bank / worn from the dump's own locations (the shared bank IS a bank) and dropped entirely when the dump has no row for it | **Auto** — `SkyLeftoversTests.TheRowAndTheHeadingsAreSaidOnceForAllThreeSurfaces`, `WhereCollapsesToBagsBankOrWorn`, `AnItemGainedSinceTheDumpIsListedWithoutAPlace` |
| The dump's own report counts the leftovers — band A only, never the weaker band, and the clause disappears rather than reading "0". It is not a warning: `Noted` stays 0 and the line stays green | **Auto** — `OutputfileAutoImportTests.TheDumpsReportCountsTheSkyItemsThatAreNoLongerNeeded`, `ADumpHoldingNoneOfThemSaysNothingAboutSky` |
