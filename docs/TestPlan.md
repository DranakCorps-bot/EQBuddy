...
| **Every theme card expands in place with ONE owner of the body** (Inline themes, complete 2026-08-26): pop-out collapses the card, closing the window leaves it collapsed, the tab follows the player both ways, and a Glance room (Drops, Inventory, Quests/General, Unlocks) draws its line and never builds its full view — on BOTH lanes, where on Avalonia two hosts of one control is a crash, not a layout bug | **Auto** — `ThemeHostTests`, `EndToEndTests` (placement facts for all four themes, named-room opens), `SurfaceOwnershipTests` (exemption list EMPTY; factories asserted positively); **Shot** — `theme-inline-kills`, `-kills-glance`, `-loot`, `-quests`, `-quests-epic`, predictions written first |
| An inline Epic/Sky room is ONE class's rows, capped at 12 with "... and N more", read-only — `QuestInline` owns the arrangement so the lanes cannot re-decide it | **Auto** — `EndToEndTests` |
...
| **Home's deep links cannot open a room that does not exist.** The body is a SECOND navigation surface, inside a room, where the rail's own filter cannot see it — a hand-written "Live" row would compile, render, photograph perfectly and open nothing. It reads `ShellPages.Landed`, the same list the rail draws from, and navigates through the same `ShellWindow.Navigate` | **Auto** — `HomeRoomTests` (every link is landed, Live is not offered, Home does not link to itself or to Settings), `ShellHostTests` (`shellHomeDeadLinks=0`, and the link count derived from `ShellPages` on both sides so Live arrives without editing the test); **Shot** — `shell-home` (four rows, not five and not six) |
| **A room's whole-room empty is CENTRED by the room, and its words come from `UI.Shared`.** Position is one decision for every room — a cell 520 to 1,400 units wide with an explanation in its top-left reads as a page that failed to load. Home is the first room whose most-likely-seen state is a room-level empty, and the first consumer of a ruling signed two PRs earlier | **Code** — `RoomEmptyState`, shared so four rooms cannot centre four ways; **Auto** — `HomeRoomTests` (the words: what is missing, the action, where, what happens next). The POSITION is not yet photographed — this harness seeds a fixture log by construction, so "no character at all" cannot be staged; the gap is named in `shoot.ps1` |
...
| **No rail row for a room that has not landed** — a disabled row is an affordance that opens nothing. Rooms join `ShellPages.Landed` in the PR that lands them, and the rail draws that list. A room's rail POSITION comes from `RailOrder` and is not arranged per PR: Home joining put its row at the top, above Progress, on its own | **Auto** — `ShellNavigationTests` (the landed five named in order, and the two that are not), `ShellHostTests` (`shellRail` equals the landed count); **Shot** — `shell-home` (five rows, Home first, no greyed Live or Settings) |
...
| **Every room gives back what it borrowed when the shell closes** — `SpawnsView`'s one-second timer, `InventoryView`'s cancellation token. Both are released by the v1 windows' own `Closed` handlers; a shell that did not would leak one of each per open, for the life of the process | **Code** — `IShellRoom.Release`, named on the interface so a new room has to answer it; the WPF layer has no unit tests and a leaked timer is invisible to a dump |
...
| Each room hosts its surfaces **exactly as they ship** — the Evolved reshapes (Raids to Live, Faction to Advanced, "what dropped for you" into Gear) all wait on rooms that do not exist yet | **Auto** — `ShellHostTests` (`shellProgressTabs=4`, and each room's tab count compared with its v1 window's); **Shot** — `shell-progress`, `shell-progress-raids`, `shell-gear`, `shell-world` |
...
| Unticking stops the window and leaves the star, so a mini-pill cell never vanishes as a side effect | **Manual** — §6. *(Held by the Avalonia render suite until E-2c; see [v2/avalonia-test-disposition.md](v2/avalonia-test-disposition.md).)* |
...
**`src/EQBuddy` (the WPF app, 25,053 lines across 85 files) has no automated coverage.
