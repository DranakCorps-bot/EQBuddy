...
| **Quests card** *(RETIRED 2026-09-05, HUD subtraction cut 1)* | ~~Card ⧉~~ → context menu **Guide…**, `toggleQuests` hotkey, and the **Guide** shell room | **Quests** | Keep | Kept here as the worked example of the obligation: the card's ⧉ was one of the three ways back, so the context-menu row went in **with** the cut — nothing is bound by default, so a hotkey is not a door (trap 59). The row it lost in Options → Cards & windows is the gap, recorded rather than papered over. | Nothing. `quests` was never a `MiniStats` key, which is why it went first. |
...
| Feature | Today's door(s) | v2 room | Class | Why | What writes it |
|---|---|---|---|---|---|
| **1. Quests** (BUILT, the template) | Context menu **Guide…** (→ the room); `toggleQuests` (→ the window); **Guide** shell room | **Quests** | Keep | One definition, one room. `QuestsView` is the lift; `QuestsWindow` is a thin host and is a later retirement, not this row. | `QuestsLeft/Top`, `QuestEraFilter`, `SkyQuest*`, `EpicQuest*` ← the surface itself. |
...
| `ShellWindow` | Context menu **Guide…** (OE-2's door, renamed 2026-09-08), plus `EQBUDDY_SHELL=1` as the review hook | **is** the shell | Keep | Already the destination. The door landed when the "rail has one row" premise expired at SR-5; ✕ on native chrome is what made it `must-fix`. | Shell geometry, per `ShellLayoutPolicy`. |
...
| `QuestsWindow` | ~~Context menu~~ (the row now opens the room); `toggleQuests`; the Loot views' 🗺 badge | **Quests** | Merge → retire | Already a thin host over `QuestsView`. | `QuestsLeft/Top`. |
...
| **Gear checklist** — *Open EQ Legends Tools* / *Import gear list…* / *Clear* — **SHIPPED 2026-09-05 (SR-2)**: the whole block, its status line included, is on `GearCardView` above the list, in both of that card's hosts; the mutations are `GearChecklistImporter.Apply`/`.Clear` | **Gear** | Merge | An import workflow is a domain action, not a setting. | `GearChecklist`, `GearChecklistName`, `GearInventoryAppliedStamp`. **Carries a `GameCommandsTests.SurfacesNeedingACommand` obligation** — the Gear tab is the surface that told a player to import a file and shipped no way to do it (trap 34). The ⧉ moves with the block. |
...
and **Guide…** are context-menu rows *and* shell rooms *and* (World) a card ⧉. The Quests row
