...
| **World card** (`misc`, was *Travels & Deaths*) | Card on the widget; `WorldWindow` (⧉, context menu **World…**, `toggleMap`/`toggleSpawns` hotkeys); ★ `deaths` | **World** | Keep | Same shape as Gear & Loot: the v1 World fold already made one window of the four tabs a room needs. Already landed as a shell room (E-3 PR 2). | `MiniStats["deaths"]` ← **`WorldWindow.xaml.cs:146/148` only**. `TrackSpawns`, `SpawnFollowZone`, `MapFolder` ← Options → Alerts & chips / Behavior. |
...
| `SpawnChipsWindow` | Automatic while timers run | **HUD** chips | Keep (host) | Already earns the overlay — a deadline with an action. Edit them on the HUD. | `SpawnChipsLeft/Top/Bottom`, `SpawnChipsGrowUp`. |
| `MezChipsWindow` | Automatic while a mez is believed active | **HUD** chips | Keep (host) | Same. | `MezChipsLeft/Top/Bottom`, `MezChipsGrowUp`, `MezChipsEnabled`. |
...
| **Overlay chips** | `SpawnChips*`, `MezChips*`, `AlertLeft/Top`, `PinWatchChips` | Keep | The chip stacks stay. Anchoring stays in `UI.Shared/ChipStackAnchor` — `ActualHeight` is 0 in a `Closed` handler and that cost #152. |
