...
|---|---:|---:|---|
| `EQBuddy.Core` | 123 | 34,983 | Parsing, aggregation, settings, catalogs, wiki, the v1 profile import. No UI. |
| `EQBuddy.UI.Shared` | 128 | 20,271 | View-model/formatting shared by the widget and the mobile projection. **Framework-free — enforced by `ArchitectureTests`.** |
| `EQBuddy.Companion` | 18 | 5,716 | LAN HTTP+WebSocket server and the mobile page. **UI-toolkit-free on purpose** — which is what let the Avalonia build host it unchanged while that lane existed, and what keeps it honest now that only one does. |
| `EQBuddy` | 109 | 40,213 | The WPF widget and its windows. Now the largest project in the repo. |
