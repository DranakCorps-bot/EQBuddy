## 2026-09-08 (lab experiments C′ + Context — EQBuddy only)

- **C′ local verification is a ladder by V0–V3 consequence, not one command for every Soft loop** · the default it could have gone the other way on is "always run `scripts/check.ps1` + E2E before you push" · ladder, because a V0 compact-trap line cannot see an E2E failure and a V3 profile write must not stop at a filtered unit test. **CI on the PR / `main` stay authoritative** — a local green is a ticket to CI, not a substitute. Lab experiment, not a Corps-wide standard; approach lives in `dranakcorps-ops`. Play Console OFF. No model-routing (B), no HELM-FEEDBACK migrate (D).
- **`Passed on rerun` is an observation on the flake ledger, never a disposition** · the default is treating a second green as the close, which is how the wiki in-flight and SettingsClobber signatures stayed "resolved" for a week at a time · occurrence, because the signature is still live until a cause is named (`open` / `watched` / `fixed` / `retired` / `accepted`). Seeded from Helm-signed handling already in the tree.
- **`CLAUDE.md` is the live manual; novels move to `docs/archive/`** · the default is leaving the 172 KB file as the session load, or gutting rules to hit a size number · split, because the progression is incident → lesson → executable guard → compact live rule, and a trap without its rule is how the next seat rediscovers it. Snapshot is byte-for-byte `docs/archive/claude-2026-09-08.md`. `LabDocsTests` holds the size band, trap numbers 1–68, and the ledger contract.
