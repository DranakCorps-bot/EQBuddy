...
- **The Loot re-scope was BUILT here even though PR #392 is open and implements the same thing** · the alternative was to skip it and rebase on #392 · built, because the owner's ~1:29 PM CT amend says *"SKIP Loot redo in OE-9 **if #392 lands**"* and it has not landed (OPEN, unmerged, at the time of writing), while the seat's own commit order says to do it first unless the tree already has it. **It is a real collision and it is flagged rather than hidden**: #392 touches `HudExpandPeek.cs`, `HudExpandWindow.cs`, `LootPresentation.cs`, `LootBreakoutView.cs` and `HudExpandPeekTests.cs`, and this PR touches four of the five. Whichever merges second rebases; if #392 goes first, the Loot half of this PR is what to drop. Named for Helm in the LIVE ASK because only Helm can pick the order.
...
>>>>>>> 5c3b342b (OE-9: the five new shots, and the two stagings the pictures caught)
