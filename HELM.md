## 2026-09-07 ~8:30 PM CT — PR #426 TestProfileIsolation harden (SIGNED; merge-when-green; E2E refuse soft-follow)

- **When / who:** 2026-09-07 ~8:30 PM CT — Helm last-look (webhook: HELM-FEEDBACK.md LIVE ASK addendum: isolation PR #426 open (head `936b2a11`), Helm last-look requested). Tip `f5571307` (channel addendum) / product `936b2a11` on `claude/test-profile-isolation-20260907` off `main` `c23058cf` (#423 merge). Implements Soft Opus HIGH from signed #423/~8:20 PM CT.
- **Thread / subject:** PR #426 https://github.com/DranakCorps-bot/EQBuddy/pull/426 — unit-suite isolation no longer trusts a bare inherited `EQBUDDY_APPDATA`. Soft ≤3 product files. At look: `build-and-test` SUCCESS; `e2e-windows` IN PROGRESS (run 34176794987). Ahead of `main` by 3 (incl. channel tip) / product tip ahead 1; behind 0. MERGEABLE / BLOCKED on pending e2e.
- **Ruling:** **SIGNED.** Soft merge #426 when `build-and-test` + `e2e-windows` green. Drop LIVE ASK tip at merge (prefer this Helm land / companion SSC). Live Holds empty. Play Console OFF. **Not needs-david** for this PR (restore door unchanged — still waiting David).
- **Asks answered:**
  1. Last-look / merge-when-green — **SIGNED** against signed #423 ask 2 shape. Spot-checked tip `936b2a11`: early-return on preset **GONE**; `Decide` pure; redirect ALWAYS unless `EQBUDDY_APPDATA_ALLOW_PRESET=1` **exact**; even then `PlayerProfiles` = `AppPaths.ProductDir` + `LegacyDir` → `RefusedPlayerProfile` still redirects to temp (only `KeptPresetByOptIn` returns early); `SameDirectory` trim/case; typo/`true`/`yes`/` 1` isolate; 16 `TestProfileIsolationTests` + live-process half; trap 68 additions-only. Opt-in name `ALLOW_PRESET` vs signed example `ALLOW_EXTERNAL` — **ACK / KEEP** (example was illustrative). Prove-fail claim (11/16 on restored early-return) ACK as filed; local 3,774 green ACK; no local re-run from this seat. CI unit bar STANDS.
  2. Further isolation surface (`tests/EQBuddy.E2E` child `psi.Environment`) — **ACK unaffected this cut.** Soft follow-up **AUTHORIZED** (own PR when Soft has room): same "never a player profile" refuse if the E2E child can ever inherit/point at live Evolved/v1 AppData — do **not** weaken e2e aiming. **Not a merge gate** for #426.
  3. Evolved `settings.json` restore — **needs-david STANDS** (already paged ~8:20). Do **not** write his profile. Snapshot `dist/evolved-settings-timestamp-2006.json` remains clobber evidence, not recovery.
- **Posture / Soft max ≤3:** (1) soft merge #426 when both CI green (HIGH isolation). (2) soft merge this SSC + soft merge #424 (#423 SSC) if still open. (3) Soft LEAVE #409/#380/#356; Soft LEAVE #425 Edit-HUD Bevel / #422 Pet DPS invent. Soft E2E refuse follow-up only after #426 on main. Until #426 lands: **STOP unit-test / `check.ps1` unit path under sessions that export Evolved AppData** still STANDS.
- **Claude kick via Dranak:** Soft: **merge #426 when `build-and-test` + `e2e-windows` green** (drop LIVE ASK tip). Soft land this SSC (`helm/ssc-426`) if not already on main. Soft merge #424 when green. Soft: no new Opus seat from this ruling (isolation HIGH discharged). Soft E2E refuse later under Soft max. Probe `claude models` only if a later kick. No TEL / Version / `v1.99.19` / Play Console / tag / publish / signing / prod secrets / Evolved wide publish / settings restore.
- **Scope hygiene:** Tests + CLAUDE.md trap 68. Not a hold. Restore still needs-david (not this PR). Live Holds empty. Play Console OFF.
