## 2026-09-09 ~10:40 AM CT — PR #472 guided-progression P1b (SIGNED; SIGNED-#445-P1b-complete; merge-when-green; #473 CLOSE WITHOUT MERGE)

- **When / who:** 2026-09-09 ~10:40 AM CT — Helm last-look (webhook: HELM-FEEDBACK.md LIVE ASK guided-progression P1b (DRA-28) BUILT, PR #472). Tip product `9dff4997` on `claude/guide-progress-store-p1b-20260909` off `main` `fcd357f2`. Ahead 2 / behind 0. MERGEABLE; at look `build-and-test` SUCCESS; `e2e-windows` IN_PROGRESS. Parallel Soft seat **PR #473** (`claude/opus-guide-p1b-20260909`, tip `cccd68bc`) opened the same minute for the same AUTHORIZE — Soft-max waste.
- **Thread / subject:** PR #472 https://github.com/DranakCorps-bot/EQBuddy/pull/472 — Soft Opus P1b only (`QuestLedgerStore.GuideProgress` per character + `UI.Shared/GuideProgressRouter` one door). Lands **dark** — no P1c render / P1d card / Phase 2. No WhatsNew. Authority: signed #445 §4/§9 + Founder lean-in ~2026-09-09 (Soft fill seats / P1b without waiting).
- **Ruling:** **SIGNED** as SIGNED-#445-P1b-complete. Soft merge #472 when `build-and-test` + `e2e-windows` green. Drop LIVE ASK tip at merge (additions-only KEEP on HELM-FEEDBACK). **CLOSE #473 WITHOUT MERGE** (prefer #472 — Soft-max duplicate seat). Live Holds empty. Play Console OFF. **Not needs-david.** Evolved profile restore **needs-david STANDS** (already paged — not re-paged).
- **Asks answered:**
  1. Last-look / SIGN #472 / merge-when-green — **SIGNED.** Spot-checked against #445 §4/§9 P1b: `CharacterLedger.Guides` → `GuideProgress` (`DoneObjectiveIds` / `SkippedObjectiveIds` / UTC `LastUpdated`); done↔skipped clear each other; untick-never-ticked writes no row; `Rekey` hand-copy + pre-tracking heuristic both count `Guides`; router in `UI.Shared` reads FROM / writes THROUGH `SkyCompleteToggle` for `RewardKey` (trap 4 — classic checklist / phone / import same tick); skip asymmetry KEEP (guide ledger for every objective; never undoes a turn-in); one-writer scan `TheRouterIsTheOnlyWriterOfGuideObjectiveTicks` (forbid + must-list, trap 34); survival `ATickSurvivesARestart` + `ATickSurvivesTheCountingRulesReset` (asserted beside a loot clear so the reset is proven). Local V2 claim ACK as filed (4028 / 31 new); CI e2e bar **STANDS** even with nothing rendering. Dark → no WhatsNew **KEEP**. `FABLE.md` P1 ladder STAYS until P1d.
  2. Parallel #473 (Core `GuideProgressRouting` + store refusal, write-through deferred) — **REJECT merge / CLOSE WITHOUT MERGE.** Intent ACK (Core-resident Func routing; read ignores ledger for reward keys). Prefer #472: plan §4 names **writes THROUGH** `SkyCompleteToggle` / `MarkRewardTurnedIn` as the one door; #472 implements that door now and proves reward path side-effects; two Soft seats for one HIGH = Soft-max waste (same shape as #428 prefer #426). Soft leave inventing a third P1b.
  3. Defaults Soft logged (router in UI.Shared; skip-on-reward allowed; UTC `LastUpdated`; no WhatsNew) — **ACK / KEEP.** Phase-3 Sky consolidation stays its own plan.
  4. David — **ACK not needed** for this PR. Founder lean-in already greenlit Soft filling P1b.
- **Posture / Soft max ≤3:** (1) soft merge #472 when both CI green (HIGH signed #445 P1b). (2) soft land this SSC (`helm/ssc-472`) when green. (3) Soft **CLOSE #473 WITHOUT MERGE** (comment prefer-#472). Soft Opus **P1c AUTHORIZED** after #472 on main under Soft ≤3 (`--model opus` / `claude-opus-5`, probe `claude models` first) — `GuideChecklistProjection` + Sky-tab both hosts + phone + `SurfaceParityTests` + E2E dump facts + `shoot.ps1` staging (traps 22/23). Soft continue Desktop gear-popup soft-captures from #465 if still owed. Soft LEAVE P1d until Bevel faces; Soft LEAVE Phase 2 authoring; Soft LEAVE Bevel note-split invent. Do **not** invent TEL / Version / `v1.99.19` / Play Console / tag / publish / signing / prod secrets / Evolved settings restore.
- **Claude kick via Dranak:** Soft: **merge #472 when `build-and-test` + `e2e-windows` green** (additions-only KEEP on HELM-FEEDBACK). Soft: **CLOSE #473 WITHOUT MERGE** (leave comment naming prefer-#472). Soft land this SSC (`helm/ssc-472`) when green. Soft: after #472 on main, `--model opus` / `claude-opus-5` (probe `claude models` first) **P1c** under Soft ≤3 → LIVE ASK → wake Helm. Soft: Desktop gear-popup soft-captures if still owed. Soft LEAVE P1d / Phase 2 / Bevel note-split invent. No Play Console / tag / publish / signing / prod secrets / Evolved settings restore.
- **Scope hygiene:** Not a hold. Not needs-david. Live Holds empty. Play Console OFF. Soft LEAVE inventing P1b from #454 **discharged** by this build. Duplicate Soft P1b seat **discharged**.
