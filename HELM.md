## 2026-09-07 ~1:10 PM CT — PR #389 Fable OE-9 expand-for-all peeks + Loot re-scope plan (SIGNED; Opus OE-9 gated on settings-reset HIGH)

- **When / who:** 2026-09-07 ~1:10 PM CT — Helm last-look (webhook: HELM-FEEDBACK.md LIVE ASK: Fable OE-9 plan for sign; tip `2fbd4ccd`; PR #389).
- **Thread / subject:** PR #389 https://github.com/DranakCorps-bot/EQBuddy/pull/389 (`claude/fable-oe9-seats-20260907` → `main`) — docs/channel only (`FABLE.md`, `HELM-FEEDBACK.md`, `BEVEL-FEEDBACK.md`). Implements #385 soft seat 3 / owner ~12:54 CT content locks (SIGNED on main via #386). Soft max ≤3. Play Console OFF.
- **Ruling:** **SIGNED.** Spot-checked against #385/#386 locks + Bevel #371 carve-outs as owner-amended (Kills IN, Deaths OUT): four `HudExpandTarget` adds (Motes/Kills/Procs/Money) on OE-7 pattern; `TargetForKey` bridge; trap-64 total destination map (Motes/Money→Progress:Wealth, Kills→Kills&Drops, Procs→Damage float + procs block); Loot peek = `TargetDropsContent` / never session; verification + WhatsNew named. Soft merge #389 when `build-and-test` + `e2e-windows` green (at look: build SUCCESS; e2e IN PROGRESS on run 34150324974). Prefer this Helm main land for SSC; drop LIVE ASK tip at merge; keep `FABLE.md` plan. Live Holds empty. Play Console OFF. Not needs-david.
- **Asks answered:** (1) sign plan — **SIGNED** (OE-9 Opus may go when a Soft max ≤3 slot opens after settings-reset HIGH). (2) seat split — **ONE seat `OE-9`** with Loot re-scope as first commit (cherry-pickable); OE-9a/OE-9b **rejected** (would spend a second Soft slot #385 §1 needs). (3) Procs ⧉ — **SIGNED** Damage float gains Live-room procs block (lock 6 / SA-2 anti-proliferation; Money lock already reads breakout as a section). New `BreakoutKind.Procs` float **named not built**. Bevel may adjust at implement review — **not a re-gate**. (4) Deaths OUT — **ACK / STANDS** (owner named four; Bevel carved both; owner pulled only Kills). (5) Loot empty-copy + Money peek density — **ACK** Bevel's under #385 soft seat 2 / Options IA; implement PR must not pre-empt Options rows.
- **Posture:** Soft max ≤3: (1) **#388 settings-reset HIGH** holds the Opus slot — **do not kick OE-9 Opus yet**; (2) soft merge #389 + this SSC; (3) Bevel Options/cog IA still AUTHORIZED (#386). Soft-close CONFLICTING leftover #387 (superseded by merged #386). Soft LEAVE #380/#356. Soft-owed: drag-verify `eqbuddy-screen.lock` (from #381).
- **Claude kick via Dranak:** Soft merge #389 when both CI green; drop LIVE ASK tip; land this SSC (`helm/ssc-389-oe9-plan`) on main. Soft-close #387. Continue/finish **#388** (`--model opus` / `claude-opus-5`) as Soft max priority. Soft Bevel (`--model sonnet` / `claude-sonnet-5`) Options/cog IA if idle. **Do not kick OE-9 Opus** until settings-reset HIGH is landed (or a free Soft slot opens with Helm ACK). No TEL / Version / `v1.99.19` / Play Console / tag / publish / signing / prod secrets.
