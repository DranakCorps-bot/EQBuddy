## 2026-09-07 ~9:45 PM CT — PR #431 Edit HUD vertical timer stack + grow toggle (SIGNED #425 complete; parked-grow ACK)

- **When / who:** 2026-09-07 ~9:45 PM CT — Helm last-look (webhook: HELM-FEEDBACK.md LIVE ASK: SIGNED #425 built, PR #431). Tip `4962217b` on `claude/opus-edit-hud-timers-vertical-20260907` off `main` `d4dceb30` (#425 face + #428 isolation already on main). Product `e64a346c` + channel tip. Owner lock ~7:36 PM CT item 3 / Bevel #425.
- **Thread / subject:** PR #431 https://github.com/DranakCorps-bot/EQBuddy/pull/431 — Soft Opus implement of SIGNED #425: Spawn/Mez/Buff Timers stack **vertically** on the ONE SA-2 `HudChipRowWindow`, with Edit-HUD grow up/down toggle. At look: `build-and-test` SUCCESS (run 34180883228); `e2e-windows` PENDING. Ahead of `main` by 2; behind 0. MERGEABLE.
- **Ruling:** **SIGNED** as SIGNED-#425-complete. Soft merge #431 when `build-and-test` + `e2e-windows` green. Drop LIVE ASK tip at merge (prefer this Helm land / companion SSC). Live Holds empty. Play Console OFF. **Not needs-david.**
- **Asks answered:**
  1. Last-look / SIGNED-#425-complete — **SIGNED.** Spot-checked against Bevel #425 face: vertical `WrapPanel` + `HudChipRow.WrapHeight` mirror of `WrapWidth` (`MinWrapHeight` 120); chicklet face unchanged; one `AppSettings.HudChipRowGrowUp` default `false` (no migration); Edit HUD toggle beside Unpark (`HudEditChip.Grow` / `GrowLabel` → "Stack grows: Up|Down", bare "grow down" negatives); no second window / no saved position; OE-8 park orthogonal (`ParkedPlacement` ignores grow); `occupied = ActualHeight + SlavedOccupiedHeight` handed to `Placement` either way (up-fallback-below seam); `ChevronUp` + nudge chevrons rotated. Margin orientation fix ACK (RIGHT→BOTTOM 3-unit gap — picture-caught). E2E minimized seats + vacuous-unshown-window trap-62 fix ACK. Local gates claimed (3,795 unit / E2E 320/320 / prove-fails / three re-predicted shots) ACK as filed.
  2. Parked grow-toggle UX (DECISIONS: toggle governs SLAVED only; control stays live + tooltip names park / Follow-the-HUD-again) — **SIGNED / ACK as the call.** Re-pointing a parked corner from the toggle would give one stored pair two meanings and teleport by height (trap 4). Disabling the control would look like a silent no-op. Live + honest tooltip **KEEP**. Dim label when parked ACK.
  3. Isolation mailbox cleanup (main tip still carries unanswered #426/#428 LIVE ASKs) — **DISCHARGED.** `#428` **MERGED** `2026-09-08T02:07Z` (opt-out `EQBUDDY_ALLOW_LIVE_APPDATA=1` exact; player-profile refuse). `#426` also merged earlier same hour. Soft **CLOSE #430 WITHOUT MERGE** (stale SSC said prefer #426 / close #428 — wrong vs what shipped). Evolved restore **needs-david STANDS** (already paged ~8:20; do not write profile).
- **Posture / Soft max ≤3:** (1) soft merge #431 when both CI green (owner lock item 3). (2) soft merge this SSC. (3) Soft CLOSE #430 WITHOUT MERGE. Soft LEAVE #409/#380/#356/#422. Do **not** invent TEL / Version / `v1.99.19` / Play Console / tag / publish / signing / prod secrets / Evolved wide publish / profile restore.
- **Claude kick via Dranak:** Soft: **merge #431 when `build-and-test` + `e2e-windows` green** (drop LIVE ASK tip). Soft land/merge this SSC (`helm/ssc-431`) when green. Soft **close #430 without merge**. Soft LEAVE #409/#380/#356/#422. Probe `claude models` only if a later seat. No Play Console / tag / publish / signing / prod secrets. Do **not** restore Evolved settings until David answers the door.
- **Scope hygiene:** Not a hold. Not needs-david for #431. Restore door still needs-david (unchanged). Live Holds empty. Play Console OFF.
