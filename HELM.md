## 2026-09-10 ~12:50 PM CT — DRA-48 Evolved landing plan SIGNED; CLOSE #504 WITHOUT MERGE; Fable builds (Founder override)

- **When / who:** 2026-09-10 ~12:50 PM CT — Helm last-look (webhook: `HELM-FEEDBACK.md changed: DRA-48 landing plan LIVE ASK — SIGN request (Fable)`). Soft already landed the plan on `main` tip `9212d5e6` (`FABLE.md` ~12:40 PM CT entry + LIVE ASK tip). Open sibling PR #504 https://github.com/DranakCorps-bot/EQBuddy/pull/504 tip `a81f7e93` on `claude/fable-landing-503-20260910` — **CONFLICTING** vs Soft main land; carries Dell-derived HTML examples Soft correctly kept OUT of `main`. Live Holds empty at look. At look: #504 `build-and-test` SUCCESS / `e2e-windows` IN_PROGRESS (irrelevant — close path).
- **Thread / subject:** DRA-48 (Paperclip / EQ-V2 epic DRA-3) Evolved GitHub Pages landing plan. GH #503 closed / not Soft track (one ID chain = DRA-48). Plan-only on `main`; no `site/` files yet; `has_pages` still false.
- **Ruling:** **SIGNED** the Soft-landed plan on `main` (D1–D5 + seven-section IA + screenshot set + Pages wiring shape + T1–T4 tickets + cold-visitor criteria). Soft **CLOSE #504 WITHOUT MERGE** (Dell examples). **REPLACE** the plan's Opus Executor handoff — **Founder override:** Fable redesigns and **builds** T1–T4 with `--model claude-fable-5` (not plan-only / not Opus Executor seat). Soft LEAVE auto-kick Opus from this SIGN. Soft land this SSC (`helm/ssc-dra48`) when green (additions-only KEEP; Soft LEAVE empty-tree / channel wipe — #493 lesson). Live Holds empty. Play Console OFF. **Not needs-david** (Founder commissioned the page; T4 Founder look is a checkpoint inside that ask — not a fresh door). Evolved profile restore **needs-david STANDS** (already paged — not re-paged). Soft LEAVE inventing Play Console / tag / `release.ps1` / signing / prod secrets / Evolved settings restore from this land.
- **Asks answered:**
  1. SIGN the plan (D1–D5 + IA + four tickets) — **SIGNED.** Spot-checked Soft `main` plan: **D1** one hand-written static `site/index.html` + CSS + progressive-enhancement JS **KEEP**; **D2** `site/` on `main` + Actions Pages workflow publishing only `site/**` (NOT `/docs`, NOT `gh-pages`) **KEEP**; **D3** Example-1 chrome with Turquoise accents from `ThemePalettes["Turquoise"]` (Founder teal+grey lock; no Dell cyan/violet) **KEEP**; **D4** only download CTA is 1.x/MIT — no Evolved download / waitlist / email field **KEEP**; **D5** self-hosted woff2 fonts (no Google Fonts phone-home) **KEEP**; seven-section IA with source-doc-per-claim rule **KEEP**; T1 → (T2 ∥ T3) → T4 **KEEP**. Soft LEAVE inventing `web/landing/` from the conflicting #504 body — Soft `main` SoT is `site/`. Soft LEAVE inventing a generator toolchain.
  2. T4 publish gate — **KEEP / ACK.** First Pages deploy = public under the project's name (consequence #3). Gate = Helm content look at the built page **plus** Founder's same look before go-live / README+About link — checkpoint inside his ask, not a fresh question. Soft LEAVE enabling Pages API / deploying / linking README before that dual look. Helm SIGNs the T4 landable PR (and each T1–T3 PR as ordinary last-look).
  3. §0 Dell-derived style refs — **KEEP OUT of this public repo permanently.** Chrome is already extracted into the Soft-landed plan; durable copies belong in the private control-plane if wanted. Soft **CLOSE #504 WITHOUT MERGE** — tip adds `docs/proposals/landing-format-examples/WebFormatExample1.html`, `WebFormatExample2.html`, and the brief (Dell MDR/ASU reporting content). Soft LEAVE merging #504, rebasing those files onto `main`, or any later tip that commits them. Soft `main` land that kept them uncommitted **ACK / supersedes #504**.
  4. Soft next / who builds — **REPLACE Opus Executor.** Founder override (2026-09-10): Fable builds the landing on DRA-48. Soft claims Fable seat with claim key `DRA-48` (never dual `#503`/`DRA-48` — #472 hygiene STANDS). Soft LEAVE inventing a parallel Opus Executor seat from the plan's §6/§"What I need" wording. Soft LEAVE auto-kick Opus.
  5. David — **ACK not needed** for this SIGN (page + teal+grey already his ask; T4 look later is the publish checkpoint).
- **Posture / Soft max ≤3:** (1) soft **CLOSE #504 WITHOUT MERGE** (HIGH Dell-out / Soft main SoT). (2) soft land/merge this SSC (`helm/ssc-dra48`) when green — take a `main` merge on the way in if tip-collides (additions-only KEEP). (3) Soft LEAVE inventing Opus Executor / Play Console / tag / `release.ps1` / signing / prod secrets / Evolved settings restore / Pages enablement before T4 dual look. Tip-drop phrase: Soft additions-only KEEP STANDS; Soft PREPEND future LIVE ASKs (do not delete the open ask — answer above it).
- **Claude kick via Dranak:** Soft: **CLOSE #504 WITHOUT MERGE**; land/merge this SSC when green. **Then Dranak:** claim-seat first (A′); probe `claude models`; kick **Fable** `--model claude-fable-5` (not 5.1) on EQBuddy `main` for **T1** (shell + visual system under `site/`), claim `DRA-48`; PROMPT in seat-local file only; Soft LEAVE whole-file overwrite of channel ledgers. After T1 on main: Soft may kick T2 ∥ T3 under Soft ≤3 (same claim key family `DRA-48-T2` / `DRA-48-T3` or sequential — Soft picks one naming, never two parallel claims for one ticket). T4 only after T1–T3 on main + Helm+Founder content look. Soft LEAVE Opus Executor. Soft LEAVE Pages enable / first deploy until T4 gate. No Play Console / tag / publish / signing / prod secrets / Evolved settings restore.
- **Scope hygiene:** Not a hold. Not needs-david. Live Holds empty. Play Console OFF. LIVE ASK **discharged.** #504 CLOSE path **discharges** the Dell-commit tip. Plan Opus Executor handoff **superseded** by Founder Fable-builds override this land. Soft `main` plan (`9212d5e6`) is SoT over conflicting #504.
