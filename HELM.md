...
- **Claude kick via Dranak:** Soft: **CLOSE #473 WITHOUT MERGE.** Soft: **merge #472 when `build-and-test` + `e2e-windows` green** (drop LIVE ASK tip / additions-only KEEP). Soft land this SSC (`helm/ssc-472`) when green. Soft LEAVE P1c until #472 on main + Soft ≤3 room. Soft LEAVE inventing mutex script from this land. Probe `claude models` only if a later kick. No Play Console / tag / publish / signing / prod secrets / Evolved settings restore.
...
- **Claude kick via Dranak:** Soft: **rebase #466 onto `main`** (trap-60 byte-safe on `HELM-FEEDBACK.md`) → CI → **merge when `build-and-test` + `e2e-windows` green** (drop LIVE ASK tip). Soft land this SSC (`helm/ssc-466`) when green. Soft: rebase #464 (CONFLICTING) onto `main` then merge, or fold its tip-drop / #462 REJECT block if still owed. Soft: continue #465 gear-menu-slim implement → Desktop soft-captures → LIVE ASK → wake Helm (already AUTHORIZED). Soft LEAVE Bevel seat / invent convert of the four icon tooltips. Soft LEAVE Pass 3 / #462 revive. Probe `claude models` only if a later kick. No Play Console / tag / publish / signing / prod secrets / Evolved settings restore.
...
- **Claude kick via Dranak:** Soft: land/merge this SSC when both CI green. Soft: rebase #463 onto `main` then merge **or** close #463 without merge once this land supersedes it. Soft: **kick Opus** (`--model opus` / `claude-opus-5`, probe `claude models` first) gear-menu-slim implement from signed #460 faces now on main → prove Desktop shots → LIVE ASK → wake Helm. Soft LEAVE Pass 3 / Bevel split invent / title-bar click-through. No Play Console / tag / publish / signing / prod secrets / Evolved settings restore.
...
- **Posture / Soft max ≤3:** (1) rebase + soft merge #451 when both CI green (HIGH owner Guide door / signed faces). (2) soft land this SSC. (3) Soft LEAVE invent beyond signed faces / Options `Cards & windows`→HUD rename (faces §D deferred). Soft LEAVE #435 invent. Do **not** invent TEL / Version / `v1.99.19` / Play Console / tag / publish / signing / prod secrets / Evolved settings restore.
- **Claude kick via Dranak:** Soft: **rebase #451 onto `main`** (trap-60 byte-safe on `HELM-FEEDBACK.md`) → CI → **merge when `build-and-test` + `e2e-windows` green**. Soft land this SSC (`helm/ssc-451`) when green. Soft: no new Opus seat from this ruling (rebase existing tip). Probe `claude models` only if a later kick. No Play Console / tag / publish / signing / prod secrets / Evolved settings restore.
...
- **Posture / Soft max ≤3:** (1) rebase + merge #414 when both CI green (HIGH owner-active Thorns under-read). (2) soft merge this SSC. (3) Soft LEAVE #410 Money peek / #409 Deaths strip / #380/#356 as standing. Do **not** invent ranked durations / mote multipliers. No live eqlwiki scrape.
- **Claude kick via Dranak:** `--model opus` / `claude-opus-5` (or executor already mid-seat): **rebase #414 onto current `main`** → CI → **merge when `build-and-test` + `e2e-windows` green**. Soft merge this SSC when green. Soft: do not open a ranked cast-correlation seat from this ruling. Probe `claude models` first if a fresh kick. No TEL / Version / `v1.99.19` / Play Console / tag / publish / signing / prod secrets / Evolved wide publish / live eqlwiki harvest blast.
- **Scope hygiene:** Not a hold. Not needs-david. Live Holds empty. Play Console OFF.
...
- **Posture / Soft max ≤3:** (1) rebase + merge #414 when both CI green. (2) soft merge this SSC. (3) Soft LEAVE #404 peek-anchor; Soft TR-1 Evolved SCR silent-zero / `TargetNotEmpty` when Soft has room (**not** this PR). Soft LEAVE wiki ranked harvest. Do **not** invent ranked durations / mote multipliers. No live eqlwiki scrape.
- **Claude kick via Dranak:** `--model opus` / `claude-opus-5` soft path (or Soft merge without new seat): **rebase #414 onto `main`** → CI → **merge when `build-and-test` + `e2e-windows` green**. Soft land this SSC (`helm/ssc-414`). Probe `claude models` only if a new Claude seat is needed. No TEL / Version / `v1.99.19` / Play Console / tag / publish / signing / prod secrets / Evolved wide publish / live eqlwiki harvest blast.
- **Scope hygiene:** Not a hold. Not needs-david. Live Holds empty. Play Console OFF.
...
- **Diagnosis ACK (measured):**
  ...
  2. EQBuddy opens all 28 Thorns landings at **900 s / 15:00**. No code path produces 23:xx. Owner's game window ~23:36; log landing→fade pairs measure **24:28** and **23:55**. Chip under-reads a ~24 min damage shield by ~9 min.
...
- **Spot-check (product `602d114c`):**
  ...
  - Total `HudExpand.DestinationOf` (four hosts: float / Progress / Creature / World); Procs→Damage float; Motes+Money→Progress Wealth; Kills→Creature; Deaths→World Travels; lock-7 destination-keyed.
...
- **Posture / Soft max ≤3:** (1) Opus settings-reset diagnose HIGH; (2) Bevel Options/cog IA + Loot peek empty-state copy if needed; (3) Fable OE-9 seats (+ Loot peek seat if separate). Soft merge #385 (or this superseding SSC PR). #356 Setup LEAVE. #380 wiki LEAVE. Do **not** kick OE-9 Opus yet. Do **not** invent TEL / Version / `v1.99.19` / Play Console / tag / publish / signing / prod secrets.
- **Claude kick via Dranak:** Soft merge this SSC / #385 when both CI green. Soft seats with explicit `--model` (probe `claude models` first):
  - `--model opus` / `claude-opus-5`: diagnose settings-reset-on-Evolved-publish (AppData / profile path / migration / publish clobber) → fix after named root, or diagnose PR + LIVE ASK if door-shaped.
  - `--model sonnet` / `claude-sonnet-5`: Bevel Options/cog IA pass + Loot peek target/empty-state.
  - `--model claude-fable-5`: Fable seat OE-9 (+ Loot peek if separate) from these locks → LIVE ASK → wake Helm. **No OE-9 Opus until that sign.**
- **Scope hygiene:** Not a hold. Not needs-david. Live Holds empty. Play Console OFF.
...
- **Posture:** Soft max ≤3: land #381 + this SSC + soft Bevel face one-liner. Soft-owed: drag-verify screen.lock. #356 Setup LEAVE (e2e-red). #380 wiki refresh LEAVE. Do **not** kick OE-9 yet.
- **Claude kick via Dranak:** CI watch → drop LIVE ASK tip → **merge #381 when both green**; land this SSC (`helm/ssc-381`) on main. Soft: `--model sonnet` / `claude-sonnet-5` Bevel affordance-face one-liner (drag grip tell + un-park face) → LIVE ASK only if material. Soft-owed own item: `drag-verify.ps1` takes `%TEMP%\eqbuddy-screen.lock`. Do **not** kick OE-9 / TEL / Version / `v1.99.19` / Play Console / tag / publish / signing / prod secrets.
- **Scope hygiene:** Not a hold. Not needs-david. Live Holds empty. Play Console OFF.
...
- **Posture:** Soft max ≤3: #377 land + OE-8 implement + soft docs. Soft close CONFLICTING #376 (superseded by merged #375). Soft close CONFLICTING leftover channel #371/#366 (SSC already on main). #365 OE-4 LEAVE. OE-9 waits a slot + OE-8 API.
...
- **Claude kick via Dranak:** `--model claude-fable-5` amend `FABLE.md` OE-7/8/9 to this Bevel-signed firming (OE-7 absorbs BreakoutKind summon; OE-8 trap-2 plan; OE-9 carve-outs; lock 6 ride-along) → LIVE ASK if material → wake Helm. Soft: merge #371/#370 when green; main wiki-pack e2e named diagnose still gates OE-7 Opus. Do **not** kick Opus OE-7 implement yet.
...
- **Posture:** Rebase onto current `main` first, drop LIVE ASK tip (or leave Helm main land), then **merge when `build-and-test` + `e2e-windows` green on rebased head**. Blind re-runs STOPPED. No e2e waive. Soft max ≤3: prefer this PR-3 land over OE build seats. Live Holds empty. Play Console OFF. Not needs-david.
- **Scope hygiene:** No OptionsWindow / TEL / Version / `v1.99.19` / Play Console / tag / publish / signing / prod secrets. Not a hold. Claude kick via Dranak (`--model opus` / `claude-opus-5`): **rebase #363 → CI → merge when both green**; then watch N=1 main e2e; rebase #357 after; leave OE merges parked.
...
- **Scope hygiene:** Docs/channel. No OptionsWindow retirement / TEL / Version / `v1.99.19` / Play Console / tag / publish / signing / prod secrets. Not a hold. Claude kick via Dranak (`--model opus` / `claude-opus-5`): **PR-3 fix now** from this signed plan / tip `FABLE.md`. Soft under Soft max ≤3: merge #360/#359 when green; leave OE merges parked; #357 rebase after PR-3 lands.
